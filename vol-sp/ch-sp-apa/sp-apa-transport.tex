...
%The purpose of the \dword{apa} transport frame is to provide a safe and strong transport environment to send \dword{apa}s from production sites to the underground clean room in the detector hall at \dword{surf}. The main requirements are to provide an enclosure for a pair of \dword{apa}s, allow lifting and rotating to many different positions, and be compact enough to go down the mine shaft to the detector location. 
...
After unpacking an \dword{apa}, a visual inspection will be performed and wire continuity and tension measurements will be made again. % with the electrical method used during production and described in Sec.~\ref{sec:fdsp-apa-prod-qa-tension}. %for a sample of approximately $\sim$10\% of the wires (around \num{350} wires). 
...
Chapter~\ref{ch:sp-install} of the \dword{dune} TDR. \dword{apa} consortium personnel will play direct and key roles throughout the integration and installation activities.  %testing must be finalized using information from the production sites and the installation schedule.   Other details of integration and installation can be found in Chapter~\ref{ch:sp-install}.  A set of final checks, following installation in the cryostat, will be performed to confirm the functionality of the \dword{apa}s in their final positions.  
