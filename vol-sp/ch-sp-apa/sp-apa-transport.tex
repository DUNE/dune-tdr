%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Handling and Transport} % and Receiving at the DUNE Integration Facility}
...
For details on installation activities at \surf, see Chapter~\ref{ch:sp-install}. %the Technical Coordination chapter in the \dword{dune} TDR.  %Here we limit the discussion to delivery of \dword{apa}s to the \dword{itf} and quality checks once there to ensure each \dword{apa} has arrived without damage. 
...
The handling of the \dword{apa}s throughout their lifetime must be carefully considered to ensure their safety.  Several lifting and handling fixtures will be employed for transferring and manipulating the \dword{apa}s during fabrication, integration and installation.  In the factories a fixture called the ``edge lift kit'' will be used to transfer the \dword{apa} to and from the process cart and the winder, and to the transport containers.  See Figure~\ref{fig:apa-edge-lift}.  %The edge lift kit will also be used to transfer the \dword{apa} from the transport frame to a process cart there.  
...
The purpose of the \dword{apa} transport frame is to provide a safe and strong transport environment to send \dword{apa}s from production sites to the far detector. The main requirements are to provide an enclosure for a pair of \dword{apa}s, allow lifting and rotating to many different positions, and be compact enough to go down the shaft at \dword{surf}. 
...
{Transport frame concept for transporting two \dword{apa}s between a production site and the \dword{sdwf} and between the \dword{sdwf} and \surf for installation.}  
...
All active detector components are shipped to a \dword{sdwf} before final transport to \dword{surf}. During the storage period, the wire tension will be measured on all APAs using the electrical method to ensure that the transport has not damaged the wires. The advantage of the electrical method here for measuring the tension is that the APA can stay in the crates during the measurements. Integration and final testing will take place underground. %; there we will have more time available to perform tests. 
...
Chapter~\ref{ch:sp-install} of the \dword{dune} TDR. \dword{apa} consortium personnel will play direct and key roles throughout the integration and installation activities.  %testing must be finalized using information from the production sites and the installation schedule. 
...
After unpacking an \dword{apa}, a thorough visual inspection will be performed and wire continuity and tension measurements will be made with the electrical method used during production and described in Sec.~\ref{sec:fdsp-apa-prod-qa-tension}. %for a sample of approximately $\sim$10\% of the wires (around \num{350} wires). 
%The default technique is the laser method that was used for \dword{pdsp}.  The method works well, but it is time consuming, so %the 
%a more efficient alternative that uses the electrical method described in Section~\ref{sec:fdsp-apa-prod-tooling} is also %being considered for use here. Improved methods %would reduce the time spent on measuring and 
%could allow sampling of more wires (even the full \dword{apa}). 
...
This guidance will be based on the \dword{pdsp} experience, where the tension of some wires changed during the production to installation process. In addition, a continuity test and a leakage current test is performed on all channels and the data recorded in the database. 
...
\fixme{99\% is threshold for success? clarify. anne} the \dword{apa} can be prepared for integration with the other components.  Other details of integration and installation can be found in Chapter~\ref{ch:sp-install}.  A set of final checks, following installation in the cryostat, will definitely be performed to confirm the function of the \dword{apa}s in their final positions.  

\fixme{can we say more about these checks?  what was done in ProtoDUNE?}
