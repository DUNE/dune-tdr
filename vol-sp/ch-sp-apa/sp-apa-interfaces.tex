...
for information on all parts of the \dword{ce} system.
...
The \dword{pds} is integrated into the \dword{apa} frame to form a single unit for detecting both ionization charge and scintillation light.  The \dword{apa} frame design must also accommodate cables for the \dwords{pd}.  
...
The interface between the \dword{pds} and \dword{apa}s involves the mechanical hardware design and production, cable routing, and integration, installation, and testing procedures. The electrical interface includes a grounding scheme and electrical insulation. The strict requirements on noise from the \dword{ce} means the electrical interface must be defined together with the \dword{sp} \dword{tpc} electronics consortium. 
...
{Cable routing scheme. Left: Conduit for the \dword{ce} cables protruding from the end of the long side tubes of the \dword{apa} frame.  Right: One of the \dword{ce} cable bundles being pulled through conduit of equal length to the two stacked \dword{apa}s.  The cable bundle is wrapped with a protective cover of braided PET plastic.  }
