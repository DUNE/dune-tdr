...
commissioning. The hardware interface has two basic components: mechanical and electrical. 
...
The \dword{pds} is integrated into the \dword{apa} frame to form a single unit for detecting both ionization charge and scintillation light.  The \dword{apa} frame design must also accommodate cables for the photon detectors.  
...
The interface between the \dword{pds} and \dwords{apa} involves the mechanical hardware design and production, cable routing, and integration, installation, and testing procedures. The electrical interface includes a grounding scheme and electrical insulation. The strict requirements on noise from the \dword{ce} means the electrical interface must be defined together with the \dword{sp} electronics consortium. 
