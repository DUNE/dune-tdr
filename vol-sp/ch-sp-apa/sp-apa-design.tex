...
\begin{comment}
\begin{dunefigure}[Photos of APA grounding mesh application in \dword{pdsp}]{fig:tpc-apa-mesh-application}
...
In addition to the bias and DC-blocking capacitors for all $X$ and $U$-plane wires, the \dword{cr} boards include two R-C filters for the bias voltages\footnote{The $V$-plane does not carry a bias voltage, so does not require these components.}. The resistors are of the same type used for wire biasing except with a resistance of \SI{5}{M$\Omega$}, consisting of two \SI{10}{M$\Omega$} resistors connected in parallel. Wire plane bias filter capacitors are \SI{39}{nF}, consisting of ten \SI{3.9}{nF} surface-mount capacitors connected in parallel. They are the same capacitors as those used for DC blocking.
...
to the \dword{dss}.]{fig:apa-yoke} % I got rid of doublet in other chaps
