...
early calibration needs to track %to track %JM
...
\input{generated/req-longtable-SP-CALIB.tex}
...
towards the center of the cryostat are placed near the \dwords{apa} where the \efield is small \fixme{check} to minimize any risks due to the \dword{hv} discharge. For the far east and west ports, \dword{hv} is not an issue as they are located outside the \dword{fc} and the penetrations are located near mid-drift (location favorable to possible source deployment).
