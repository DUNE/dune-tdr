...
The eight calibration ports closer to the end-walls (four on each side) are not positioned on top of the \dword{tpc}, but instead located about \SI{40}{\cm} away from the \dword{fc} along the $z$ direction. In this case \dword{fc} penetration would be quite complicated, having to come from the sides. If the periscope baseline design would be used for the end-wall periscopes, then the volume coverage would be quite limited, similarly to the coverage limitation mentioned in Section~\ref{sec:sp-calib-sys-las-ion-des}
