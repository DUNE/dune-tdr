...
The periscopes on top of the \dword{tpc} can be installed after the relevant structural elements, especially the to \dword{fc} modules. Installation should proceed in sequence with the assembly of other components, with the furthest from \dword{tco} assembled first. The \dword{lps} can be used to align the periscopes as they are installed, so it is important that the \dword{lps} is also installed in the same sequence as the periscopes. %\fixme{Please check the previous sentence for accuracy. It was not clear if the reference in the sentence was to periscopes or \dword{tpc} components.}
