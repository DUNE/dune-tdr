...
%Earlier sections described tests in member institute laboratories 
%needed 
%to converge on a baseline design. 
...
%in such a system 
in mechanical and optical requirements, long-term reliability, laser track length, performance of the \dword{lbls}, \dword{daq} interface, and effect on \efield with alternative design options all lead to corresponding goals for a test installation and operation in \dword{pdsp} that could be done in the post-LS2 run. As Figure~\ref{fig:protoDUNESP_topView_marked} shows, \dword{pdsp} has ports of the same size as the \dword{dune} \dword{fd} that could be used for these tests. If a pair of ports can be used, then one could even have crossing tracks within a single drift volume. 
