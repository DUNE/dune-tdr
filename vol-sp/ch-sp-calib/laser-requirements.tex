...
In addition, we need to consider that the method used to estimate \efield distortions is based on obtaining position displacement maps~\cite{bib:uBlaser2019}, and that the comparison between the reconstructed and true direction of a single track does not %univocally 
...
(see \dword{tdr} \spchapa). In the $y$ and $z$ coordinates, the wire pitch of \SI{4.7}{\mm} meets this requirement while in the drift ($x$) direction, the position is calculated from timing, so we could claim that it should be known better.
