...
Additional systems are installed in parallel with the \dword{apa}s, e.g., the \dword{fc} and their \dword{hv} connections, elements of the \dword{cisc}, and detector calibration systems. The cathode plane, \dword{fc}, and \dword{apa} together define the \dword{tpc} active volume. \fixme{the outer APAs and FC define it, don't they? Anne 10/17}
...
\input{generated/risks-longtable-SP-FD-INST.tex}
...
Unemployment in the Lead area remains low. At the time work is ramping up it may be difficult to find local people with the requisite skills. To mitigate the risk, we plan to hire the core team early and assume they will be trained at \dword{ashriver}. \fixme{make sure they get trained?} This will enlarge the potential %possible 
pool of candidates by relaxing the requirements,\fixme{what requirements does this relax?}  and allow a longer hiring period if needed. The residual risk is considered low.
