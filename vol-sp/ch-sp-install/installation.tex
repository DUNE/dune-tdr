...
\includegraphics[angle=90,height=.99\textheight]{graphics/high-level-schedule}
\end{dunefigure}
...
%\includegraphics[width=.85\textwidth]{graphics/install-infrastructure-top}
...
\begin{dunefigure}[Installation of electronics crosses]{fig:install-elect-cross}
 ...
 \includegraphics[width=.5\textwidth]{graphics/install-elect-cross}
\end{dunefigure}



Once the crossing tubes are installed and leak-checked, we connect the  \dword{tpc} electronics crosses and mount  the  \fdth flanges for the \dword{ce} and \dwords{pd} onto the crosses. 
...
The primary changing room is in the southwest corner of the cleanroom and a smaller changing room (shown in Figure \ref{fig:install-cleanroom}) will be situated near the stairs at the \dword{4850l} for people accessing the work platforms.
...
\includegraphics[width=0.95\textwidth]{graphics/CISC-feedthru.pdf}
\end{dunefigure}
...
\begin{dunefigure}[Distribution of various CISC devices inside the cryostat.]{fig:cisc_devices}
  ...
  \includegraphics[width=0.98\textwidth]{graphics/cisc_distribution.png}
  \includegraphics[width=0.85\textwidth]{graphics/cisc_distribution_legend.png}
\end{dunefigure}
...
{One row of the \dword{apa} and \dword{cpa} with associated \dwords{fc}. The \dwords{fc} are shown deployed in the final orientation. The equipment in the figure represents 1/25 of the total \dword{tpc}.}
 \includegraphics[width=0.3\textwidth]{graphics/install-single-row}
\end{dunefigure}
...
\includegraphics[width=0.95\textwidth]{graphics/Single-APA-Schedule.pdf}
\end{dunefigure}
...
\includegraphics[width=0.95\textwidth]{graphics/install-apa-prep}
\end{dunefigure}
...
\includegraphics[width=.9\textwidth]{graphics/apa-assembly-v4}
...
\includegraphics[width=.9\textwidth]{graphics/install-cpa-assemble}
\end{dunefigure}
...
\begin{dunefigure}[Model of the electronics and photon detector cabling]{fig:install-cable-routing}
  {Working model of the cable trays and routing of the \dword{pd} cables in the trays. The scale is set by the \SI{2.3}{\meter} \dword{apa} width.}
\includegraphics[width=.95\textwidth]{graphics/install-cable-routing}
\end{dunefigure}
...
\includegraphics[width=.9\textwidth]{graphics/install-cpa-fieldcage}
\end{dunefigure}
...
\includegraphics[width=.9\textwidth]{graphics/install-row25}
\end{dunefigure}
...
At this point, the cryostat is classified as a confined space and the corresponding safety measures are required. (These measures include a search of the area before closure, confined-space training for workers, and controlled access into and out of the cryostat. ) After the \dword{tco} is closed the four access holes (two for ventilation and two for personnel access)  can be closed and the pulsed neutron source can be placed in position above two of the access holes. The east end of the cryostat is then cleaned and the plastic sheeting is removed
...
\includegraphics[width=0.49\textwidth]{graphics/NOvA-Assembly-Area}
\vspace{-12pt}
\includegraphics[width=0.7\textwidth,trim=0pt 0pt 0pt 0pt,clip]{graphics/DUNE-Trial-Assembly-Ash-River}
\end{dunefigure}
...
and measurement of the baseline and \dword{rms} of the noise for 
...
in a laboratory on the surface at \dword{surf} or at a nearby institution to perform checks at \dword{ln} temperature
...
down to a temperature close to that of \dword{ln} in the \coldbox. The bias voltage
...
result in the loss of one entire \dword{femb}. 
...
through the \dword{apa} frames. 
%The connections between the \dword{apa} wires and the outside world are extensively tested. 
The \dwords{femb} are tested immediately after installation, after the cables are installed, and during and after thermal cycling. This ensures that the connections are robust before the \dword{apa} enters the cryostat.  Testing at \dword{ln} temperature, done with the final cables attached, indicates clearly the capacitance of the wires and it verifies that the connections to the \dwords{femb} and the cables is maintained during thermal cycling.  After installation in the cryostat the \dwords{femb} are monitored continuously. 
%
...
\includegraphics[height=.40\textheight, angle=0]{graphics/pds-scanner}
\end{dunefigure}
