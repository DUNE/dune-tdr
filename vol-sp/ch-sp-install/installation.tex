...
%The \dword{jpo} labor includes a scientific lead, a manager, two deputy managers (one manager on each shift), three safety officers (one on each shift), and administrative help. 
The \dword{jpo} detector installation team includes the underground cavern
coordinator and deputy, and the detector installation manager, along with
the far site \dword{esh} coordinators (one for each shift). They are responsible for communicating with the logistics organization to ensure that needed components are shipped underground and properly maintained in the inventory system.  
