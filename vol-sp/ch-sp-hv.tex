...
\item SP-FD-16: This downtime requirement is already met in \dword{pdsp}; the much lower ionization density in underground operation and optimization in the design (\dword{fc} to \dword{gp} distance) will ensure meeting the requirement even in the case of the much wider detector surface. \fixme{no reference found, note SP-FD-16 is not included in the table above} 
...
must withstand a large over-power condition.  Radially out from the resistor is an insulator. \fixme{weird sentence. Maybe: ``A cylindrical insulator is placed around the resistor''?}
...
\end{minipage}
\end{dunefigure}
\fixme{check figures - I adjusted relative widths}
...
\fixme{sounds weird to ``equip a voltage'' - maybe ``the voltage of the \dwords{gp} above and below each drift region can be monitored via a high-value resistor connecting the \dword{gp} to the cryostat. ''?}
...
\fixme{check RP} %Resistive panels (RPs) 
...
Argonne National Laboratory shipped the \dword{cpa} material to the detector hall as single pre-assembled resistive panels held in a \frfour frame; i.e., as \dword{cpa} units (Table~\ref{tab:cpaparts}). \fixme{check if `unit' is right}
In the cleanroom adjacent to the \dword{pdsp} cryostat, three \dword{cpa} units were mechanically and electrically connected to produce a \dword{cpa} panel. %unit 
...
\fixme{should add to table 3: FC EW, count=8, L=3, W=3.5, H=- Gr Tot=8}
...
In addition, these processes seem to be enhanced by the \dword{lar} bulk high purity, which allows the electric current to develop. \fixme{sentence not clear enough: what's bulk high purity, and it sounds like high purity is bad.}
...
\fixme{the year=2019? still monitoring? and do you want \dword{pdsp2}?}
...
\item \dword{ewfc} module assembly: 1.5 days/module with 2 workers, \fixme{EW ``module'' not defined in table 3} 
...
The process of connecting individual endwall modules \fixme{module?? (several instances)} to build an endwall exposed another alignment issue.
...
The \dword{cpa} units are assembled horizontally on a smooth, flat, highly stable table %. The %single table is needed 
...
\fixme{seems like 6,7 could also apply to ops phase, no?}
