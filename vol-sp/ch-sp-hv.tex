...
In DUNE, although the minimum requirement of the drift electric field has been set to 250 V/cm, the goal in the design of the HV system is to reach an electric field of 500 V/cm for long term operation under stable conditions. This can compensate for unexpected low purity conditions that could arise over the two decades of planned operations. The positive ProtoDUNE experience indicates that this goal is within reach. Indeed, attempts to run ProtoDUNE SP at a higher HV value (up to $\sim$10\% as allowed by HV cables and filtering systems) are under consideration for a better understanding of the HV stabilty issues.
...
and that the electronics signal-to noise ratio is comparable to that achieved in ProtoDUNE SP. 
...
%\end{dunetable}
We note that SP-FD-1 is discussed in the text above the table. SP-FD-2  is met in case of stable HV operation (lessons from protoDUNE). The noise contribution from HV instabilities is unclear and under investigation with protoDUNE. The remaining requirements specific to the HV system are all addressed and referred to in the remainder of this document. Here is a summary. SP-FD11: Non-uniformity could be due to defects in resistor chains; muon and laser calibrations will mitigate this effect. SP-FD-12: Lessons learned from ProtoDUNE demonstrate that the present filtering scheme is adequate. SP-FD-16: This down time requirement is already met in ProtoDUNE; the much lower ionization density in underground operation and optimization in the design (FC to GP distance) will ensure meeting the requirement even the case of the much wider detector surface. SP-FD-17: The CPA design is based on few MOhm/square resistivity surfaces. Such surfaces have been demonstrated in ProtoDUNE to be adequate to prevent fast discharges that could potentially damage CE and the cryostat (no event was ever recorded). Underground operation will allow higher resistivity thus slowing further down the potential release of stored energy. SP-FD-24 is met by calculation in ProtoDUNE. the present design is further reduces the E-field in the critical region between FC and GP. SP-HV-1: The HV distribution and filtering has been tested in ProtoDUNE; the design of these items will be revised to minimize long term degradation and maintenance requirements. SP-HV-2: Two point connections to the CPA are foreseen. The HV feed through and its connection to the CPA is designed in such a way that it could be extracted and replaced even with the detector filled with LAr (based on ICARUS experience).
...
operating potential. Only the region close to the discharge site shifts positively toward 0V. Charge injection to the wire readout electronics, proportional to $dV/dt$ averaged over the cathode area facing an APA, is therefore greatly suppressed. 
}
\centering
...
All structural members of the CPAs and FCs are made of either FR4 or FRP with very similar CTEs. However, the structures supporting the CPAs and FCs are made of stainless steel, whose CTE is about 50\% greater.  To accommodate the mismatch in the CTEs, small expansion gaps are added between CPAs at installation time. These gaps are set during installation between CPA panels by adjusting the distance between the CPA hanger bars and between CPA planes at the top of the TPC on the CPA beam - a total of 49 - 3 mm gaps which will disappear once the TPC is submerged in \lar at operating temperature.
...
This separated setup accommodates any necessary different running voltages between the two CPA arrays.
...
There are at least two instances of electrical connections on the \dword{cpa} array and between the \dword{cpa} array and other \dword{hv} system components (top, bottom, and \dwords{ewfc}) and four connections between \dwords{rp} in a \dword{cpa} unit.  Each of the different types of electrical connections on the CPA were tested in a \dword{ln} tank at \dword{bnl}~\cite{bib:docdb2338}  and in ProtoDUNE. There were no failures at BNL and none at ProtoDUNE.  The \dword{hv} connection from the \dword{hv} power supply is a closed loop around the \dword{cpa} that can sustain at least one broken connection without loss of the cathode \dword{hv}.  This ensures compliance with requirement SP-HV-2.
...
boards \fixme{check - boards or chains} are connected in parallel to each CPA array for a total of about $158~\mu A$, well within the capability of the selected HV power supply.
...
In a similar way, QC checks of connections between CPA modules and between CPA and FC modules were performed after installation.
...
QC tests on the HV components of ProtoDUNE required many measurements to be made with several different test devices.  Extrapolating these measurements to the scale of DUNE will require development of dedicated tools so that the \dword{qc} process can be made more efficient and optimal at each step.
...
\item CPA (2-panel) plane - 2 days/plane with 4 workers,
...
\dword{cisc} & \dword{hv} vs. \dword{lar} level interlock, sensor locations in high field regions, cold/warm camera coverage, \dword{hv} signal monitoring, etc. & \citedocdb{6787} 
 \\ \colhline
\dword{itf} & Storage buffer, inspections/tests, repackage for underground delivery & \citedocdb{7039} 
 \\ \colhline
...
The \dword{hv} feedthroughs and filters are custom devices. As for ProtoDUNE, the feedthroughs  designs are made by collaborators and fabricated by an external company or major laboratory.
 Raw materials such as stainless steel, \dword{uhmwpe} rods, and flanges are readily available and are machined to make a feedthrough. Similarly, the resistors, steel or aluminum, and insulator material for the filters are readily available. The feedthroughs and filters require testing before being delivered to the ITF.    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The \dword{cpa} units are assembled horizontally on a smooth, flat table to provide a highly stable surface. There is one table per factory with up to 3 factories making CPAs. The single table is needed to ensure flatness and straightness of the entire panel before units are pinned together.
...
Firms that specialize in the machining of fiberglass components for electrical applications will produce the FRP and \frfour components of the top and \dwords{botfc}, as was successfully done for \dword{pdsp}. \fixme{removed but on punch list for next version: All parts are to be machined in the absence of water to minimize absorbed water in the \dword{tpc}, and then cleaned with a lacquer thinner. }
...
Aluminum profiles are inserted into the cutouts of the box beams and attached with screws and stainless steel slip nuts to L-shaped FRP brackets that are mounted on the FRP box beams. Small changes in part sizes will help to simplify the assembly  procedure with respect to the one adopted for ProtoDUNE. Currently we expect that pre-assembly of the FC endwall frames is no longer required. The full modules will be assembled at the factory (LSU) and then complete FC endwall panels will be shipped to the \dword{sdwf}. 
...
Testing of the \dword{hv} feedthrough will be done in a closed cryostat leading to no exposed high voltage.  Special attention will be given to the grounding between the power supply and cryostat. Testa are performed in a closed liquid argon cryostat to assure the nominal voltage is functional.  There is no exposed \dword{hv} during testing.  The power supply is grounded to the cryostat as a further safety measure . Equipment will undergo safety review. 
...
For DUNE, the purity requirements of the \lar place restrictions on the introduction of material into the cryostat, precluding either permanent tags or ink or paint markings on the detector.  We will implement a system of temporary tags containing QR\footnote{\textit{Quick Response Code}, The QR\texttrademark{} code system was invented in 1994 by the Japanese company Denso Wave. \url{https://www.qrcode.com/en/index.html}.} %The use of QR code technology is freely licensed as long as users follow the standards for QR Code documented with JIS or ISO. 
...
In the current installation scenario, no assembly activities are foreseen at the \dword{sdwf} site for any components of the HV system. Only visual inspection of the HVS modules crate condition will be performed to verify the integrity after shipping. %\This only requires the opening and the successive re-closing of the shipping crates. No specific safety procedures are associated with these operations. 
No disruption in installation should occur in the event of shipping damage since there are two week's installation storage underground at \surf and one month's storage at the \dword{sdwf}.  The HV consortium will coordinate procedures for underground handling with Technical Coordination.

A detailed Gantt chart on the production and installation schedule for the HV System of the first \dword{spmod} is shown in Figure~\ref{fig:tasks}.
...
\begin{comment}  Anne changed to std format 4/16
\begin{dunetable}
...
\fixme{new standard risks table. Anne}
...
Table~\ref{tab:risks:SP-FD-HV} presents a summary of the risk items identified for the HV system of  the far \dword{spmod}. 
...
Risk 1: Open circuit on the FC could happen if a resistor in a conventional voltage divider fails open. Such a scenario could result in HV discharges across the open circuit gap. Mitigation: perform stringent component selection and cryogenic testing. Use parallel resistors chains to provide redundancy. Varistors, capable of withstand several thousand amperes of current impulses, are added in parallel with the resistor chains to protect them from large current surges. Resistance are checked several times during FC fabrication and assembly phases, including one after the FC deployment.   
...
Risk 8: Sudden discharge on the HV system injects charge to the front end ASICs, overwhelming the protection circuits and causing permanent damage. Mitigation: key aspects of the HVS design were aimed at reducing the charge injection to a safe level for the CEs: segmenting the field cage, and making the cathode planes resistive.  We are still searching for higher resistive material on the cathode to increase the safety factor. 

Risk 9:  Each cathode is made of light weight non-porous material with an area of 58\,m $\times$ 12\,m.  It could move under the LAr's convectional flow.  Mitigation: CPA structure is designed to withstand pressure from LAr flow based on fluid model predictions; Static deformation can be calibrated by lasers or cosmic rays.

Risk 10: Ageing of insulator components in LAr could be an issue,  but  experience in Atlas (Kapton \& PCB, 20+ years), Icarus (G10, feedthrough 4+ years; feedthrough exchangeable) is trending favorably. Mitigation: more experience will be gained with liquid argon TPC's. Make feedthrough exchangeable.
...
Risk 12: Add SWF contingency, full-scale installation trial at Ash River site prior to installation. Estimates based on ProtoDUNE experience. With the present knowledge, the HV system is not on the critical path for installation.
...
Table~\ref{tab:HVsched} gives the most high level milestones for the design, test, production and installation of the HV System of the \dword{spmod}. Dates in this tentative schedule are based on the assumed start of installation of the first \dword{spmod} at \surf. The dates for the HVS production of a second \dword{spmod} are also included as a reference.
`
 The production scenario assumed for the schedule presented in Table~\ref{tab:HVsched} is based on two factory sites for the CPA construction, two for the top/bottom FC modules and one for FC end wall modules. Given the present starting date for installation underground of the first \dword{spmod}, this assumption is fully compatible with the time available after the operation of the ProtoDUNE-II prototype.
...
{tab:HVsched}
{High level Milestones and Schedule for the production of the HV System of the \dword{spmod}}   
...
Gantt chart providing a detailed view of the production and installation schedule for the HV System of the first \dword{spmod}.}
%\includegraphics[width=0.48\textwidth]
%{fc_endwall_detail_side}
