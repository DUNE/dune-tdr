...
\fixme{Anne - looks like they need top level 1 and 2 in their spreadsheet now that the top five are no longer auto-included; labels are { spec:min-drift-field } and { spec:system-noise }, resp}
...
\fixme{removed funding stuff (and therefore risks 13-14; schedule risk should still be here? anne}
...
%Item [ID08] 
Risk 8 requires an accurate analysis of collected muon data (this activity is in progress), and the disentangling of space charge effects. \fixme{check risk numbers; 8 seems wrong. anne}
...
apply to the \dwords{detmodule}. However, the positive experience gained with \dword{protodune} justifies the low risk probabilities assigned to most of the items. The same is true for their impact on the cost and schedule. \fixme{remove prev sentence?} To better justify these statements, brief explanations are given below, together with the identified mitigation actions.
...
leading to feed-down \fixme{flow down?} of high-energy neutrino backgrounds to low-energy reconstructed categories.  Mitigation: Consider addition of a laser calibration %will be considered 
...
The mitigation for risk 7 involves improvement at the design level to increase as much as possible the distance between the \dword{fc} and \dwords{gp}, and to avoid high-field regions by smoothing all electrodes exposed at \dword{hv}. \fixme{to or at?} %ProtoDUNE NP02, 
