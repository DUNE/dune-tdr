...
Cathode Plane Assembly (CPA) arrays are made up of adjacent resistive cathode panels, secured in frames and connected by a \dword{hv} bus. A \dword{hv} cups are mounted at both ends to receive input from the power supply.
...
An internal study\cite{bib:docdb1320} has shown that with a metallic cathode structure, an HV discharge could swing the outer wire plane by nearly 100V, and inject 0.9A current into the input of the front end amplifiers connected to the first induction plane, possibly overwhelming the internal electrostatic discharge (\dword{esd}) protection in the \dword{fe} \dwords{asic}.  
...
Understanding the origin of the "localized current streamers" is one of the goals of the long term operation of ProtoDUNE SP in 2019 exposed to cosmic rays.
...
No disruption in installation should occur in the event of shipping damage since there are two week's installation storage underground at \surf and one month's storage at the LW.  The HV consortium will coordinate procedures for underground handling with Technical Coordination.
...
[ID01] Open circuit on the FC could happen if a resistor in a conventional voltage divider fails open. Such a scenario could result in HV discharges across the open circuit gap. Mitigation: perform stringent component selection and cryogenic testing. Use parallel resistors chains to provide redundancy. Varistors, capable of withstand several thousand amperes of current impulses, are added in parallel with the resistor chains to protect them from large current surges. Resistance are checked several times during FC fabrication and assembly phases, including one after the FC deployment.   

[ID02] Local limited scratches could be possible, due to accidental contact operation during module assembly or installation. Mitigation: No operation is required if scratch is limited in size. For larger scratches that can induce delamination, replace with a spare panel. 
...
[ID08] Sudden discharge on the HV system injects charge to the front end ASICs, overwhelming the protection circuits and causing permanent damage. Mitigation: key aspects of the HVS design were aimed at reducing the charge injection to a safe level for the CEs: segmenting the field cage, and making the cathode planes resistive.  We are still searching for higher resistive material on the cathode to increase the safety factor. 

[ID09]  Each cathode is made of light weight non-porous material with an area of 58\,m $\times$ 12\,m.  It could move under the LAr's convectional flow.  Mitigation: CPA structure is designed to withstand pressure from LAr flow based on fluid model predictions; Static deformation can be calibrated by lasers or cosmic rays.

[ID10] Ageing of insulator components in LAr could be an issue,  but  experience in Atlas (Kapton \& PCB, 20+ years), Icarus (G10, feedthrough 4+ years; feedthrough exchangeable) is trending favorably. Mitigation: more experience will be gained with liquid argon TPC's. Make feedthrough exchangeable.

[ID11] Current costings suggest that International funding could be a potential issue. Mitigation: cost reduction through design optimization and scaling. Effort to include more international collaboration institutes. 

[ID12]Add SWF contingency, full-scale installation trial at Ash River site prior to installation. Estimates based on ProtoDUNE experience. With the present knowledge: HV system is not on the critical path for installation.
