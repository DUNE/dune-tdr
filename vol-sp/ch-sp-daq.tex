...
By offline considerations, the \dword{daq} is limited to sending no more than \offsitepbpy to offline storage.
...
At this average rate and if saved to offline storage, the \dword{snb} triggers will lead to production of 1.8 PB/year uncompressed from one single-phase module.  
...
control the consumption of available network bandwidth to surface. Power, cooling, and space
constraints in the \dword{cuc} are limited to \cucpower total and \cucracks for all four \dword{fd}
modules.

There are three key challenges for the DUNE \dword{fd} \dword{daq} system: 
\begin{itemize}
\item First, the system must be able to evolve to accommodate newly commissioned sub-components as they are installed into a detector module which is under construction. 
  ...
  This partitioning will facilitate performing intervention in respond to faults that interrupt the collection of quality data from some portion of the detector.
  The faulty portion can be isolated to its own DAQ partition while the failure is understood and mitigated. 
  Meanwhile, high quality physics data may continue to be collected from the operational portion.
  Partitions  allow other special operations to be performed on a subset of the detector while not negatively impacting the remainder.  These include  detector or DAQ development and specialized running condition such as needed for calibrations.
...
The implementation of a continuous storage element in the upstream DAQ allows for the formation and capture of delayed, data-driven trigger decisions with no loss of physics information.
The specification for this look-back buffer is determined in consultation with physics groups.
The buffer size is driven primarily by the need to record up to ten seconds of unbiased data preceding a \dword{snb} (with the neutronization time taken as the time of the burst).
...
Collecting data containing information on other types of interactions and decays do not pose requirements on a pre-trigger buffer and the latency to form the required triggers is expected to be well within the \dword{snb} look-back buffer.

The \dword{daq} system is also designed so as to be able to apply lossless compression to these records, as well as filter them to remove unnecessary data regions in an intelligent way, i.e.~without compromising physics performance.
...
Generation of overlapping inputs to the trigger decision will be possible, and ordering and prioritization by the \dfirst{mlt} will prevent data readout duplication. 

\item Finally, the limitations in accessing the underground environment of the DUNE far detector requires that the \dword{daq} operates with high reliability and fully remote operation.

  To ensure minimal impact to overall detector live-time, the \dword{daq} system is fully configurable, controllable, and operable from remote locations, with authentication implemented to allow exclusive control. The \dword{daq} provides monitoring of the quality of the detector data and of its own operational status.
...
\fixme{We will drop this table and assure references are spread through text, as appropriate.}
...
\dwords{apa}. In the case of the \dword{pds}, 6-8 RU are each responsible for the
...
\begin{enumerate}
\item Data reception (Section~\ref{sec:fd-daq:upstream-receiver}), facilitated by two \dword{felix} cards;
...
The physical interface between the detector electronics and the DAQ are 10 Gbps point-to-point serial optical links running a simple (e.g., 8/10 bit encoded) protocol. 
...
In the baseline design, the 20 fibers from two \dwords{apa} are aggregate into one \dword{fec} with each \dword{apa} connected to one \dword{felix} FPGA PCIe Gen3 board. 
If technology advances sufficiently a PCIe Gen4 version of \dword{felix} may be produced to increase the data aggregation such that each board would accept 20 links and a total of 40 links per \dword{fec} would be accomodated.
...
The upstream \dword{daq} system also is tasked with forming \dwords{trigprimitive}.
They summarize time periods on a channel where the digitized waveform is no longer consistent with noise.
...
\fixme{Giovanna, please rewrite this next paragraph to make it clear what is baseline and what is option.}
...
A factor of two-four compression is
...
have taken the approach of validating and demonstrating a 
\dword{tpc}-based data selection.
An additional \dword{pds}-based data
selection component can be accommodated within the same data selection design and 
may augment data selection capabilities and
efficiencies.
...
architecture. A purely software implementation of data selection has been
...
\fixme{Giovanna, please clarify the above as to the baseline and option.}
...
% The entire data selection chain including the resulting query of data from the upstream \dword{daq} buffers must be accomplished promptly (latency expected to be on order one second and within the \dword{snb} pre-trigger buffer).
...
\fixme{I (bv) commented out discussion of white rabbit which doesn't make sense to me to be in this section.}
...
In the upstream \dword{daq} this includes the \dword{daqubi} that  provides access to the upstream \dword{daq} primary buffers for queries by \dword{eb} and other components. 
...
as well as a PCIe x4 socket which will host an \dword{nvme} SSD on an adapter,
