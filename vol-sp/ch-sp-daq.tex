...
distributing those decisions to the Egress sub-system. Data processing is executed in various stages of firmware and
...
The DS follows a hierarchical level structure, where low-level
...
structure is illustrated in Fig.~\ref{}. The hierarchy structure
...
A Trigger Primitive (TP) is defined nominally on a per-channel basis, and
in the case of the SP TPC, it is identified with a per-wire
``hit''. TPs are generated in the front-end readout part of the DAQ
system, either in FPGA firmware, or in CPU or GPU software. 

Algorithms for trigger primitive generation are under continuing development
\cite{}. An example algorithm is one which ....

This algorithm has been validated with both Monte Carlo
for the DUNE FD and real data from protoDUNE. The performance is
summarized in Sec.~\ref{xx}.
...
Ar39 rates. Assumes the above TP format prescriptiont, the anticipated
trigger primitive rate is expected to be xx . The subsequent stage of
the DS must be able to absorb and process this TP rate in real time.
