...
requirement that the \dword{dune} \dword{fd} maintain a $\lt$30~PB per yr to permanent
...
The \dword{dune} \dword{fd} \dword{daq} validation strategy is four-fold: 
\begin{itemize}
\item It exploits \dword{protodune} as a
demonstration and development platform. 
\item It uses \dword{fd} MonteCarlo simulation studies.
\item It uses Vertical Slice Teststands for development and testing of
  individual \dword{daq} subsystems as well as the overall \dword{daq}.
...
\subsubsection{FELIX Throughput Demonstration at \dword{protodune}}
...
The FELIX \dword{daq} at \dword{protodune} demonstrates the readout scheme for
...
operates similarly to \dword{dune}. What differs from the final \dword{dune}
...
host RAM to the Back-end system at \dword{protodune} is much higher
than that anticipated for \dword{dune}.

\subsubsection{RCE Throughput Demonstration at \dword{protodune}}
