...
The command is executed by a \dfirst{daqdfo} residing in the \dfirst{daqbes}
...
through an external trigger interface, which is part of the \dword{daqdsn}
system. The
...
\item Thirdly, the \dword{snb} physics requirements necessitate large buffering in the upstream \dword{daq}. 
  ...
  The implementation of a buffer element in the upstream DAQ allows
  ...
  of time while forming the trigger decision, thus identifying activity compatible with \dword{snb}.
  ...
  data for the trigger decision, preceding a \dword{snb} trigger.
...
{Summary of important parameters driving the DAQ design. The \dword{pds}
...
The time required to form a trigger decision based on localized
...
module-level trigger decision, which generates a \dword{trigcommand};
...
candidate, and trigger decision generation may be found
...
It should be noted that this data selection strategy is applicable to both \dword{pds} and \dword{tpc}, operating in parallel up to the module level trigger stage, where \dword{pds} and \dword{tpc} information can be combined to form a module level trigger decision. 
...
external trigger interface is to send \dword{snb} triggers
