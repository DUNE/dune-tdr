...
The nature and rates of these signatures necessitates a data selection
...
It should be noted that this data selection strategy is applicable to both \dword{pds} and \dword{tpc}, operating in parallel up to the module level trigger stage, where \dword{pds} and \dword{tpc} information can be combined to form a module level \dword{trigdecision}. 
...
\begin{dunefigure}[Data selection strategy and hierarchy]{fig:daq:data-selection-hierarchy}{Data selection
  ...
  \includegraphics[width=0.9\textwidth, trim=5cm 5cm 5cm 0cm]{DS_hierarchy.pdf}
\end{dunefigure}
...
The beam time information can also be sent to the \dwords{mlt} so that they may either alter \dword{trigdecision} criteria or merely include the information in contemporaneous \dwords{trigdecision}.
...
\item In atypical situations in which there are insufficient event builder resources available to handle the rate of triggers produced by the data selection sub-system, the DFO will alert the DS sub-system that the rate of triggers needs to be reduced.  When such reductions are requested, the DS sub-system will update the calculation of the module-level \dword{daq} live time appropriately.
...
Upstream \dword{daq}, \dword{daqdsn} and \dword{daqbes} \dword{daq} are interconnected by a \SI{10/100}{ Gbps} ethernet network for data exchange.
...
The ProtoDUNE \dword{daq} supported a test-beam experiment, and the requirements of the DUNE \dword{daq} are substantially different in scale and performance.
...
On the other hand, regularly the \dword{daq} will be validated via test stand integration with detectors, such as \dword{protodune} or pre-installation sites.
