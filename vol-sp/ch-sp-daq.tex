% Spelling:
%
% "subsystem" not "sub-system"
%
% "back-end" as adjective, "back end" as noun, etc for "front end".  "Back-end" or "Back end" as start of a sentence.
...
hierarchical \dfirst{daqdss} consumes minimally-processed
...
through an \dfirst{daqeti}, which is part of the \dword{daqdss}. The
...
\SI{10}{\second}) time period and over the entirety of a \nominalmodsize  module. Because of the high
...
needed to yield $>$90\% efficiency for a \dword{snb} occurring at a
...
triggers will lead to production of \SI{1.8}{\peta\byte/\year} uncompressed from one
...
Expected data rates from physics signals of interest, which fit the requirement that limits \offsitepbpy sent to permanent storage, are summarized in Table~\ref{tab:sp-daq:rates} and detailed in~\citedocdb{9240} and potential bottlenecks were analyzed in~\citedocdb{11461}.
...
time which is specified to \SI{100}{\second}, around every \dword{snb}
trigger. This time has been defined in consultation with \dword{dune}
physics groups.
...
conservatively for the first DUNE \dword{fd} module, and aggressively reducing
...
\item Finally, the \dword{daq} needs to reduce the volume of data to be permanently stored offline to a maximum of \offsitepbpy.
...
\dwords{apa}. In the case of the \dword{pds}, up to 8 \dword{daqrou} are each responsible for the
readout of raw data from a collection of \dword{pds} subdetectors. 
Each \dword{daqrou} is comprised of a \dword{fec} which is a commodity server that hosts a
...
links and a total of 40 links per \dword{fec} would be accommodated. Tests were performed to verify
...
The \dfirst{daqdss}  is a hierarchical, online, primarily
...
To facilitate partitioning, the \dword{daqdss}  will be
...
\item[High-energy interactions] High-energy interactions include cosmic muons, neutrino beam interactions, atmospheric neutrinos, and nucleon decays. 
  The trigger efficiency for these interactions is explicitly required to be $>$99\% for any given particle type (electron,muon, photon, etc.) that has a localized (confined to a few contiguous APAs) visible energy deposition above \SI{100}{\MeV}.
  To achieve this requirement, algorithms for creating high-energy trigger candidates are targeting a trigger efficiency of 50\% at \SI{10}{\MeV} visible energy, thus ensuring $>$99\% efficiency or better at \SI{100}{\MeV}.
...
\item[Low-energy interactions] The primary physics target for
  ...
  Low-energy trigger candidates (with threshold at or below \SI{10}{\MeV} visible energy) are generated and are input to an ``extended low-energy trigger'' that looks for bursts inconsistent with fluctuations in low-energy background events. 
...
been demonstrated through Monte Carlo simulations that the resulting
\dword{snb} trigger efficiency reaches $>$99\% for any \dwords{snb}
occuring within our galaxy, and efforts to extend this reach to the
Large Magellanic Cloud through refined algorithms are currently under
way \cite{bib:docdb11215,bib:docdb14522}. At the same time, the
...
each trigger primitive comprises the channel address (\SI{32}{\bit}), hit
start time (\SI{64}{\bit}), the time over
threshold (\SI{16}{\bit}), the integral ADC value (\SI{32}{\bit}),
an error flag (\SI{16}{\bit}), and possibly also
the waveform peak (\SI{12}{\bit}) associated with the hit. 
As such, \SIrange{20}{22}{\byte} provides a generous data
...
calibration systems. As an interface between \dwords{mlt}, it is responsible for receiving and distributing information about module-level \dword{snb} \dwords{trigcommand}.
...
components (to White Rabbit and \dword{dune} timing system) for timing and external trigger signal I/O.
...
%The 62.5 MHz was pushed by the DAQ group.  The White Rabbit system (used 
%by DP) is based on a 125 MHz clock... thus 62.5 MHz rather than 64 MHz.  
%This difference is inconsequential for the CE.
...
inputs to the \dword{dune} \dword{fd} design. For example,
...
% Continuous self-triggering of the detector is also new with respect to
...
% via use of dynamic and fixed-baseline zero-suppression implemented in
...
only counting information, the efficiency is 6.5\%. The \dword{daq}
consortium is working on further refining these algorithms to improve
\dword{snb} trigger efficiency, in accordance with \dword{dune} physics
requirements. Regardless, the design provides flexibility
