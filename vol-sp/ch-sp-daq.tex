...
The functionality of the \dfirst{daqbes} comprises the \dfirst{daqdfo}, \dfirst{eb} and storage buffer blocks shown in Figure~\ref{fig:fd-daq:layout}. 
...
Second, it provides local storage sufficient for uninterrupted \dword{daq} operation in the  event that the network connection between the \dword{fd} and \fnal is temporarily lost. 
A capacity of at least \SI{0.5}{\peta\byte} is envisioned,
sufficient to buffer the nominal output of the entire \dword{fd} for about one week.
Based on prior experience
of the consortium with losses of 
connectivity at other far detector experiment sites, this is a
conservative capacity value.
...
This section begins with an overview of the organization of the DAQ consortium, that currently consists of 34 international institutions. Table~\ref{tab:daq-ib} provides the list of the participating institutions. Subsequently we present the schedule for design and R\&D, production, integration and installation of the DAQ within the detector. Finally, we discuss the risks associated with the project.
