...
An overview of the \dword{dune} \dword{fd} \dword{daq} system design is
provided in Fig.~\ref{fig:fd-daq:designoverview}. The system is
physically located at the \dword{fd} site, and occupies space and
...
\begin{dunefigure}{fig:daq-designoverview}{\dword{daq} Design Overview}
...
sensing time scales.
Ionization charge measurement by the \dword{tpc} requires a
...
\dword{lartpc} experiments including ICARUS, \microboone, and
\dword{protodune}, and other long-baseline neutrino experiments including T2K, \nova,
and MINOS. 
...
\dword{protodune}, has motivated the adopted solution described later in this chapter. 
...
built into the design of each \dword{fd} \dword{module}. %\fixme{This sentence simply repeats the table title. A little more discussion of the trends revealed in the table would help here, but obviously, that must wait for the values.}
