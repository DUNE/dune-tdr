...
\dword{daq} services each \dword{fd}
...
4850 ft level and the surface level at \dword{surf}. Specifically, the \dword{daq} occupies space and
power both in the \dword{cuc} and the on-surface \dword{daq}
...
underground in the \dword{cuc}, while the \dword{daqbes}, %back-end part of the system,
 which is responsible for
event-building, as well as run control and monitoring, lives on the
surface. Data flows through the \dword{daq} from 
...
upstream \dword{daq}, thus minimizing data bandwidth to the surface. A
...
flow orchestrator process is activated as part of the \dword{daqbes}
...
\begin{dunefigure}[DAQ physical layout for one module]{fig:fd-daq:layout}{\dword{daq} design physical
    layout focusing on a single \nominalmodsize
    module. Highlighted in orange, blue, yellow, gray, and brown are
    the upstream \dword{daq}, \dword{daqdsn}, \dword{daqbes} (\dword{daqdfo}, \dword{eb} and buffer), timing and
...
active detector components from which the \dword{daq} system must acquire
...
latter (\SI{2.25}{\milli\second}) assumes DUNE's nominal drift electric field of 500~V/cm.
...
By offline considerations, the \dword{daq} is limited to sending no more than
...
trigger prompts \SI{5.4}{\milli\second} of losslessly compressed \dword{tpc}
data plus \SI{5.4}{\milli\second} of lossy compressed \dword{pds} data from
...
event record. The one  extended trigger per month would amount to a
...
constraints in the \dword{cuc} are limited to 600 kW total and 52 racks for all four \dword{fd}
modules.

There are three key challenges for the DUNE \dword{fd} \dword{daq} system: 
\begin{itemize}
...
location requires that the \dword{daq} operates with high reliability and fully remote operation.
...
authentication implemented to allow exclusive control. The \dword{daq}  
 facilitates online monitoring of the detector and of
...
TPC CE & Data format, rate, number and type of links, timing, inherent noise & \citedocdb{6742}{v6}\\ \colhline
PDS Readout & Data format, rate, number and type of links, timing &  \citedocdb{6727}{v2} \\ \colhline
...
systems themselves, upon receipt of time stamps from the \dword{daq} system;  %, and those 
...
Accuracy of timestamps delivered to  detector endpoints will be $\pm$\SI{500}{\nano\second} with respect to UTC.  Synchronization between any two endpoints in the detector will be better than \SI{10}{\nano\second} on average.   Between detector modules, synchronization will be better than \SI{25}{\nano\second} on average.  The timing system will also provide a synchronized clock source by which \dword{daq} computer system clocks may be synchronized using standard network time protocols.  System clocks are expected to be synchronized to within a ms using NTP and \si{\micro\second} using PTP standards. 
\end{description}
...
\begin{dunetable}[Technical notes on DUNE DAQ subsystem design]
{p{0.7\textwidth}p{0.2\textwidth}}{tab:sp-daq:tech-notes}{Summary of %relevant and 
detailed \dword{daq} technical notes on DUNE \dword{daq} subsystem design.}
...
ML-based Data Selecion for DUNE FD & \citedocdb{11311} \\\colhline
...
The  \dword{daq} system is composed of six distinct subsystems: (1) upstream
\dword{daq} (Section~\ref{sec:daq:design-upstream}), (2) \dword{daqdsn}
(Section~\ref{sec:sp-daq:design-data-selection}), (3) \dword{daqbes} (Section~\ref{sec:fd-daq:design-backend}), (4) \dword{daqccm} (Section~\ref{sec:fd-daq:design-run-control}), (5) \dword{daqtss} (Section~\ref{sec:sp-daq:design-timing}), and (6) infrastructure software. The physical extent
...
detail in the following subsections. To first order the \dword{daq} system
is implemented as an online data processing and data flow system. As
such, \dword{ipc} is an
additional, central component to the FD \dword{daq}, which spans across multiple
subsystems, as do other Infrastructure Software components, e.g.~databases. Due
to its central role, \dword{ipc}  is described below in a separate subsection (Section~\ref{sec:daq:design-ipc}).
...
facilitates event building in approximately ten servers and buffering for built
...
filter/data reduction stage, which is part of overall data selection,
 ...
 the  \dfirst{daqccm} provides the central orchestration
 (Section~\ref{sec:fd-daq:design-run-control}), \dword{ipc} provides
...
\begin{dunefigure}[DAQ design implementation for a single module]{fig:daq-design}{\dword{daq} Design
...
\dword{apa}s. In the case of the \dword{pds}, 6-8 RU are each responsible for the
...
\begin{enumerate}
\item Data reception (Section~\ref{sec:fd-daq:upstream-receiver}), facilitated by a \dword{felix} card in the host server;
...
\item Data processing (Section~\ref{sec:fd-daq:upstream-proc}), facilitated by \dword{fpga} resources on the \dword{felix}
  ...
  boards (interfacing directly with the \dword{felix} card); and 
...
other \dword{daq} subsystems, the upstream \dword{daq} makes use of the common software framework for control, configuration, and monitoring as described in Section~\ref{sec:fd-daq:design-run-control}.
...
data to other blocks of the upstream \dword{daq} subsystem.

\subsubsection{Network based I/O}
\label{sec:fd-daq:upstream-io}

The upstream \dword{daq} subsystem provides access to the \dword{daqdsn} and \dword{daqbes} through a commercial, off-the-shelf switched network as illustrated in Figure~\ref{fig:daq:readout}).
...
The data processing functional block resides either on the \dword{felix} board \dword{fpga}, or on
an additional, dedicated co-processor (see Section~\ref{sec:sp-daq:design-validation}) providing additional \dword{fpga}
...
detector (in the \dword{tpc} or \dword{pds}) as a function of time.
...
data into different streams (collection plane vs. induction plane,
...
The upstream \dword{daq} system also facilitates the identification and
...
stages of the
\dword{daqdsn} that make correlations and ultimately decide whether and which data is to be saved.

This functional block in principle may be implemented via \dword{fpga}, GPU, CPU, or a
...
within upstream \dword{daq} and \dword{daqdsn} at \dword{protodune}. The \dword{daq} design includes both \dword{fpga} and
CPU elements in the upstream \dword{daq}, which provides the needed
flexibility and 
...
time, and requires extracting as much as 115 TB from the \dword{tpc} upstream \dword{daq}.
It is not cost effective to design the \dword{daq} to extract such
extended data record (100-second-long waveform from each \dword{tpc} channel
and corresponding-in-time \dword{pds} information) in an online fashion. Thus, additional buffering is provided to catch
...
expected, based on MicroBooNE and \dword{protodune} experience. 
...
a wire/channel'' activity summary. \dwords{trigprimitive} are aggregated
into \dwords{trigcandidate}, which represent information associated with
higher-level constructs derived from trigger primitives,
for example
``clusters of hits''. \dword{trigcandidate} information is subsequently used to inform a module-level trigger decision, which
generates a \dword{trigcommand}; this takes the form of either a localized high energy
...
level filter. The data selection strategy is applicable to both \dword{pds}
and \dword{tpc} indistinguishably, up to the module level trigger stage, where
\dword{pds} and \dword{tpc} information can be combined to form a module level trigger
decision. Data selection design efforts
have taken the approach of validating and demonstrating a solely
\dword{tpc}-based data selection, with the expectation that \dword{pds}-based data
...
\item    Localized activity associated with low deposited energy (as
  low as below 10 MeV) is input to a different type of trigger,
...
tests at \dword{protodune}. An \dword{fpga} implementation of the lowest level of data selection
(specifically, trigger primitive generation), 
however, is possible within the upstream \dword{daq} design, and offers an
...
ongoing development at \dword{protodune}.
...
\cite{bib:docdb11275}. Nominally, trigger primitive generation proceeds
...
The module level trigger sends its produced \dwords{trigcommand} to the
...
the upstream \dword{daq} buffers must be 
...
latency is governed by the upstream \dword{daq} 
...
% The timing system for the SP is custom. But does use White
%Rabbit as an inter-module interface.  There is a whole note on this on the
%Wiki---basically the system looks like the ProtoDUNE version (a master timing
%card plus a fanout, with the clock and PPS coming from GPS). So WR in the ETL
%is probably reasonable, because that talks to everyone.
...
\dwords{trigcommand} generated by the module level trigger are also sent,
...
It accepts \dwords{trigcommand} produced by the \dfirst{daqdss} as described in Section~\ref{sec:sp-daq:design-selection-algs}. 
It queries the upstream \dword{daq} buffers and accepts returned data as described in Section~\ref{sec:daq:design-upstream}. 
Finally, it records \dwords{trigcommand} and the corresponding data in the
...
\fixme{Remove egress reference from figure. Replace with Back-end
  \dword{daq}. Do we need to show event builder idle pool? Brett}
...
the appropriate \dfirst{daqfbi}, as addressed by a received \dword{trigcommand}. 
...
As part of the \dword{daqccm}, this include the issuing of control and reconfiguration commands to and receiving of monitoring messages from essentially all \dword{daq} components. 
As part of \dword{daqdsn}, this includes the transfer of \dword{trigprimitive}, candidate and command messages. 
In the upstream \dword{daq} this includes the \dword{daqfbi} that  provides access to the upstream \dword{daq} primary buffers for queries by \dword{eb} and other components. 
...
For example, if the Buffer Interface Service is implemented as an \dword{artdaq} Board Reader it would necessarily use \dword{artdaq} \dword{ipc} . 
...
demonstration of \dword{fpga}-based data reduction for \dword{tpc}.
...
processing from the \dword{tpc} or \dword{pd} systems for self-triggering. 
Second, the \dword{protodune} 
system sits at the surface with a much higher
data occupancy 
due to cosmic ray activity.
Overcoming the first key difference to demonstrate \dword{daqdsn} capability for the \dword{fd} \dword{daq} design is a main component of future \dword{daq} development plans, described in Section~\ref{sec:sp-daq:design-validation}. \fixme{right sec ref? Anne}
...
\begin{itemize}
\item Upstream \dword{daq}: successful
front-end readout hardware and data flow functionality for the readout
of two out of the six \dword{apa}s employed in \dword{protodune}. This was achieved
with two \dword{tpc} RUs, without co-processor boards, and only one \dword{apa} read
out per \dword{felix} board. The \dword{dune} \dword{daq} design will ultimately
accommodate readout of two \dword{apa}s per \dword{felix} board. In addition to data
flow functionality, \dword{protodune} \dword{fe} readout also demonstrates interface to
...
\item \dword{daqbes}, \dword{daqccm} and software infrastructure:
 successful \dword{daqbes} implementation, including event builder
  ...
  areas of \dword{daqccm} and \dword{ipc}  and for the data flow orchestrator part of the
  \dword{daqbes}.
...
interfaces, i.e.~understanding the exact \dword{daq} scope and the interfaces to \dword{tpc}, \dword{pds}, and offline. The use of commercial off-the-shelf solutions
...
greater detail in \cite{Hennessy:CDRReview}. Figure~\ref{fig:sp-daq:protodunedaqpic} shows the 
\dword{protodune} \dword{daq} hardware as used during beam data taking.

\begin{dunefigure}[ProtoDUNE DAQ hardware during beam data taking]{fig:daq-rates}{\dword{protodune} \dword{daq} hardware as used during beam data taking. \label{fig:sp-daq:protodunedaqpic}}
...
and \dword{ipc} , which is common across multiple \dword{daq} subsystems.
...
In \dword{protodune}, a single \dword{apa},  \dword{felix}-based readout consists of two servers,
with a point-to-point 100 Gbps network connection.
The \dword{felix} I/O card
...
continuous DMA transfer. The \dword{felix} host PC runs a software process
that publishes the data to any client subscribing to it, based on
link identifiers. The clients of this application are the \dword{felix}
BoardReader processes, which are part of the \dword{artdaq} framework. In
...
publisher) were carried out specifically for \dword{protodune}: each \dword{felix}
host receives and publishes data at ~75 Gbps. The BoardReader hosts
are equipped with embedded Intel QuickAssist (QAT)~\footnote{Intel\textregistered QuickAssist Technology, \url{https://www.intel.com/content/www/us/en/architecture-and-technology/intel-quick-assist-technology-overview.html}.} technology for
hardware accelerated data compression. The \dword{protodune} implementation of
the \dword{felix} \dword{fe} readout is
...
readout by supporting two \dword{apa}s on a single server. 
...
with a ``\dword{felix} demonstrator'' (so called because it demonstrates use
of Xilinx Virtex-7 Ultrascale+ \dword{fpga} and Gen-4 PCIe) which has a FMC+
connector which will be connected to a PBM. \fixme{pbm?} Passive optical splitters
...
\begin{dunefigure}[CPU core-time for primitives and CPU utilization in live data]{fig:daq-cpu-hf-speed}{CPU core-time (left) required
...
\begin{dunefigure}[Trigger primitives, one APA  in ProtoDUNE-SP data]{fig:daq-hitfinder}{Illustration of the trigger primitives found by the CPU algorithm in one nominal readout of one \dword{apa}  in \dword{pdsp} data.  Although the data was ultimately recorded due to trigger, the algorithm ran in full-stream mode.}
...
electronics noise levels and radiological rates~\cite{bib:docdb11236}. \fixme{fix}
...
record throughput (from the \dword{eb}) through such filtering
...
% The algorithm is described in detail in \cite{docid-11236}; a Monte Carlo test has demonstrated the algorithm can do real-time processing of one \dword{apa}s worth
...
% increases of 50\% above the default \dword{dune} Monte Carlo settings \cite{bib:docdb11275}. 
...
ZeroMQ \dword{ipc}  has been successfully exercised at \dword{protodune} as part of a practical solution to transfer the full stream of trigger primitives out of one process and in to another. 
...
Figure~\ref{fig:daq-org}. The \dword{daq} consortium board currently comprises
representatives from 32 institutions as shown in Table~\ref{tab:daq-ib}. The consortium leader is the spokesperson for the consortium and
...
and commissioning 
of the DUNE FD \dword{daq} system. To realize this, the consortium was
initially organized in the form of five working groups: (1)
Architecture, (2) Hardware, (3) Data Selection, (4) Back-end \dword{daq}, and (5)
Installation and Infrastructure. This organization has seen the
project through the completion of the conceptual design phase.  

A new organizational
structure has been adopted to see the project through engineering
design and construction, and this structure is expected to evolve
further in the future in
...
overall editing and delivery of this TDR document.
...
\fixme{need text?} 
The high-level DAQ milstones, as well as key DUNE project milestones,
is provided in Tab.~\ref{tab:daq-sched} and illustrated in Fig.~\ref{tab:fd-daq:schedule}.
...
general safety plans referenced above. The long-term emphasis is on remote operations capability
...
$>25$\%, $10-20$\%, and $<10$\%, respectively; high (H), medium (M), and low (L)
...
\item[Detector noise specs not met] Excessive noise will make it
  ...
  studying noise conditions at ProtoDUNE; leaving provisions in the
  system for additional front-end filtering (in the form of the
  co-processor and/or CPU) and/or post-event builder processing (in
  the form of the high level filter). Mitigation includes augmenting
  filtering resources using larger FPGAs or computing system.
...
subsequent DUNE far detector modules to the
surface. Mitigation includes expending additional resources on an
...
\item[Excess fake trigger rate from instrumental effects] Instrumental
  ...
  data volume, increasing processing resources in high level
  filter, and augmenting filtering using larger FPGAs or CPU in upstream
  DAQ.
...
selections tage to carry out online analysis and data
...
\item[Cost/performance of hardware/computing excessive] Costs of
  system-as-constructed may exceed available budget. Prior mitigation
...
\item[Insufficient throughput in computing system] The computing system
  ...
  DAQ. Prior mitigation: System design should allow for expansion of resources as required (i.e. bottlenecks avoided by design). Mitigation: Expand system capacity before commissioning. 

\item[Event builder throughput insufficient] The event builder system
  as-constructed may not scale to 
meet throughput requirements of DAQ.  Prior mitigation: System design should allow for expansion of resources (i.e. by avoiding single point bottlenecks). Mitigation: Expand system capacity before commissioning.
...
\item[Additional data reduction steps required after event buiding] An
 ...
 what can be accommodated in high level filter.Prior mitigation: Allow provision for scalable high level filter on surface. Mitigation: Add expanded high level filter resources.
\item[PDTS fails to scale for DUNE requirements] The PDTS concept may
  not scale to DUNE in scale 
or performance.  Prior mitigation: Test PDTS at realistic scale before final design. Mitigation: Replace PDTS with upgraded hardware.
\item[Full remote operation of DAQ proves non-viable] The DAQ consoritum
 ...
 the system must be reliable enough to support this. Prior mitigation: Carry out remote operations tests during prototyping and pre-production test phases. Design human interface tools for remote operations. Mitigation: Allocate surface or underground control room space and facilities for DAQ operations at SURF.

\item[DAQ system does not meet overall DUNE uptime specification] All
  DAQ components must be intrinsically reliable or 
redundant to meet overall uptime specification.  "Prior mitigation: Carry out extensive QA on all nonredundant components. Exercise failure modes and recovery in realistic
conditions in an extended ProtoDUNE run. Mitigation: Replace non-compliant components with alternatives during future DUNE program."

\item[External services do not allow reliable operation of DAQ
  system] Power/cooling/network services may not be sufficient
reliable to meet overall uptime specification. Prior mitigation: Specifiy an acceptable level of performance for external services before construction. Mitigation: None possible after construction.