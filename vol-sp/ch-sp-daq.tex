...
%\metainfo{Somewhere in this chapter it will be specified that the DAQ
 % consortium brings together resources and expertise from UK, CERN,
 % ..., US, including key expertise on LArTPC readout and DAQ.}
...
%   same boat).}
The \dword{fd} \dword{daq} system basic functionality is to provide
timing and synchronization for all \dwords{detmodule} and
subdetectors; receive, synchronize, compress, and buffer streaming
data from the subdetectors; extract information from the data at a
local level to subsequently make local, module, and cross-module data
selection decisions; build ``events'' from selected space-time volumes,
and relay those to permanent storage, and carry out local data
reduction and filtering of the data as needed.

This chapter provides a description for the design of the \dword{dune}
\dword{fd} \dword{daq} system. 
The system is designed so that it services all \dword{fd}
\dwords{detmodule}, and therefore most aspects of the design are
identical across the different \dwords{detmodule}. 
...
A small part of the \dword{daq} design must be tailored to meet module-specific requirements. 
Those portions of the design are documented in sections of this chapter which are unique to the \dword{detmodule} covered in this TDR volume. 
Those sections are identified by their use of module-specific terms.

The chapter begins with requirements for and interfaces between \dword{daq} and other \dword{dune} systems. 
...
The DAQ  consortium brings together resources and expertise from the UK, CERN, USA, France, the Netherlands, Japan, and Colombia.  Its members bring considerable experience from ICARUS, MicroBooNE, and DUNE prototype LArTPCs, as well as from ATLAS at the LHC and other major HEP experiments across the world.
...
The \dword{dune} \dword{fd} \dword{daq} system must meet the
\dword{dune} top-level as well as \dword{daq}-level
requirements. The top-level requirements which explicitly pertain to
the \dword{daq} are provided below. The \dword{daq}-level requirements are 
...
%\section{SP-FD Specifications for DAQ}
...
\input{generated/req-SP-FD-det-dead-time.tex} % 16
...
\input{generated/req-longtable-SP-DAQ.tex}

DUNE has three main physics drivers: neutrino CP violation and related
long baseline oscillation studies using the high intensity beam
provided by Fermilab, off-beam measurements of atmospheric
neutrinos and searches for rare processes such as baryon number
violation, and detection of supernova neutrino bursts occurring within
our galaxy.  The DUNE SP LArTPC implements two detection technologies
to access this physics, each with very different readout times.
Charge measurement of an ionizing event requires a nominal recording
of data over a time window of multiple milliseconds, 
to collect 
information from the full detector volume as determined by the
ionization electron drift speed in argon.  The photon detection system (PDS)
measures argon scintillation light emission, which occurs and is
detected over a timescale of a multiple nanoseconds for any given event and/or
subsequent subevent process. 
 
The long baseline physics program makes use of precise beam timing
information from Fermilab that can be used by the DAQ.  Experience
from earlier LArTPC experiments like ICARUS and MicroBooNE, ProtoDUNE,
and other long baseline experiments like T2K, Nova, and MINOS, lends
high confidence to a DAQ solution that enables this physics.  Nucleon
decay in DUNE can occur at any time and lacks an external trigger
input; however, the high energies of final state particles provide
robust PDS and TPC signatures that likewise lead to relatively
straightforward delayed, data-driven trigger decisions informed by
only a few TPC readout time windows.  The main DAQ challenge which is
unique for DUNE as opposed to past LArTPC experiments is to
self-trigger on supernova bursts, which occur over a \SI{10}{s}
timescale; such data-driven decision must prompt unbiased data from
the charge and light colleciton systems,  created by the
tens-to-hundreds of supernova  electron neutrino interactions in the
few to tens of MeV range, over a \SI{100}{s} time interval.  An
extensive R\&D program informed by several years of MicroBooNE data
and DAQ developments,
as well as ongoing development efforts on ProtoDUNE, has led to the
adopted solution described later in this chapter. 

Within the adopted solution, the \dword{daq}-level requirements are
...
met, several key specifications must be defined for the \dword{daq}
...
to be recorded by the \dword{daq}. This time span is to be configurable at run start and
...
is to be recorded. The \dword{daq} system should be capable of
...
implementation of and scalability of the \dword{daq} system. 
 
With respect to operation considerations, the need to minimize
supernova burst deadtime necessitates a partitionable \dword{daq} system,
...
furthermore facilitate online monitoring of the detector and the \dword{daq}
system in itself.
...
Consequently, the \dword{dune} \dword{fd} \dword{daq} system interfaces with the TPC Cold Electronics, PDS
...
The \dword{daq} and TPC Cold Electronics interface is described in
\citedocdb{6742}. The physical interface is at the Central Utility
...
The \dword{daq} and PDS Readout interface is described in
\citedocdb{6727}. This remains to be refined, but it is anticipated to
...
\metainfo{Buffer disk.  Agreement on sysadmin support and computer
  procurement, ssh gateways, non data networks.  Address reference how
  the data model described above is acceptable.} The Online Computing
...
The \dword{daq} and CISC interface is described in
\citedocdb{6790}. The DAQ provides a network in the CUC for CISC; operation information and hardware
...
\dword{dune} \dword{fd}: a laser calibration system and a neutron
...
\dword{protodune} which facilitated two incarnations of DAQ
...
\dword{protodune} beam running began in Q4 of 2018. The detector is
...
The \dword{protodune} DAQ system is depicted in
...
CPU. The baseline system for DUNE FD \dword{detmodule} instead
...
Besides overall readout architecture, there are two additional key differences between \dword{protodune} and
\dword{dune} DAQ, namely that (1) the \dword{protodune} DAQ is externally
triggered (and at a trigger rate which is more than an order of
magnitude higher than that anticipated for \dword{dune}),
and therefore the current \dword{protodune} DAQ does not facilitate online data
...
successful operation of \dword{protodune} DAQ has provided several key
demonstrations for \dword{dune} DAQ, in particular data flow
...
to use a single clock frequency for all \dword{dune} subdetectors has been
formalized in a central timing interface document for the \dword{dune}
\dword{fd} \citedocdb{}. Hardware integration tests prior to
installation and commissioning were found insufficient, and were
primarily a consequence of limited
availability of hardware and experts; for \dword{dune} those are
...
the consortium. Finally, in the case of the \dword{protodune} trigger, despite its simplicity
relative to the \dword{dune}'s data-driven data selection scheme, it
was found that the system required more commissioning time. This has
been built into the \dword{dune} project schedule. More specific
outcomes, per \dword{protodune} subsystem, are outlined below.
...
Control for the \dword{dune} \dword{fd}. This should happen early in
...
%In summary, \dword{protodune} has been a successful platform for DAQ
...
\begin{dunetable}{|p{0.7\textwidth}|p{0.2\textwidth}|}{tab:sp-daq:tech-notes}{Summary of relevant and detailed \dword{daq} technical notes.}
  ...
  What are the DUNE FD DAQ Bottlenecks? & \citedocdb{11461} \\
\end{dunetable}
...
Figure~\ref{fig:daq-conceptual-overview} provides an illustration of the high-level conceptual elements that make up \dword{dune} \dword{fd} \dword{daq}. 
It applies to all \dword{fd} modules. 
The box in the illustration indicates the overall conceptual scope of the \dword{daq}.
...
\item A substantial portion of this data then flows into the \dfirst{daqdss} which is the majority portion of the \dfirst{daqtrs} resulting in a stream of \dwords{trigcommand} (see Section~\ref{sec:sp-daq:design-selection-algs}).
...
\item Pervasively, the central orchestration is provided by \dfirst{daqccm} (Section~\ref{sec:fd-daq:design-run-control}), overall communication by the \dfirst{ipc} sub-system (Section~\ref{sec:daq:design-ipc}) and synchronization is provided by the \dfirst{daqtss} (Section~\ref{sec:sp-daq:design-timing}).
\end{itemize}


In addition to deconstructing the design into sub-systems, the DAQ is also separable into multiple \dwords{daqpart} or instances. 
...
An example of a domain-specific IPC is that used by artDAQ that will be the heart of the \dword{daqbes} as described in Section~\ref{sec:fd-daq:design-backend}.
...
The \dfirst{daqccm} subsystem (Section~\ref{sec:daq:design:ccm:control}) must communicate with all DAQ domains and in some cases directly to domain components and so is also expected to utilize the \textit{lingua franca} IPC. 
...
The \dfirst{daqccm} subsystem, illustrated in Figure~\ref{fig:daq-ccm-subsys}, encompasses, as it's name says, the software needed to control, configure, and monitor the rest of the \dword{daq} (as well as itself).
...
In addition to this \dword{daq} \dword{daqccm} monitoring sub-system, a separate system shall be used to monitor the quality of the detector data content itself.  See Section~\ref{sec:fd-daq:design-data-quality} for the description of this data quality monitoring system.
...
The readout system provides access to the data selection and \dword{daqbes} through a commercial, off-the-shelf switched network as illustrated in Figure~\ref{fig:daq:readout}).
...
In addition, a small portion of the data must be buffered for much longer to absorb the strongly punctuated bottlenecks that form when as potential \dword{snb} has been observed by the \dword{daqdss}.
...
The extended activity (ie, that which may be due to a potential \dword{snb}) presents a far more challenging set of buffering requirements.  
...
and provides addresses (in space and time) of the data in the \dword{fe} buffers which capture information about the activity.
...
It must also select data associated with potential galactic \dfirst{snb}, indicating that the data selection sub-system must be able to self-trigger on galactic \dword{snb} with galactic coverage\footnote{Galactic coverage is defined as efficiency-weighted probability of galactic \dword{snb}.} of $>$95\% efficiency.
...
The \dword{daqdss} sub-system design follows a hierarchical structure, where low-level
...
timing signals, etc.) within a single \dword{detmodule}.
...
\dword{daqdss} from the CCM system.

Ultimately, each trigger decision culminates in a command sent to \dword{daqbes}. 
...
to the readout of the entire detector module for a period equal to 5.4ms.  Extended activity triggers (\dword{snb}) will
...
The \dword{hlt} therefore serves primarily to down-select and thus
...
\item Requests and their replies may be sent as synchronously and replies are expected even if data has already been purged from the \dword{fe} buffer.
...
Section~\ref{sec:daq:design:ccm:monitoring} described a monitoring system for the \dword{daqccm} sub-systems. 
...
All components of the \dword{fd} use clocks derived from a single \dfirst{gps} disciplined source, and all module components are synchronized to a 62.5 MHz clock. 
...
\end{itemize}
The timing system associates the \dword{gps} time to its master clock by using the latter to stamp the arrival time of the \dword{pps} of the former.
To provide an absolute time to the nearest second the information from \dword{ntp} is used.
...
\begin{itemize}
\item A model of trigger primitive rates and will be developed which is parameterized by signal, background and noise.  This model will be informed by \dword{protodune} and simulation studies.  The model will include the data density per unit of front-end computing as informed by the trigger primitive validation work described above.
