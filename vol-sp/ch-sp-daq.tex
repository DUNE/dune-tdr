...
\dword{daqdsn} and \dword{daqbes} through a commodity switched network as
...
These regions of interest are then used as input to the \dword{daqdsn} sub-system where they begin the process of forming a trigger decision.
...
The \dword{daqccm} software suite will run on approximately 15 servers interconnected by a \SI{1}{Gbps} ethernet network to upstream \dword{daq}, \dword{daqdsn}, \dword{daqbes} as well as detector and calibration daq interface elements. While this network has a lower throughput compared to the data network, it has many more endpoints O(2000).
