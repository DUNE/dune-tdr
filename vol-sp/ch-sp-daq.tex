...
architectures, one based on \dword{felix}, develped by ATLAS \cite{daq-atlas-felix}, and the other on \dword{rce}, developed at
SLAC \cite{daq-slac-rce}. \dword{daq} design and construction for
...
exclusively in
custom-designed \dword{fpga} firmware and carrier board, and the other  in which the data is processed primarily with custom software run in commodity
servers. The baseline system for a %DUNE FD 
\dword{detmodule} instead
merges elements of the two approaches. \hlfix{Specifically}{confusing sentence}, it uses  \dword{felix}  as
...
resources.
In that sense,\dword{protodune} has provided demonstration of the
 \dword{felix}  platform as \dword{fe} readout data receiver, and
of \dword{fpga}-based \hlfix{data reduction}{how is this demonstrated?} for \dword{tpc}.
...
% Figure~\ref{fig:sp-daq:protodunedaqpic} shows the 
...
interfaces with its host PC through 16-lane PCIe 3.0 (theoretical bandwidth of \SI{16}{GB/s}).
It transfers
...
that publishes the data to any client subscribing to it.
\hlfix{Subscriptions may identify from which input optical links the received data stream will be sourced.}{is it correct}
The clients consuming these streams are
``BoardReader'' processes implemented in the \dword{artdaq} data acquisition toolkit.
In
order to sustain the data rate, modest modifications of the firmware
(increased \hlfix{block size}{what block size} and chunk merging) and software (scatter-gather
publisher) were carried out specifically for \dword{protodune}: each \dword{felix}
host receives and publishes data at $\sim$75 Gbps. The BoardReader hosts
...
hardware accelerated data compression. The \dword{protodune} application of
...
a form that allows hardware SIMD acceleration (AVX2).
...
more than two orders of magnitude. \hlfix{Inference latency}{Georgia: please clarify} is under study to validate limiting event
...
is provided in Tab.~\ref{tab:daq-sched} and illustrated in Fig.~\ref{fig:fd-daq:schedule}.
