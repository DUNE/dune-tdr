...
redundant to meet overall uptime specification.  Prior mitigation
includes carrying out extensive QA on all nonredundant components, and
exercising failure modes and recovery in realistic
conditions in an extended ProtoDUNE run. Mitigation includes replacing
non-compliant components with alternatives during future DUNE
program. 

\item[External services do not allow reliable operation of DAQ
  system] Power/cooling/network services may not be sufficient
reliable to meet overall uptime specification. Prior mitigation
includes specifying an acceptable level of performance for external
services before construction. In this case, no mitigation is possible after construction.