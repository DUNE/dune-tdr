...
4850 ft level and the surface level at SURF. Specifically, the \dword{daq} occupies space and
...
underground in the \dword{cuc}, while the back-end part of the system, which is responsible for
...
flow orchestrator process is activated as part of the back-end \dword{daq}
...
\begin{dunefigure}[DAQ physical layout for one module]{fig:fd-daq:layout}{\dword{daq} design physical
    ...
    the Upstream DAQ, Data Selection, Back-end \dword{daq} (Data Flow
...
The \dword{daq} system is designed as a single, scalable system which can service
...
\dword{daq} (Section~\ref{sec:daq:design-upstream}), (2) Data Selection
(Section~\ref{sec:sp-daq:design-data-selection}), (3) Back-end \dword{daq} (Section~\ref{sec:fd-daq:design-backend}), (4) Control,
...
Upstream \dword{daq} and Timing and Synchronization live underground in the \dword{cuc}; Data Selection occupies
both underground and above-ground spaces; Back-end \dword{daq} is above-ground
...
co-processing \dword{fpga}/CPU hardware, all of which is hosted in O(100) servers in the \dword{cuc}. A
...
external trigger inputs in issuing a ``trigger command'' to the back-end \dword{daq}
subsystem. The back-end \dword{daq} subsystem 
facilitates event building in O(10) servers and buffering for built
events on non-volatile storage; upon receiving a trigger command, the back-end \dword{daq} queries
...
namely the \dword{daqdsn} and the back-end \dword{daq}.
...
\dword{apa}s. In the case of the PDS, 6-8 RU are each responsible for the
...
within upstream \dword{daq} and Data Selection at protoDUNE. The \dword{daq} design includes both \dword{fpga} and
...
time, and requires extracting as much as 115 TB from the TPC upstream \dword{daq}.
...
\dword{daq} as described in Section~\ref{sec:fd-daq:design-backend}. 
...
tests at ProtoDUNE. An \dword{fpga} implementation of the lowest level of data selection
...
nominally in \dword{fpga} firmware (TPC) and CPU (PDS), as described in
...
\dword{daq} buffers, the back-end \dword{daq} will support parallel readout of event
...
\begin{dunefigure}[DUNE DAQ back-end operation]{fig:daq:backend}{Illustration of \dword{dune} \dword{daq} back-end operation.}
...
The \dword{daq} back-end subsystem will provide the instances of the \dfirst{eb} most likely as \dword{artdaq}~\cite{artdaq} components of the same name.
...
Referring to Figure~\ref{fig:fd-daq:layout}, IPC is used for both in-band detector data flow between Upstream \dword{daq} and back-end Event Builders and for out-of-band messages as part of Control, Configuration and Monitoring.  The IPC used by the Data Selection spans both descriptions as it passes derivations of a subset of detector data (trigger primitives, candidates) and culminates in a source of out-of-band message (trigger commands) to direct the readout by Event Builder and other components of detector data that is held in the Upstream \dword{daq} buffers.
...
As part of the CCM, this include the issuing of control and reconfiguration commands to and receiving of monitoring messages from essentially all \dword{daq} components. 
As part of Data Selection, this includes the transfer of trigger primitive, candidate and command messages. 
In the Upstream \dword{daq} this includes the Buffer Interface Service which provides access to the Upstream \dword{daq} primary buffers for queries by Event Builder and other components. 
IPC must be implemented broadly across many \dword{daq} systems and ZeroMQ allows their problems to be solved in common, shared software.  As CCM has the most complex IPC needs, this work is organizationally considered part of this system.
...
As described in~\ref{sec:fd-daq:design-backend}, \dword{artdaq}~\cite{artdaq} utilizes IPC between its back-end components. 
It has been well tested with ProtoDUNE and other experiments. 
\dword{artdaq} may be used for some portions of the IPC described above. 
For example, if the Buffer Interface Service is implemented as an \dword{artdaq} Board Reader it would necessarily use \dword{artdaq} IPC. 
This would limit the types of clients that could query for data in the buffers to be \dword{artdaq} modules. 
Understanding how to optimally select an IPC for such parts of the \dword{daq} connection graph is an area of ongoing R\&D effort.
...
of two out of the six \dword{apa}s employed in protoDUNE. This was achieved
with two TPC RU's, without co-processor boards, and only one \dword{apa} read
...
\item Back-end \dword{daq}, CCM and Software Infrastructure:
 successful back-end \dword{daq} implementation, including event builder
...
ProtoDUNE \dword{daq} hardware as used during beam data taking.

\begin{dunefigure}[ProtoDUNE DAQ hardware during beam data taking]{fig:daq-rates}{ProtoDUNE \dword{daq} hardware as used during beam data taking. \label{fig:sp-daq:protodunedaqpic}}
...
\item introduction of \dword{fpga}-based hit finding (for TPC readout)
...
and IPC, which is common across multiple \dword{daq} subsystems.
...
\dword{apa}. In ProtoDUNE, \dword{felix} allows streaming of data coming over multiple 10
...
In ProtoDUNE, a single \dword{apa},  \dword{felix}-based readout consists of two servers,
...
publisher) were carried out specifically for ProtoDUNE: each \dword{felix}
...
the \dword{felix} front-end readout is
...
trigger candidates generation on the \dword{felix} host. 
...
\begin{dunefigure}[Topology of the FELIX-based
    ...
    upstream \dword{daq} of ProtoDUNE (from~\cite{pdsp-felix}). The \dword{felix} host servers are publishing the data from the WIBs over 100Gb network interfaces. The BoardReader hosts are carrying out trigger matching, data compression and forwarding of fragments to the event builder.}
...
Upstream \dword{daq} development efforts at protoDUNE include a parallel test 
...
using the \dword{felix} Demonstrator and PBM at ProtoDUNE will focus on
...
data selection and back-end \dword{daq} subsystems
at ProtoDUNE. 
...
\begin{dunefigure}[Trigger primitives, one APA  in ProtoDUNE-SP data]{fig:daq-hitfinder}{Illustration of the trigger primitives found by the CPU algorithm in one nominal readout of one \dword{apa}  in ProtoDUNE-SP data.  Although the data was ultimately recorded due to trigger, the algorithm ran in full-stream mode.}
    \includegraphics[width=0.9\textwidth]{daq-hitfinder-montage.pdf}
\end{dunefigure}

\begin{dunefigure}[Trigger primitive rates in ProtoDUNE-SP, four categories]{fig:daq-tp-rates}{Trigger primitive rates in ProtoDUNE-SP as a function of threshold in four categories: all data (top left),  after removal of particularly noisy channels (top right), with HV off so no contribution to signal (bottom left) and HV off and noisy channels excluded (bottom right).}
...
Trigger candidate generation, building on trigger primitives information
and considering integral ADC and trigger primitive proximity by channel
and time (in 50 microseconds) space, has also been studied with Monte
Carlo simulations~\cite{riveralast}.
Trigger candidates with sufficient total integral ADC can be accepted to
generate corresponding Trigger Commands for localized high energy
...
back-end \dword{daq} subsystem will be developed and tested in a similar way.
