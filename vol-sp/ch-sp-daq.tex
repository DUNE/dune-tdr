...
of raw data processing and buffering is performed underground, thus controlling consumption of available data bandwidth  to the surface. A
...
and detection of neutrinos from a nearby \dfirst{snb}. The
...
activity, on the other hand, is characteristically distinct, as it can
manifest as up to several thousands of low-energy neutrino interactions
arriving over multiple seconds. As such, supernova burst neutrinos
are associated with activity that extends over the entirety of the
detector and over a relatively long time interval.
...
Strategies will be developed and validated during commissioning and early running that will limit the readout to some subset of the detector module and these are expected to allow an increase in this rate limit by about an order of magnitude.
...
single-phase module. There is nevertheless no requirement to
...
Expected data rates from physics signals of interest, which fit the requirement that of \offsitepbpy sent to permanent storage, are summarized in Table~\ref{tab:sp-daq:rates} and detailed in~\citedocdb{9240} and potential bottlenecks were analyzed in~\citedocdb{11461}.
...
\item[High-energy interactions] High-energy interactions include cosmic muons, neutrino beam interactions, atmospheric neutrinos, and nucleon decays. 
  The trigger efficiency for these interactions is explicitly required
  to be $>$99\% for any given particle type (electron, muon, photon, etc.) that has a localized (confined to a few contiguous APAs) visible energy deposition above \SI{100}{\MeV}.
...
\item[Low-energy interactions] The primary physics target for
  ...
  Low-energy trigger candidates (with threshold at or below
