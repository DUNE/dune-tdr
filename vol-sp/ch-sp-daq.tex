...
\fixme{Add missing
  references. Georgia}
\fixme{Add section on co-processor development. Georgia}
\fixme{}
...
nominally set to 5.4~ms, corresponding to 2.4$\times$2.25\SI{\milli\second}. The
latter (2.25\SI{\milli\second}) assummes DUNE's nominal drift electric field of 500~V/cm.
...
trigger prompts 5.4\si{\milli\second} of losslessly compressed TPC
data plus 5.4\si{\milli\second} of lossy compressed PDS data from
...
is expected to be of relatively low energy (10-30\si{\mev}),
...
\begin{dunefigure}{fig:daq:readout-blocks}{\dword{dune} upstream
  ...
  \includegraphics[width=0.8\textwidth]{daq_functional_blocks_upstream.pdf}
\end{dunefigure}
...
Viable algorithms for the low level and module level trigger already exist, including
...
statistical metrics carried by the input \dwords{trigprimitive}, which
...
to the readout of the entire detector module for a period equal to 5.4\si{\milli\second}.  Extended triggers (\dword{snb}) will
...
high level filter, which resides in the back-end part of the \dword{daq} and
...
The functionality of the \dfirst{daqbes} is represented by the Output circle in Figure~\ref{fig:daq-conceptual-overview}. 
...
To minimize data extraction latency, the \dword{daqbes} must execute
trigger commands to completion in an asynchronous way (not serially). 
This asynchronous execution is governed by the \dword{daqdfo} and operates as illustrated in Figure~\ref{fig:daq:backend} and as discussed here:
...
The artDAQ system~\cite{artdaq} utilizes IPC for connecting its various elements.  It is full featured and well tested in production settings including in ProtoDUNE.  It is a natural choice for providing a basis for the DAQ back-end.  It will also be considered for providing a basis for the Data Selection system.  Upstream DAQ readout queries may also be implemented in terms of artDAQ elements.
...
In ProtoDUNE, a single APA’s FELIX readout consists of two servers,
with a point-to-point 100 Gbps network connection. The FELIX I/O card
interfaces with its host PC through 16-lane PCIe Gen3. It transfers
the incoming WIB data directly into the host PC’s memory using
...
are equipped with embedded Intel QuickAssist (QAT) \cite{xx} technology for
...
\fixme{Add text on co-processor development}
