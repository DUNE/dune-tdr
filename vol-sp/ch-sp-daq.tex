...
is \dword{snb} distance-dependent). \fixme{confirm DUNE requirement}
...
components (White Rabbit) for timing and external trigger signal I/O.
...
(dynamic and fixed-baseline zero-supression)
...
month, in accordance with \dword{dune} physics
requirements. An energy-weighted multiplicity count scheme can be
applied to further increase efficiency and minimize background
\cite{bib:docdb14522}. This is illustrated in
Figure~\ref{fig:daq-snb-eff-Eweight}, demonstrating $>$90\% efficiency
...
only counting information, the efficiency is 6.5\%. 
...
\begin{dunefigure}[Supernova burst trigger
  ...
  \includegraphics[height=6cm]{daq-efficiency-snb.pdf}%
  \includegraphics[height=6cm]{daq-snb-eff-lmc.pdf}%
\end{dunefigure}
