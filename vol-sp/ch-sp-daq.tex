...
\begin{dunefigure}{fig:fd-daq:layout}{\dword{daq} design physical
    layout focusing on a single \SI{10}{\kilo\tonne} module.
  }
...
the PDS electronics (see Chapter~\ref{pds}); therefore the total raw data volume received by
...
\begin{dunefigure}{fig:daq-rates}{Expected physics-related activity
  ...
  \includegraphics[width=0.5\textwidth]{daq-rates.png}
\end{dunefigure}

\begin{dunetable}
[Expected DAQ Yearly Data Rates]
{lrl}
{p{0.3\textwidth}p{0.2\textwidth}p{0.4\textwidth}}{tab:sp-daq:rates}{Summary
...
trigger\\\colhline
Front-end calibration & 200 TB & \\\colhline
...
channels only; 6 bytes per primitive \\\colhline
\end{dunetable}
...
{p{0.4\textwidth}p{0.4\textwidth}p{0.2\textwidth}}
...
The entire DAQ is composed of six distinct subsystems: (1) front-end
readout, (2) data selection, (3) back-end DAQ, (4)
inter-process communication (IPC), (5) control,
configration, and monitoring (CCM), and (5) timing distribution. The physical location
...
Figure~\ref{fig:daq-conceptual-overview}, and Figure~{fig:daq-design}
summarizes the design implementation of the 
DAQ system. Front-end readout is carried out by custom data receiver and
processing FPGA/CPU hardware which is hosted in O(100) servers in the CUC. A
corresponding number of servers is responsible for the execution of
additional CPU-based low-level processing of trigger primitives
...
subsystem. The back-end DAQ subsystem is hosted in O(10) servers
responsible for event building, and dedicated storage for built
events. Upon receiving a trigger command, the back-end DAQ queries
data from the front-end readout buffers and builds that into ``event
records'', which are stored as files on non-volatile storage. Event records can be optionally processed in a high-level
filter/data reduction stage, which is part of data selection, for
further down-selection, prior to becoming custody of DUNE offline. Pervasively, the  \dfirst{daqccm}subsystem provides the central orchestration (Section~\ref{sec:fd-daq:design-run-control}), the \dfirst{ipc} subsystem provides overall communication (Section~\ref{sec:daq:design-ipc}), and the \dfirst{daqtss} provides synchronization (Section~\ref{sec:sp-daq:design-timing}).

\begin{dunefigure}{fig:daq-conceptual-overview}{\dword{daq} Conceptual
   Overview of DAQ System Functionality}
  \includegraphics[width=0.8\textwidth]{daq-toplevel-conceptual.pdf}
\end{dunefigure}

\begin{dunefigure}{fig:daq-design}{\dword{daq} DAQ Design Implementation}
  \includegraphics[width=0.8\textwidth]{daq-overview.pdf}
\end{dunefigure}

Key to the implementation of the DAQ design is the requirement that
the system is partitionable. As a result, the system can operate in
the form of multiple independent DAQ instances, each independently
mapped across all subsystems and uniquely mapped among subsystem components. 
More specifically, any given partition may span the entire %up to one
\dword{detmodule} or some subset of it. This ensures continual readout of the
...
From intro figure: Localized activity
    ...
    Localized activity with a 10 MeV deposited energy threshold
