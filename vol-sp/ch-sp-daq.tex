...
modules.
%
The underground computing required for the DAQ to service the SP detector module is expected to occupy less than ten racks in the \dword{cuc}.  Less than eight are needed for housing the FELIX host machines.  The machines hosting data selection triggering software will fill less than two racks.  A small number of 1U machines are needed to host various other services and they can fill the remaining space in the ten racks.  Allocating a quarter of the total racks to the SP module thus leaves two racks available for contingency.
%
The power usage by the FELIX, data selection and service host machines is expected be less than but near one quarter of the power available to the total DAQ in the CUC.
To meet a fair share fraction, it is assumed that the FELIX hosts and data selection machines will consume less than \SI{500}{\watt}. 
On top of that, each FELIX board may use up to the maximum allowed for PCIe x16 cards (\SI{75}{\watt}).
The machines hosting services are not large contributors to the power budget as they are small in number and expected to run somewhat idle require no more than \SI{200}{\watt}.
...
A new organizational structure has been adopted to see the project through engineering design and construction, and this structure is expected to evolve further in the future in order to meet the needs of the consortium. This is shown in Figure~\ref{fig:daq-org}. Each working group has a designated working group coordinator.
