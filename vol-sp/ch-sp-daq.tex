...
\fixme{requirements vs specifications is murky. Anne}
...
the \dword{daq} are provided below. The \dword{daq}-level requirements
 specifications are 
...
An additional
challenge and one that is also unique to DUNE relative to past \dword{lartpc}
experiments is to self-trigger on \dword{snb} activity.
...
\fixme{trouble parsing above sentence. Here's a try:  This allows data-driven trigger decisions to be delayed, and for those related to interesting trigger physics data, to be captured later with minimal loss of physics information. }
The
...
%With respect to 
Regarding readout considerations, with the exception of
\dword{snb} triggers, unbiased data spanning a time on the order of the maximum possible drift for any other given trigger is to be recorded by the \dword{daq}. 
\fixme{The daq must record all non-SNB unbiased data that span the maximum drift time window for any non-SNB trigger .. ? Anne}
...
The data selection subsystem, an online, primarily software-based system implemented accross multiple levels of data selection for both TPC and PDS readout, will facilitate the data-driven trigger decision. 
...
\item Design very conservatively for the first \dword{detmodule}, e.g.,~utilizing
  all available infrastructure, keeping data selection decisions
...
and summarized in Table~\ref{tab:sp-daq:interfaces}. 
\fixme{is `interface agreements' important? anne}
The system
...
Section~\ref{xx}). Ten \SI{10}{Gbps} links are expected per \dword{apa}, and have
been specified as 300m OM4 multi-mode fibers from SFP+ at the \dword{wib} to
miniPOD on \dword{felix}. The data format has been specified to use no
...
be of the form of 150  \SI{10}{Gbps} OM4 fibers from the full FD module. 
\fixme{from a full module or the full detector? anne}
This
...
for real-time data quality monitoring.  The offline computing consortium 
\fixme{consortium or coordination?}
is
...
for transfer to
\fnal. \dword{daq} and offline computing are jointly responsible for data
...
The DUNE FD \dword{daq} Consortium constructed and operated the \dword{daq} system for
\dword{protodune} which facilitated two incarnations of \dword{daq} 
architecture: Felix and RCE based. \dword{daq} design and construction for
...
ray activity, and facilitating further \dword{daq} development toward
\dword{dune}.

The \dword{protodune} \dword{daq} system is depicted in
Fig.~\ref{xx}. The \dword{daq} is split among two readout architecture
...
\dword{dune} \dword{daq} , namely that (1) the \dword{protodune} \dword{daq} is externally
triggered (and at a trigger rate which is more than an order of
magnitude higher than that anticipated for \dword{dune}),
and therefore the current \dword{protodune} \dword{daq} does not facilitate online data
...
overcome to demonstrate data selection capability for the DUNE \dword{daq} 
design, and this is a main component of future \dword{daq} development plans, described
in Sec.~\ref{xx}.

Despite being a variant design and with more limited scope, the already
successful operation of \dword{protodune} \dword{daq} has provided several key
...
interfaces (understanding the exact \dword{daq} scope and interfaces to TPC
...
to be carefully planned out for the early stages of \dword{daq} development and
...
respect to debugging hardware issues from the upstream \dword{daq} end. It was
...
maintainign this same capability is part of the DUNE \dword{daq} philosophy.
...
data analysis; standardization of remote \dword{daq} operation solutions;
better control room tools; additional \dword{daq} infrasturcture (disk
...
These concepts are mapped to specific \dword{daq} sub-systems as follows:
...
In addition to deconstructing the design into sub-systems, the \dword{daq} is also separable into multiple \dwords{daqpart} or instances. 
...
For the most part, the FD \dword{daq} will require that supported transport mechanisms are reliable. 
...
A single inter-domain IPC system will be developed for the FD \dword{daq} . 
...
An example of a domain-specific IPC is that used by art\dword{daq} that will be the heart of the \dword{daqbes} as described in Section~\ref{sec:fd-daq:design-backend}.
...
The \dfirst{daqccm} subsystem (Section~\ref{sec:daq:design:ccm:control}) must communicate with all \dword{daq} domains and in some cases directly to domain components and so is also expected to utilize the \textit{lingua franca} IPC. 
...
It is not cost effective to design the \dword{daq} with a full path to final output which can accept this rate pulse.
...
sub-system must make data selection decisions in a way that allows the \dword{daq} 
...
\item Has been evaluated favorably CERN~\cite{Dworak:2012mf} and has been used in various \dword{daq} contexts in other experiments.
\end{itemize}

Initial prototype tests have demonstrated that the raw data throughput of ZeroMQ inter-thread transport is sufficient for use in even the highest rate \dword{daq} context (input to trigger primitive production).
