...
nominally set to \spreadout, corresponding to 2.4$\times$\SI{2.25}{\milli\second}. The
...
{tab:sp-daq:rates}{Summary
  of expected data rates for initial single-module running.
  ...
  at a time.  Improved readout algorithms will be developed and evaluated with the initial data and are expected to provide about an order of magnitude reduction in data while retaining efficiency.}
...
%
(4) \dfirst{daqccm} (Section~\ref{sec:fd-daq:design-run-control}), 
%
(5) \dfirst{daqtss} (Section~\ref{sec:sp-daq:design-timing}), and 
%
(6) infrastructure software. The physical extent
%
...
facilitates event building in approximately \hlfix{ten servers}{Why 10?} and buffering for built
...
threshold defined in terms of the noise level. This threshold crossing
represents a ``hit''. 
...
with a point-to-point 100 Gbps network connection to the \dword{felix} host computer.
