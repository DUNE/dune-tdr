...
\begin{enumerate}
\item one dual socket multicores 2U server, with two 10 Gbps and two 1 Gbps ethernet ports for redundant data transmission and control, with at least 256 GB of DDR4 RAM, and sufficient PCIe lanes to host 2 TB of SSD disks,
\item two \dword{felix} cards, with a PCIe (IIIx16) interface, supporting up to 12 9.6 Gbps serial optical links for data reception,
...
In the baseline design, the 20 fibers from two \dwords{apa} are aggregate into one \dword{fec} with each \dword{apa} connected to one \dword{felix} board. 
If technology advances sufficiently a PCIe IV (or V) version of \dword{felix} may be produced to increase the data aggregation such that each board would accept 20 (or 40) links. Nevertheless, considerations of fault tolerance, may advice against a very large concentration of links into a single RU.
