...
The \dword{daq}-level requirements are imposed to ensure that the
...
Fig.~\ref{xx}. \fixme{describe here the main two hardware platforms,
  neither of which is exactly the baseline for the TDR, but how the
  baseline combines elements of the two}.
There are two key differences between \dword{protodune} and
...
functionality. Below we discuss important lessons learned.
...
formalized in \citedocdb{}. Hardware integration tests prior to
...
pre-production. In the case of the trigger, despite its simplicity
relative to the \dword{dune}'s data-driven data selection scheme, it
was found that the system required more commissioning time.
...
Back-end was found to be too monolithic, and hardware configuration
and readout processes should be decoupled in future incarnations of
this subsystem.

Run Control. the eventual design must accommodate
changes in the overall system and provide more flexibility and
granularity.
...
From that input the \dword{daqdss} must form a \dword{trigdecision} by producing a \dword{trigcommand}.
This command summarizes the observed activity that led to the decision and provides addresses of the data in the \dword{fe} buffers which capture information about the activity.
This command is consumed and executed by the \dword{daqbes} as described in Section~\ref{sec:fd-daq:design-backend}. 
It is also provided to the \dword{etl} which may distribute it to other \dwords{daqpart} for consideration.
...
The \dword{daqdss} must select data associated with beam interactions, atmospheric neutrinos, rare baryon number violating events, and cosmic ray events depositing visible energy in excess of 100 MeV with high (targeted at $>$99\%) efficiency.
...
To meet the requirement that the \dword{dune} \dword{fd} maintain a $<$\SI{30}{\peta\byte/\year} to permanent storage, the \dword{daqdsn} sub-system must output a stream of data which is almost four orders of magnitude reduced from its input.
...
The \dfirst{daqbes} will thus allow for some form of L2 processing although its scale has not yet been determined. 
To facilitate development and implementation of L2 data reduction, the \dword{daqbes} will support processing modules were originally developed in an offline context.
