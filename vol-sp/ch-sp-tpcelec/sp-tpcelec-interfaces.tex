...
frames); Electrical (bias voltage, \dword{femb}--CR boards connection, grounding 
scheme) & \cite{bib:docdb-6670}
\\ \colhline
\dword{daq} & Data output from the \dword{wib} to the DAQ backend, clock distribution,
controls and data monitoring responsibilities & \cite{bib:docdb-6742}
\\ \colhline
\dword{cisc} & Rack layout, controls and data monitoring, & \cite{bib:docdb-6745}
\\ \colhline
...
\\ \colhline
\dword{itf} & Material handling and testing activities of  the Cold Electronics 
...
The most important interface is that between the Cold Electronics
and the \dword{apa} consortium. The design of the \dwords{femb}
and of the \dwords{apa} are strictly intertwined, both from the
...
Cold Electronics and the \dword{pds} cold cables are initially
attached to the yoke of the \dword{apa}. The Cold Electronics
cables for the bottom \dword{apa} need to be routed through the 
frames of both the bottom and top \dwords{apa}. The Cold
Electronics consortium provides the bias voltage for the \dword{apa}
wires, as well as for the electron diverters and the field
cage termination electrodes (the latter are a responsibility of
the \dword{hv} consortium), using the SHV boards that are mounted
on the \dwords{apa}. The grounding requirements discussed in
Section~\ref{sec:fdsp-tpcelec-design-grounding} informs the
design of all the mechanical and electrical interfaces between
the Cold Electronics components and the \dwords{apa} and also the
design of the connections between the top and bottom \dwords{apa}
...
\dword{pdsp}, taking into account the need 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{HV}
\label{sec:fdsp-tpcelec-interfaces-hv}

The Cold Electronics and \dword{hv} consortium's interface
...
the SHV boards that are mounted on the \dwords{apa}, that
...
The Cold Electronics consortium will work with the teams responsible
...
the \dwords{apa} after integration with the \dwords{femb} will be 
...
The \dword{pds} detector component should be isolated from the Cold
...
\dword{pds} and Cold Electronics cables will be housed together in
...
the \dwords{pds} warm electronics on the top of the cryostat, as
well as connecting the clock distribution system for the mini--racks.

The \dword{cisc} consortium mostly provides software for the slow
control and monitoring of the status of the Cold Electronics components.
The \dword{cisc} and Cold Electronics consortia may also have an
