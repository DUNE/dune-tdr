...
which includes the cold boxes that shield the \dwords{femb}, the
...
SACI protocol~\cite{SACI} for configuration rather than \dword{i2c}.
\end{itemize}
...
two stacked \dword{apa} frames, which is described in Section~\ref{sec:fdsp-apa-qa-prototyping}.
...
on all of the \dword{apa}s that had been tested in the cold box. % by replacing the \dword{femb}. 
...
\dword{microboone} by means of RC filters that have been added
on the \dword{pdsp} \dwords{femb}. Further work is required to
understand why these RC filters do not completely suppress this
...
It should be noted that even if the reference for \dword{spmod} makes use
...
and further validation is planned for Fall 2019. We consider this solution as
