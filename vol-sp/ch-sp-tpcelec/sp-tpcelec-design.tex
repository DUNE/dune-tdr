...
\dword{lvds} can be selected as designed.  The debug mode of the data formater has not yet been tested,
but in its normal mode, the data formater operates as designed.  The \dword{adc} pipeline functions as
...
input to the \dword{adc} pipelines.  This is essentially the Integral Non-Linearity (INL) of the uncalibrated
pipeline, the difference being that the deviation from linearity is plotted as a function of time rather than
as a function of the \dword{adc} output code.  This result is very close in magnitude to the INL expected from
...
codes was fit to the expected distribution for a sine wave.  The deviation from the fit is the Differential
Non-Linearity (DNL).  A cumulative sum of deviations yields INL.  The DNL is reasonably close to expectation,
but the INL indicates that the \dword{adc} is not yet fully calibrated.
