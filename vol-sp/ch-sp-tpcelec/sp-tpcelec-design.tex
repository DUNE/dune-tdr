...
During the commissioning of \dword{pdsp}, violations of these
...
the calibraiton capacitance is extremely stable against temperature variations, 
...
A clock-data separator on the \dword{wib} separates the signal received from the timing
system  into clock and data. 
...
The  \dword{spmod}  will likely need two additional iterations of the custom design \dword{asic}s, one 
for the \dword{adc} and another for the data serialization, and the possibility of using a 
\fixme{Marco to fix. or would this be the one asic that rules them all?} single 
