...
To avoid structural ground loops, the \dword{apa} frames described in Chapter~\ref{ch:sp-apa} are 
...
Signals associated with the \dword{pds}, described in Chapter~\ref{ch:sp-pd}, are carried directly on shielded, 
...
{fig:ce-scheme}
{The baseline \dword{ce} architecture. The basic unit is the \num{128}-channel \dword{femb}. Note that only one \dword{ce} flange is shown to simplify the illustration. Note that \dword{ssp} stands for \textit{SiPM Signal Processor} (see Chapter~\ref{ch:sp-pd}).}
...
The TPC data rate per \dword{apa}, with an overall multiplexing factor of 32 and 80 $\sim$1~Gbps data channels per \dword{apa},
...
Optical fiber is employed externally from the \dwords{wib} on the signal flange to the \dword{daq} and slow control systems described in Chapter~\ref{ch:sp-daq} and Chapter~\ref{ch:sp-cisc}, respectively.
