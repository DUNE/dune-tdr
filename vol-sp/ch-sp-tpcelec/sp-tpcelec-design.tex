...
from LBL, \dword{bnl}, and \dword{fnal}.  The \dword{asic} uses a conservative,
...
flanges that are distributed along the cryostat roof. The \dword{tpc} data rate 
...
\fixme{what is most probable? Is it that we can reduce the number of wires (by these methods)  only if all the asics fit on a single board?}
