...
the higher capacitance of the \dword{dune} \dword{spmod} wires compared to the small 
pads of \dword{nexo}. The \dwords{femb} constructed using this chip would use only 
two \dwords{asic}, compared to the \num{18} (eight \dword{fe}, eight \dword{adc} and 
two \dword{coldata}) needed in the baseline design. This drastic reduction in 
...
cap \dword{dac} based on V$_cm$ switching~\cite{5482529}, and has the option to be 
...
these are described in the literature~\cite{yeh} and have been successfully used in previous i
...
\item The warm interface is different. \dword{cryo} operates in synchronous 
...
\dword{fe} and \dword{adc} \dword{asic} design in LAr with the same input capacitance.
...
stable across temperature. Injected pulses on the frontend channels are visible on 
...
\forlbnc{Additional results on the performance of CRYO will be added prior to the
final submission of the TDR in July.}
...
the two kinds of \dwords{asic} will be available in early summer 2019 
...
\item three \dword{i2c}-like control lines (clock, data-in, and data-out).
\end{itemize}
...
and \SI{2.5}{V}), including the power dissipated in the linear voltage
...
resistance of the cable bundle is $\SI{41}{mi\milli\ohm}$ for the upper 
...
dissipated in the cables are reduced by two thirds for operation
in \dword{lar}, which allows for a reduction of the voltage 
provided by the \dword{wib}. The size of the cable bundles planned
for \dword{dune} represents a small reduction compared to that used
for \dword{pdsp}, where bundles of nice \num{20}AWG twisted-pair wires
...
a factor three for operation in \dword{lar}. The total power 
dissipated inside the power cables (few W at \dword{lar} temperature)
is small compared to the total power dissipaged in the \dwords{femb},
\SI{240}{W}.
...
This configuraiton would entail a small increase ($\sim14$\%) of the 
total resistance seen on the return wires. The low-skew shielded
twin-axial cables are required to transmit the following differential signals
between the \dword{wib} and the \dword{femb}:
\begin{itemize}
...
$\SIrange{335}{360}{W}$ out of \SI{650}{W} available.
...
used in \dword{pdsp} are prototype of the \dword{dune} ones discussed in the 
...
by the first prototype (P1) of an \dword{adc}, implemented in the \SI{130}{nm} 
technology and using the ``domino'' architecture~\cite{dominoADC}, and by
...
\begin{dunefigure}
[Signal over noise for muon tracks reconstructed in \dword{pdsp}.]
...
large angle wth respect to the direction of the wires in the plane considered
...
distortions of the electrical field in the \dword{tpc} volume that may locall change the
...
Section~\ref{sec:fdsp-tpcelec-design-femb-fe} discusses the plans and timeline 
