...
directly connected to a WIB. That \dword{coldata} chip relays \dword{i2c} 
...
serial streams operating in parallel. Data from the \dword{adc}s is captured 
...
of data per channel. Information from two \dword{adc}s are merged by a Data 
...
pipeline stage data are read out from one of the two pipelined \dword{adc}s 
