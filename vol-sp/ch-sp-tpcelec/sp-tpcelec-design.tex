...
\item{All electronical connections (low voltage power, bias voltage,
...
\dword{tpc} readout electronics should be connected to
the common plane of the \dword{femb};}
\item{The return leads of the \dword{apa} power line and any shield
for the clock. control. and data readout shall be connecdted
to the common plane of the \dword{femb} at one end and
to the flange of the \dword{sft} at the other hand; this shall be 
the only connection of the \dword{apa} frame to the cyostat;}
\item{The mechanicel suspension from the frame of the \dword{apa}
to the cryostat should be insulated.}
\end{itemize}
...
filter for the \dword{tpc} signals, optimized for loads in the range 
\SIrange{170}{210}{pF}, that is the capacitance of the \dword{apa} wires. It has programmable gain 
...
In the \dword{dune} \dword{spmod} the external clock operates at \SI{62.5}{MHz}
clock as discussed in Section~\ref{sec:fdsp-tpcelec-design-femb-coldata}. 
For the remainder of this section we assume for simplicity that the main clock is 
running at \SI{64}{MHz}. \dword{coldadc} is implemented in the \dword{tsmc}
...
\dword{coldadc} and share its' rising edge with the \SI{2}{MHz} clock. 
...
is not related to the use the master clock signal received from the \dword{wib}. Instead
...
\dwords{femb} in the early Fall of 2019.
...
sample~~\ref{sec:sp-daq:design-timing}, is inserted in the data frame
