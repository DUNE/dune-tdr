...
characterized at room temperature and \lntwo temperatyre (\SI{77}{K}). 
...
that are due to mechanical failures in the connection between
the \dword{apa} wire and the \dword{fe} electronics has
changed with time, with some channels becoming again active,
while others become inactive. Only one additional dead channel
...
leading to many insights on the detector behaviour and on the interactions
