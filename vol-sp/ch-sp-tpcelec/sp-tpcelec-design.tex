...
voltage power and the bias voltage to the \dword{tpc} electronics. Finally,
...
system design of the \dword{tpc} electronics.
...
that the \dword{tpc} electronis can surve in \dword{lar}. 
...
\dword{fe} \dword{asic} has been optimized for the capacitative
...
has highlighted a saturation problem in the design of the P2 \dword{fe} \dword{asic}
...
\dword{coldadc} will perform well, many circuit blocks can be bypassed, such
...
and easy-to-drive load to \dword{larasic}. Without the buffer, \dword{larasic} would
...
\dword{coldadc} was received at the end of January 2019.  Bench tests are being performed at
\dword{bnl}, \dword{fnal}, and \dword{lbnl}.  These tests use \dword{adc} chips mounted directly
...
\dword{lvds} can be selected as designed.  The debug mode of the data formatter has not yet been tested,
but in its normal mode, the data formatter operates as designed.  The \dword{adc} pipeline functions as
designed.  The operation of the digital logic that selects which weights to sum and does the arithmetic
to produce the \dword{adc} output has been validated; this has been done using a test mode that uses bits
...
multiplexer that connects SHA outputs to the \dword{adc} pipelines operate correctly.  The \dword{cmos}
...
\dword{larasic}.  One circuit block in the design hierarchy was unintentionally omitted, and as a result, the
...
the \dword{larasic} signals) from being input correctly to the sample-and-hold amplifiers.

To summarize, three problems have been revealed during functional testing of \dword{coldadc}:
\begin{itemize}
...
voltage be higher than designed.  This problem is also under study.  If necessary, the \dword{cmos} reference
block can be used.
...
power rail.  This would ``hardwire'' one option, input from \dword{larasic} to the single-ended to differential
input buffer.}.
\end{itemize}

Performance measurements of \dword{coldadc} have also begun.  Figure~\ref{fig:ADCpipelinenoise} shows the
...
\dword{adc} alone is thus $3.2 * 48.5 \mu V = 146.5 \mu V$ RMS.
...
Figure~\ref{fig:ADCpipelinedeviationfromlinear} shows the deviation from a linear fit to the \dword{adc}
output produced (at slightly below \lntwo temperature) with a slow ramp input to the differential test
input to the \dword{adc} pipelines.  This is essentially the \dword{inl} of the uncalibrated
pipeline, the difference being that the deviation from linearity is plotted as a function of time rather than
as a function of the \dword{adc} output code.  This result is very close in magnitude to the \dword{inl} expected from
...
{Deviation from a linear fit to the \dword{coldadc} output with a linear ramp input to the test inputs of the
  \dword{adc} pipeline (measured cold).}
...
A preliminary measurement (made at room temperature) of the static linearity of the \dword{adc} pipeline is
...
codes was fit to the expected distribution for a sine wave.  The deviation from the fit is the \dword{dnl}.
A cumulative sum of deviations yields \dword{inl}   The \dword{dnl} is reasonably close to expectation,
but the \dword{inl} indicates that the \dword{adc} is not yet fully calibrated.

\begin{dunefigure}
[Static linearity of an \dword{adc} pipeline.]
{fig:ADC-static-linearity-warm}
{Static linearity of an \dword{adc} pipeline.  This measurement was made at room temperature with a
\SI{17}{kHz} sine wave input to the test inputs.}
\includegraphics[width=0.7\linewidth]{sp-tpcelec-ADC-static-linearity-warm.png}
\end{dunefigure}

Initial measurements of the power consumption of \dword{coldadc} have also been
...
connecting the \dword{femb} to the \dword{ce} flange.
...
that the \dword{tpc} electronics be synchronized to a level of a few ns, which is
...
several improvements to the \dword{tpc} electronics system design. 
...
required to complete the desing of the various detector components. Later 
...
{p{0.20\textwidth}p{0.15\textwidth}p{0.40\textwidth}p{0.15\textwidth}}
...
Component & Status & Expected work & Required resources\\ \toprowrule
\dword{larasic} & Advanced & Fix issues observed in \dword{pdsp}, port differential output from \dword{coldadc} design & 1 FTE \\ \colhline
Commercial ADC & Complete & None & None \\ \colhline
\dword{coldadc} & \multicolumn{2}{l}{See text for details} & 2 FTE \\ \colhline
\dword{coldata} & \multicolumn{2}{l}{See text for details} & 1 FTE \\ \colhline
\dword{cryo} & \multicolumn{2}{l}{See text for details} & 0.8 FTE \\ \colhline
\dword{femb} & Advanced & Experience with multiple prototypes, final design will follow the \dword{asic} selection & 1 FTE \\ \colhline
Cold cables & Very advanced & Minor modifications, additional vendor qualification & 0.5 FTE \\ \colhline
Cryostat penetrations & Advanced & Add \dword{ce} flange for bottom \dword{apa} & 0.5 FTE \\ \colhline
\dword{wiec} & Very advanced & Add air filters and hardware interlock system & 0.4 FTE \\ \colhline
\dword{wib} & Advanced & Update design to use cheaper FPGA, modify \dword{femb} power, new firmware & 3 FTE  \\ \colhline
\dword{ptc} & Very advanced & Add interface to interlock system & 0.5 FTE \\ \colhline
Power supplies & Very advanced & Investigate possible additional vendors, rack arrangement & 0.2 FTE \\ \colhline
Warm cables & Very advanced & Finalize cable layout, identify vendors & 0.2 FTE \\ \colhline
Readout and control fiber plant & Very advanced & Finalize plant layout & 0.1 FTE \\ \colhline
\end{dunetable}
