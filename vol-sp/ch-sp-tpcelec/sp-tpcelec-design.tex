...
\dword{fe} \dword{asic} has been optimized for the capacitive
...
{Simulated integral non-linearity of an uncalibrated \dword{adc} pipeline.}
...
\begin{dunefigure}
[\dword{coldata} output eye diagram]
{fig:128Gbpseyesim}
...
\forlbnc{Additional results on the performance of the ColdADC ASIC will be added prior to the
...
\dword{fe} and \dword{adc} \dword{asic} design in \dword{lar} with the same input capacitance.
...
\fixme{ Figure 4.31 is still the old ProtoDUNE one. It needs to be replaced.}
\begin{dunefigure}
