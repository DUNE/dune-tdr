...
\dword{coldata} wafers, and 35 \dword{cryo} wafers for one \dword{sp} detector. Wafers
...
of the \dword{tpc} electronics consortium, requires many large procurements that 
...
could bid on the same package. Depending on the requirements of the funding
...
\dword{surf} using a color coding to indicate the activities that
are performed by external vendors, those that take place at one of
the consortium institutions or at \dword{surf}.
...
the \dwords{femb}, like voltage regulators and crystals, 
...
not necessary. One component used for the \dword{pdsp}
that we are not planning to use for the \dword{dune} \dwords{femb},
...
entire read-out chain where the input to the \dword{fe} amplifier
...
can be performed connecting them directly to a standalone \dword{wib},
...
one \dword{dune} \dword{sp} detector we plan to distribute the 
...
five sites for the \dwords{femb}, and each test site will be 
equipped with a cryogenic system like the \dword{cts}. All tests
...
with \dword{pdsp}, where all the testing activities were concentrated
...
all the testing activities for \dwords{asic} and \dwords{femb}
will rest with a member of the management of the \dword{tpc}
...
basis and then replaced them after a certain number of
testing cycles. 
...
connected to the \dwords{femb}. These tests will involve continuity
...
in consortia institutions responsible for the \dword{tpc} electronics and \dword{pds}; the
...
testing, a setup similar to the one for \dword{cts} (see
...
setups that include using \dwords{cts} for the cryogenic part
...
the \dword{tpc} electronics consortium, as well as the availability of the
integration setup at \dword{surf} in front of the cryostat, it is
desirable to integrate the \dwords{femb} on some of the \dword{apa}s 
and perform tests in cold boxes. For \dwords{apa} fabricated in the UK
these tests will be performed at CERN using the \dword{pdsp} cold box.
A similar setup needs to be put in place at \dword{fermilab} to perform 
these tests in the US ahead of the shipment of the \dwords{apa} to 
\dword{sdwf} and \dword{surf}. Both the setup at \dword{cern} and
at \dword{fermilab} will require a full power, control, and readout system, similar
