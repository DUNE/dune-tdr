...
\fixme{Again, a brief introduction here would help the logical flow of the document.}
...
build at least 3,200 \dword{femb}, 5\% more than necessary. If more spares are needed during the
\dword{qc} process or during integration, additional 
\dwords{femb} can be produced quickly if they have sufficient components on hand that require long lead times.
...
batches of 25 wafers. The exact number of \dwords{asic} 
...
of chips per wafer is 390 for \dword{larasic}, 928 for \dword{coldadc}, and 232 for \dword{coldata}; these chips will be fabricated together on the same wafer,
220 in the case of \dword{cryo}. The differences between these
...
used for installation on the \dwords{femb}, we assume that 10\% of the chips will fail during the \dword{qc}
process described later in this section, and an additional
5\% of the chips will fail during dicing and packaging. This means
that chips would be required for the 3,200 \dwords{femb}, the approximately 30,000 \dword{larasic} and
\dword{coldadc}, and the 7,500 \dword{coldata}, or 7,500
\dword{cryo} chips. This corresponds to at least 77 \dword{larasic} wafers,
33 \dword{coldadc} wafers, \dword{coldata} wafers, and 35 \dword{cryo} wafers for one
\dword{sp} detector. Wafers must be ordered
in batches of 25, so clearly we will have a significant number of
spares. The number of spare chips available can be 
reduced if wafers are purchased for two \dword{sp} detectors at a time. \fixme{Check the numbers here. It was confusing because of the way the lists were written, so the revision should be checked.}
...
Assuming we will have unused spares from the first detector, we will reduce the number of spares for the second \dword{sp}
detector.
...
\dwords{femb} to the \dwords{wib}; a technical qualification
includes tests of the entire cold chain (from the \dword{femb}
to the receiver on the \dword{wib}). Another 
...
limited access to the detector components will be provided by the \dword{ce}
consortium. After the \dword{tco} is closed, no access to detector components will be available because they should
...
version of the \dword{qc} program will be developed before the engineering design review. The program will then be used for
...
to rework the \dwords{femb}. This is more important where the three \dword{asic} solution is chosen for the
\dword{femb}. Given that in this solution, 18 \dwords{asic} are on the
\dword{femb}, only 2\% of the \dwords{femb} should
...
installation and before the functionality test, which for \dword{dune} will be performed in \lntwo. The
...
the current \dword{cts}. (Nine have already been
...
the baseline and of the RMS \fixme{Should this abbreviation be in the glossary?} of the noise. Calibration 
...
\dword{femb} \fixme{The last part of the previous sentence is unclear, as if a word is missing. It is "...transmitting data at the same time as data from the FEMB"?}. Connectors will be visually inspected 
...
one for each kind of \dword{asic}, will have evolved from those
...
power supplies and some cable testing equipment, most likely (at least for the power cables) \dword{cots} components.
