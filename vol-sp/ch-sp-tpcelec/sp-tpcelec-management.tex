...
\dword{esh} and for \dword{qa}/\dword{qc}. For the moment these activities are mostly
...
\dwords{asic}, the design of printed circuit boards, and testing, that
...
of the \dword{ce} detector components for the first Single
Phase \dword{tpc} Far Detector. This schedule, and the cost associated
...
\dword{tpc} Far Detector. To develop a schedule for the detector construction 
...
\dword{tpc} Far Detector is driven by the availability of the \dwords{femb},
...
is driven by the fact that the \dword{ce} consortium is
...
with the seventh \dword{pdsp} \dword{apa} in the cold box at CERN,
and with the \dword{tpc} in the \dword{iceberg} cryostat at Fermilab), which
...
for the construction of the first Single Phase \dword{tpc} Far Detector
...
If a second Single Phase \dword{tpc} Far Detector is built the critical
...
\dword{ce} consortium can be produced on a relatively short
...
before the \dword{asic} choice. The first components to be
...
\dword{ce} components to be installed on top of the 
...
done at Brookhaven, that of the \dword{cryo} chip at SLAC, while the
design of the new \dword{coldadc} was a joint effort of Brookhaven,
Fermilab and LBL. The \dword{coldata} ASIC was designed at Fermilab, with
...
\dword{sp} detector starting in the middle of 2018. Almost all
...
A detailed estimate of the \dword{ce} project costs has
...
will have only minor changes relative to the \dword{pdsp} design. While the
...
\dword{tpc} Far Detector, including all the costs in the prototyping and
