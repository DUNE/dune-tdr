...
\fixme{An overview of this section would help the logical flow of the document.}
...
DOE national laboratories. Table~\ref{tab:SPCE:institutions} provides a list of the participating institutions
and the contact people at each institution. \fixme{The first two sentences can be moved to the introductory section under 1.9 Organization and Management.} The present consortium organization
structure includes a leader and a technical lead (both currently
from \dword{fermilab}), with personnel from BNL helping with system design. A working group structure has been recently 
...
Technical Coordination and the consortia (mainly the \dword{apa}
...
the Technical Coordination team. The consortium \fixme{Which consortium?} will also have 
...
positions in the consortia is listed in Table~\ref{tab:SPCE:leadership}.
...
will be formed as necessary. For instance, a task force was
...
hwas discussed in Section~\ref{sec:fdsp-tpcelec-qa-reliability}. This 
task force will consider past experience from cryogenic detectors that have
operated for a long time (ATLAS \lar calorimeters, NA48 liquid 
krypton calorimeter, HELIOS); space-based experiments 
(FERMI/GLAST); and lessons learned from constructing and commissioning \dword{pdsp}. We will also seek input from other fields. Later 
this \fixme{If task force and working group have different definitions, they should be defined and then used in different ways. I note that they are used interchangeably in this paragraph. Is that the case?} task force will help in developing the \dword{qc} 
...
from outside the \dword{dune} collaboration, may form to establish the
criteria for selecting the \dwords{asic}. This task force will
be charged with proposing a recommendation that will then go to
...
of the \dword{ce} detector components for the first \dword{sp} \dword{tpc} far detector. This schedule, and the costs associated
...
appropriate tested, \fixme{I see nothing about testing in the table. Can the phrase be deleted or should testing be included in the table?} required for the first \dword{sp}
\dword{tpc} far detector. To develop a schedule for detector construction, 
...
A second detector may
be built using the \dword{sp} technology, so construction
of the \dword{ce} components for the second detector would
immediately follow construction of the first detector. The total number of
components for the second detector will be less than for the first detector, assuming that, for components inside
the cryostat, the spare parts from the first detector can be used for the second detector. Similarly, for the components on the top of the cryostat, the
pool of spares from the first detector should be sufficient to cover
the second detector.

\begin{dunetable}[Number of \dword{ce} components required 
for the Single Phase \dword{tpc} Far Detector]
{p{0.5\textwidth}p{0.5\textwidth}}
{tab:SPCE:components}
{Number of \dword{ce} components required for the single 
phase \dword{tpc} Far Detector (accounting for spares and yield).}
Component & Number required \\ \toprowrule
\dword{fe} ASIC & 30,000 chips (at least 77 wafers) \\ \colhline
...
Construction of the first \dword{sp}
\dword{tpc} far detector is driven by the availability of \dwords{femb},
which in turn depends on completing the design of the
\dwords{asic}. Thus, construction
of the \dword{apa}s could start as early as Spring 2019, while the
...
and then testing various flavors of \dwords{femb} before deciding \fixme{Deciding what?}
...
during Summer 2019. At the same time, lifetime
tests will be performed on all \dwords{asic}. Designs of the \dwords{asic} and \dwords{femb}, including results
...
for constructing the first \dword{sp} \dword{tpc} far detector
...
If a second \dword{sp} \dword{tpc} far detector is built, the critical
path will transition at a certain point \fixme{At a certain point is not very precise.} from the \dwords{femb}
to the \dword{apa}s, assuming that construction
of both \dword{apa}s and \dwords{femb} will continue 
without interruption after construction of the first detector. This is because constructing one \dword{apa} requires 
...
the corresponding rows of \dword{apa}s and properly connecting the \dword{apa}s to power, control, and read-out. The \dword{apa}s should be tested as
...
Design and prototyping for the \dword{sp} \dword{dune} detector have 
been concentrated so far at the DOE national laboratories, mostly
because the focus was on designing the new generation of 
\dwords{asic}. The design of the \dword{fe} \dword{asic} was
done at Brookhaven, \dword{cryo} chip at SLAC, and the new \dword{coldadc} was a joint effort of Brookhaven,
\dword{fermilab}, and LBL. The \dword{coldata} \dword{asic} was designed at \dword{fermilab}, with
...
Most of the
design and construction work for the \dword{pdsp} detector was done at Brookhaven, with other
...
\dword{sp} detector, which began in the middle of 2018. Almost all
the engineering of detector components, except for the boards to be installed on the \dwords{wiec} and most boards and set ups for testing, will remain a responsibility
...
\fixme{Some dates in the following depend on the actual LBNF
...
after the January 2019 DUNE Collaboration Meeting.}
...
be completed and prototypes fabricated. Procuring
...
take 10 months starting in Spring 20XX. Table~\ref{tab:SPCE:timeline} shows a preliminary list of 
...
be completely new, but the fabrication costs are rather well
known because the prototyping and fabrication of
\dwords{asic} are performed using frame contracts, either
through MOSIS in the US or through IMEC \fixme{Do MOSIS and IMEC need to go in the glossary?} in Europe, which
has fixed costs for production, while costs in the engineering
...
\dword{tpc} far detector, including all costs for prototyping and
...
{lrrrrr}
{tab:SPCE:personnel}
{Personnel needs (in FTE--years) for the construction of the \dword{ce} detector 
