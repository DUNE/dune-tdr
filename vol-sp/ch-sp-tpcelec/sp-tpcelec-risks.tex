...
Table~\ref{tab:risks:SP-FD-TPC} contains a list of all risks that we are 
...
mitigation activities discussed in the text are put in place. 
The risk probability, after taking into account the planned risk-mitigating
actions, is ranked as L (low, $<\num{10}$\%), M (medium,
\num{10}\%--\num{25}\%), or H (high, $>\num{25}$\%). The cost and schedule 
impacts are ranked as L (the cost of the subsystem increases by $<\num{5}$\%, 
schedule delay $<\num{2}$ months), M (\num{10}\%--\num{25}\% and \num{2}--\num{6} months, 
respectively) and H ($>\num{20}$\% and $>\num{2}$ months, respectively). It should
...
\input{generated/risks-longtable-SP-FD-TPC}
