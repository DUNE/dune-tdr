...
\fixme{And again, a brief overview of the section would help the logical flow of the document.}
...
and construction of the \dword{dune} \dword{sp} detector. In the two following
sections, we will discuss the risks that could be encountered during commissioning and
operation. For every risk, we will describe the mitigating actions 
...
considering; we will then discuss them in detail.
...
on the \dword{apa}s. This risk has several triggers: a lower yield 
that what we planned during the \dwords{asic} and \dwords{femb} 
fabrication or during the QC process; downtime at one of the QC sites; or losses during 
...
we can also split the QC process among various sites. We will also emphasize,
...
delays in \dword{apa} integration. As for the previous risk, we
do not expect to have delays in the installation of the detector inside the cryostat. \fixme{This last sentence in the paragraph appears to be misplaced. If this is about the first risk, the statement should go at the end of the discussion of that risk, not here.}
...
installation of the cold cables. (This risk was actually realized during
...
to use larger tubes, which have been studied many times in recent months. 
In these studies, we have assumed that we slightly reduce 
the cable plant size from \dword{pdsp}. We expect to retire this risk
...
between various components of the far detector go undetected until
...
integration tests tot reduce this risk as much as possible. These tests include integration tests with components provided
...
is still apparent in the \dword{pdsp} data, this should not preclude using the data collected for calibration and for 
physics analyses. Further studies are planned when \dword{pdsp}
data taking ends in 2018 to investigate the remaining sources of noise.
