...
Custom \dwords{asic} for \dword{lartpc} readout have been under devleopment for many years, and
...
cryostat was filled with \dword{lar};}
...
\fixme{Marco: replace figure with one where the correct X plane label is used}
...
can meet the noise %requirements 
specification. % presented in Section~\ref{sec:fdsp-tpcelec-overview-requirements}. % can be met.
...
\fixme{I think the new text fixes Anne's comment "some of the ``some channels'' needed masking? An approx value was obtained for a majority of the ``some channels'' (not the masked ones?)"} 
...
\fixme{Marco: replace the label Y-->X in the figure}
