...
%It should be 
Note that requirements SP-FD-01, SP-FD-03, SP-FD-04, and SP-FD-05
apply to %on 
other detector components. % other than the \dword{ce} ones.
...
range is driven by the requirement that the ADC digitization %does 
not 
...
be used reliably at room temperature. This is accomplished by using transistors
%fabricated 
with longer channels % lengths that are larger than the length commonly used
than those commonly used %for 
in otherwise similar \dwords{asic}, %using of the same technology, 
and operated at reduced bias voltage. 
