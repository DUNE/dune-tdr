...
The \dword{tpc} electronics encompass the hardware systems necessary to 
amplify, digitize, and transmit the \dword{tpc} ionization charge 
signals out of a \dword{dune} \dword{sp} \dword{lartpc}. This 
includes the cryogenic \dword{fe} electronics (amplifiers, digitizers, 
...
to the \dwords{apa}.  The \dword{tpc} electronics as presented here does not 
...
wires of the \dword{apa}s takes place inside the \dword{lar}, in boards that 
are directly mounted on the \dword{apa}. This approach to the \dword{tpc}
readout was tested for the first time in the \dword{dune} \dword{35t},
and extensively tested in the \dword{pdsp} prototype. It has also 
been adopted by the \dword{sbnd} experiment. %Sometimes, the 
The \dword{tpc} \dword{fe} readout components immersed in the \dword{lar} are also referred 
to as the \dword{ce}. 

The \dword{fe}  electronics are mounted inside the \dword{lar} to exploit the fact that 
...
electron transport through the grid plane of the \dword{apa} and its two planes of induction
...
The reduction in noise level obtained with the \dword{ce} 
%\dword{fe} electronics at \dword{lar} temperature 
greatly extends the reach of the \dword{dune} 
physics program. It allows measurement of smaller charge deposits, which 
mitigates  the risks of inability to reach the desired drift field 
...
minimal \dword{s/n} required for pattern
recognition depends on the tracking algorithms and the offline signal 
processing. %For the design of the \dword{tpc} electronics we 
We use the
minimal requirement of a total \dword{enc} less than 1000 e$^-$, consistent 
...
for the baseline oscillation analysis described in \physchlbl. The event classification is based
...
data compression, as discussed in Section~\ref{sec:daq:requirements}, that
...
discussed in Chapter~\ref{ch:daq};
...
\begin{dunefigure}
[The reference architecture for the TPC electronics]
{fig:ce-scheme}
{The reference architecture for the \dword{tpc} electronics. The basic unit is the
...
\includegraphics[width=0.99\linewidth]{graphics/sp-tpcelec-schematic-v3.pdf}
\end{dunefigure}
...
serial links. The \dword{tpc} signal processing is implemented in \dwords{asic} 
...
The \dwords{wib} are installed, together with \dfirsts{ptc} that distribute
...
%\pagebreak  test if this is really needed
...
all the \dword{apa}s in one \dword{dune} \dword{sp} detector module should
...
back end on optical links at a speed of $\sim\!\SI{10}{Gbps}$. This speed 
...
diameter of \SI{6.35}{cm} (2.5 inch) that are inserted in the frame of
...
The reference design of the \dword{tpc} electronics detector components is
...
\dfirsts{femb}, with each \dword{femb} enabling digitized wire readout 
...
\dword{apa} shares the \dword{pd} flange with the 
upper \dword{apa} but has a separate TPC readout flange.}
\includegraphics[width=0.7\textwidth]{graphics/sp-tpcelec-DUNE-FD-APA-readout-scheme-v1.pdf}
\end{dunefigure}

The reference design for the \dword{ce} calls for three 
...
from the \dwords{ptc} to the \dwords{wib} on 
the \dword{ptb}.}
...
to the hot-carrier effect~\cite{Hot-electron}, which limits the lifetime of \dwords{asic}.
...
similar to those caused by radiation damage, which can cause \dword{cmos}
...
of the transistors' channels. Another drawback of integrated circuits 
operated at \dword{lar} temperature is that the spread of the transistor 
properties becomes larger, making it more difficult to rely on transistor 
matching for circuit design. We must carefully test any commercial 
