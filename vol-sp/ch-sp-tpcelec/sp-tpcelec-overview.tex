...
\fixme{Marco: the following table is autogenerated from DocDB-11074 and DocDB-6419.
There are a typo (reduce is misspelled as redice) and a mistake (the validation
is Bench test not ProtoDUNE) in SP-FD-21, and "enc" should be replaced by "e$^-$"
in multiple places. The "13 bits" goal of SP-FD-20 is no longer in the official
requirements table (decision taken during the Executive Board meeting of
January 29).}
...
of increasing the power consumption inside the \dword{lar} and
...
The complete list of the components of the \dword{ce} required for
one \dword{apa} (the number of wires connected to each of these
...
The operation of the readout electronics in \dword{lar} for the 
\dunelifetime lifetime of the \dword{dune} experiment forces 
technological choices that have to be implemented in the design 
of the \dwords{asic} planned for the \dword{spmod}. It also imposes 
...
\begin{dunefigure}
[\dword{tpc} noise levels measured at \dword{pdsp} after \dword{lar} fill]
{fig:apa3-noise}
...
alternatives are being considered for \dword{dune}. The \dword{sbnd} 
Collaboration has demonstrated that there is also a \dword{cots} 
...
the \dword{dune} requirements and that could be used \dword{dune}, in the
...
Finally, it should be noted that the analysis of the \dword{pdsp} data
...
of \dword{apa} before and after the filtering. The spectrum prior to
