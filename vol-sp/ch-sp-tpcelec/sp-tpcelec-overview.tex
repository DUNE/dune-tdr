...
\fixme{Fourteen channels that consistently showed no capacitive load?} 
...
The collection channels with noise larger than \SI{1500}{e$^-$} had a problem %are 
caused by a problem in the P1-\dword{adc} 
...
\dword{pdsp} satisfies the %\dword{dune} 
\dword{sp} %\dword{fd} 
\dword{ce} system requirements for the \dword{fd} 
...
that from a \dword{microboone} event (Figure~\ref{fig:microboone-display}~\cite{Acciarri:2017sde}) demonstrates the improvements in performance of \dwords{lartpc}. 
...
show that the \dword{dune} detector can meet the  %requirements 
...
are motivated by the experience with the \dword{ce} in \dword{pdsp}. 
...
is available in~\cite{bib:docdb12367}. This document also discusses %In addition to reporting the lessons learned, 
the plans and timeline for addressing the issues 
observed in \dword{pdsp}. % are also discussed in this document. Here
This section and the following cover only the main issues  % are discussed, 
...
To resolve this problem, a new \dword{adc} \dword{asic} (also called a 
\dword{coldadc} \dword{asic}) is being developed by a  
...
alternatives are being considered for the \dword{spmod}. % \dword{dune}. 
...
and that could be used \dword{dune}, in %the
case that the custom \dword{asic} is not %developments were not successfully
...
of \num{384,000} chips), which would increase the cost %cause significant cost increases
and would require a very extensive \dword{qc} program. The qualification
...
%\dword{dune} 
than is expected in the \dword{spmod} due to the presence of cosmic rays traveling parallel
to the \dword{apa} wires. % in the first case. 
The problem cannot be 
discounted for \dword{dune}, however, since it could %also 
...
Section~\ref{sec:fdsp-tpcelec-design-femb-fe} discusses the olans and timeline for addressing this issue in a new \dword{larasic} prototype.
...
detached from
the \dword{femb}, causing a loss of communication.  
For \dword{pdsp} we replaced the \dword{femb} %This problem was fixed for 
on all the \dwords{apa} tested in the cold box. % by replacing the \dword{femb}. 
...
%,that contains 
a comparison of the noise spectrum on the three planes of \dword{apa} before and after the filtering. 
The spectrum prior to
the filtering shows %some interesting features. At 
that at very low
frequencies $<$\SI{60}{kHz}, the %noise is dominated by the
low voltage regulators that are installed on the \dwords{femb} dominate the noise. 
...
thanks to hardware filters developed at \dword{microboone} that have been
...
some cases the noise spectrum \fixme{sources?} have been identified (cameras,
