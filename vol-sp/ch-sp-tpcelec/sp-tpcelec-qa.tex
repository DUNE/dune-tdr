...
\dword{tdr} (Volume~\volnumbertc, Chapter 9). %Commented out because it breaks the automatic build system %(\refch{tc}{vl:tc-QA}).
...
many of which will be developed as part of system design tests in \dword{qa},
...
\dword{apa} will be characterized in September 2019 in the cold box like the \dword{apa}s installed
...
the most recent prototypes of the \dwords{femb} built using the \dword{asic} solution chosen
for building the \dword{sp} \dword{detmodule}. If possible \dwords{asic} from the engineering 
...
final \dword{dune} prototypes. A second period of data-taking 
...
planned for \dword{dune}, where the last electrode of the \dwords{fc} is also connected to
...
power to the \dword{ptc}, the fans and the heaters located on the 
...
%initial operation of \dword{iceberg} has been plagued with problems with the
...
in March 2018 with \dword{pdsp} \dwords{femb} installed on the \dword{tpc}. The
initial operation of \dword{iceberg} was impacted by problems with the
control of the argon gas pressure in the ullage of the cryostat, which caused
frequent boiling of the \dword{lar}, and by discharges of the cathode power supply. 
...
\dword{iceberg} commenced in June 2019, demonstrating stable operations of the
...
\begin{dunefigure}
[TPC ENC levels for all channels of the ICEBERG detector]
{fig:ENC-all-ICEBERG}
...
Each of the three setups (\dword{apa} in the cold box at \dword{cern}, \dword{iceberg} \dword{tpc},
...
both setups, and in particular, of tests performed using the \dword{pdsp}
\dword{apa}, is that the detector size is the one used in \dword{dune} 
...
new \dwords{asic} and \dwords{femb}.
...
on the \dwords{pcb} represent the most relevant aspect for \dword{dune}.
