...
that we can use for system tests: the \dword{pdsp} facility at dword{cern}, the
...
\dword{pdsp} took beam data in the \dword{cern} Neutrino Platform in 2018 and will continue
...
Five of the six \dword{apa}s were tested in the \dword{pdsp} \coldbox
...
on the \dword{apa}. Therefore, a very similar set of \coldbox tests are planned at \dword{surf} 
with the fully-instrumented \dword{dune} \dword{apa}s. A seventh \dword{apa} was delivered
to \dword{cern} in March 2019 and will be first equipped with \dword{pdsp} \dwords{femb}. This
\dword{apa} was characterized in October 2019 in the \coldbox like the \dword{apa}s installed
...
early 2020 using \dwords{femb} equipped with the \dword{cots} \dword{adc}, \dwords{femb}
with the \dword{cryo} \dword{asic}, and \dwords{femb} with the new \dword{coldadc} and
\dword{coldata} \dwords{asic}. The \coldbox will also be used to study the effect of
...
While the \coldbox test at \dword{cern} and \dword{pdsp} operations provide important 
...
\coldbox at \dword{cern}, the advantage of \dword{iceberg} is that it houses a small \dword{tpc} which
allows measurements with ionization tracks, which is not possible when performing
tests in the \coldbox. In addition, \dword{iceberg} enables system-wide studies with new
...
accommodate two half-length \dword{pdsp} \dwords{pd} with dimensions and connectors that already 
...
\includegraphics[width=0.26\linewidth]{graphics/sp-tpcelec-ICEBERG-cryostat.jpg}
\includegraphics[width=0.61\linewidth]{graphics/sp-tpcelec-ICEBERG-Tee.jpg}
\end{dunefigure}
...
\includegraphics[angle=270,width=0.43\linewidth]{graphics/sp-tpcelec-ICEBERG-TPC.jpg}
\includegraphics[angle=270,width=0.43\linewidth]{graphics/sp-tpcelec-ICEBERG-DAQ.jpg}
\end{dunefigure}
...
of five of the \dword{pdsp} \dword{apa}s at \dword{cern}. The core of the \dword{daq} system 
...
\begin{dunefigure}
[One side of the \num{40}\,\% APA with four FEMBs and the \dshort{ce} flange]
{fig:tpcelec_40apa}
...
Each of the three setups (\dword{apa} in the \coldbox at \dword{cern}, \dword{iceberg} \dword{tpc} at \dword{fnal},
...
design. Tests performed in the \coldbox at \dword{cern} and with the \num{40}\,\% \dword{apa} 
at \dword{bnl} are limited to noise measurements. These tests are not
performed at \dword{lar} temperature in the \dword{cern} setup. The advantage of
of the \coldbox at \dword{cern} and of tests performed in \dword{pdsp}
...
The \dword{tpc} \dword{ce} system of the \dword{dune} \dword{sp} \dword{fd} must meet 
...
calorimeter \dword{ce} have been in cold since 2004, with
...
segmentation of the \dword{ce} to understand which failures will most affect data taking, revisit recommendations for the \dword{asic} design, 
...
fabrication of the \dword{ce} components. The working group will also review
