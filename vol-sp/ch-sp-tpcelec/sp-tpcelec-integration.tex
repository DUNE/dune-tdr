...
required for the read-out of a pair of \dword{apa}s are installed prior 
...
at the \dword{itf} for performing checks at liquid Nitrogen temperature
...
liquid Nitrogen temperature after the integration with the \dword{pds}
...
cables for the bottom \dword{apa}s are not damaged when they are routed 
...
All the read-out tests are repeated again after the \dword{apa}s are put
...
read-out is working as planned, the \dword{ce} consortium will also
...
and read-out by the \dword{daq} back-end. Tests will be performed on the
...
gained with protoDUNE-SP, including an estimate of the time required
