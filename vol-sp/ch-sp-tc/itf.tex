...
\fixme{This heading can be eliminated so the introductory text immediately follows the major heading.}
...
Measurements of the detector responses are made at 16 positions along the length of the module (on two sides for double-sided \dword{pd} modules), checking the performance of each of the dichroic \fixme{Is this spelled correctly?} filters. 
...
{Picture of the scanner for operational tests of the \dword{pd} modules before integration into the APAs.} \fixme{This is unclear. What is being integrated into the APAs? The scanner or the modules? (This is probably obvious to anyone familiar with the APAs, but for anyone else, it would be a problem.) Is it before the modules are integrated into the AOAs? or before the scanner is integrated into the APAs?}
