...
The height of the crosses was chosen so a person can comfortably work on the WEC \fixme{WEC is not in the common glossary} and \dword{pd} flanges while standing on the cryostat roof. 
...
\subsubsection{Introduction} \fixme{As for the other introduction sections, the text could simply be placed after the major heading.}
...
The NOvA Far Detector Laboratory located at Ash River is owned and operated by the University of Minnesota using grants from the DOE and Fermilab. The Trial Assembly program for both \dword{protodune} and \dword{dune} are operated here. During the \dword{protodune} detector design many of the installation and deployment concepts where tested and modified here.  It was critical that we could attempt different concepts with full-scale mechanical components. Hazard analysis and procedure documents were developed and hands on working group meetings with the different consortia were held.  The lessons learned and training from the trial assembly helped insure that the assembly and integration for \dword{protodune} went well at CERN.
...
\begin{dunefigure}[DUNE - trial assembly at Ash River] %this gets %sniffed for table of figures
...
this facility can also be used as a training site for lead workers as \dword{dune} begins set up and testing mechanical modifications.
\end{enumerate}
...
{ll}
{tab:table-label}
{The full caption that appears above the table.} \fixme{I will assume this table will have an appropriate title.}
...
\item {\bf Liquid Level Monitoring.} Once installed in the four cryostat corners, the capacitive level meters are tested in situ \fixme{In situ should be in italics.}
...
\begin{enumerate}
    ...
    \item \dword{pd} system noise (pedestal, RMS) will be measured at regular intervals and its trend in temperature recorded.
     ...
     \item Values of the temperature sensors deployed in several parts of the cryostat will be constantly monitored to understand the progress of the cool-down phase and to relate them to the behaviour of the other \dword{detmodule} subsystems. 
