...
during the filling process. % These systems are fully described in Sec. X.3 of this chapter.
 
Argon purity must be established as early as possible in the filling process, a period in which gas analyzers are most useful, and must maintain an acceptable value, corresponding to a minimum electron drift lifetime of \SI{3}{ms}, throughout the data-taking period.  Dedicated  purity monitors %(Sec. X.3) 
...
\fixme{this doesn't quite match the subsystem chart figure}
...
\fixme{Not sure about use of word "requirement" in this section. anne}
%Some common 
...
The temperature measurement resolution should be less than \SI{2}{mK}, and the relative precision of those measurements should be less than \SI{5}{mK}.\footnote{The resolution is defined as the temperature RMS for individual measurements and is driven by the electronics. The relative precision also includes the effect of reproducibility for successive inversions in \dword{lar}.}
\fixme{should or must?} 
\fixme{inversions or immersions? (in footnote) anne}
The relative precision is particularly important because gradients below \SI{20}{mK} should be characterized.  
...
For gas analyzers, the operating range \fixme{must be such that what happens? important for what? anne} is an important requirement; details  are in Table~\ref{tab:fdgen-slow-cryo-requirements-1}.

For slow controls, the system must %be designed to 
be sufficiently robust to support %a large number of monitored variables, at 
\fixme{monitor?}
a minimum of 50,000  variables, and 
\fixme{support} a broad range of monitoring and archiving rates. The system must also %be capable of an 
...
Discrepancy between lab and {\em in situ}                                                                                                   		                                          
calibrations for temperature sensors			             & < 5 mK			                                                    & < 3 mK		                                          \\ \colhline                           
Discrepancy between measured temperature                                                                                                                                                      
map and \dword{cfd} simulations in \dword{pdsp}	                     & < 5 mK			                                                    & < 5 mK		                                          \\ \colhline                             
...
Precision (additional level meters, \dword{dune} side)				 & 10 mm			                                                    &  <5 mm		                                                      \\ \colhline         
...
LAr purity				                                     & O2 and H2O low enough  to measure drifting                            		                                                      
                                                               electrons of devices under test, ~\SI{0.5}{ms}.                                                                        
...
\begin{dunefigure}[\dshort{cfd} example]{fig:cfd-example}
  {Distribution of temperature on a plane intersecting an inlet (right) and halfway between an inlet and an outlet (left), as predicted by previous \dshort{cfd} simulations (from~\cite{bib:docdb5915}). (See Figure~\ref{fig:cfd-example-geometry} for geometry.)}
...
\fixme{to ensure that the simulation represents the design? anne}
\item 
...
\fixme{This doesn't make sense to Anne. The attenuation increases the lifetime?}
...
\begin{dunefigure}[Measured electron lifetimes in the four purity monitors at \dword{pdsp}]{fig-pdsp-prm}
  {The measured electron lifetimes in the four purity monitors as a function of time in \dword{pdsp}.}
  \includegraphics[width=0.9\textwidth]{PrMon_pdsp-PrM.pdf}
\end{dunefigure}

\fixme{three or four purity monitors?}
...
\fixme{each hold separate bias voltages and? (anne)} are electrically accessible via modified vacuum-grade \dword{hv} \fdth{}s. % and separate bias voltages held at each one.   
The anode grid and the field shaping rings are connected to the cathode grid by an internal chain of \SI{50}{\mega\ohm} resistors to ensure the uniformity of the \efield{}s in the drift regions. A stainless mesh cylinder is used as a Faraday cage to isolate the purity monitor from external electrostatic background. 
...
\fixme{check prev sentence - within what?. anne} The application continuously displays the waveforms and important parameters like measured electron lifetime, peak voltages, and drift time % of electrons 
in the purity monitors, and shows 
\fixme{the variation over time? anne} these parameters over time, thus pointing out %.This allows validating the impurity of the \dword{lar} and seeing 
...
\fixme{is LArTPC trigger window the same as neutrino spill trigger window? Can we just say 'trigger window'?}
...
\fixme{for fig \ref{fig:PrMon-SystemString}, say schematic, and give more info,  e.g., length, say it's shown horiz but will be vertical...(anne)}
...
\fixme{string?} with all purity monitors will be tested in the \lar test facility. The full string assembly will be installed and shipped to the \dword{fd} site. 
\fixme{install before ship?} A vacuum test in a long vacuum tube will be performed onsite before inserting the full assembly into the \dword{spmod} cryostat. 
...
(called individual sensors) at the top and bottom of the \dword{detmodule}, which are the most delicate regions. 
\fixme{delicate in what sense? (anne)}
...
and the top and bottom of the detector. 
\fixme{can we replace detector with cryostat in prev sentence?} 
...
\fixme{why here? and is it end wall of cryostat or field cage? No hyphenated end-wall; pls fix.}
...
Figure~\ref{fig:sensor-support}, right shows the baseline design of the
PCB support for temperature sensors with an IDC-4 male connector. 
It is $52\times \SI{15}{mm^2}$. 
\fixme{what is, the whole thing pictured, sensor mounted on pcb?}
...
\item a readout system based on a National Instrument\texttrademark{} Compact RIO Device  with a high-accuracy voltage signal readout NI9238 module with 24 bit resolution over a \SI{1}{V} range.
\end{itemize}
This readout system also drives the multiplexing circuit and calculates temperature values. The Compact RIO device is connected to the detector Ethernet network, sending temperature values to the DCS software through a standard OPC UA driver.
\fixme{have you defined these acronyms?}
...
the detector 
\fixme{the tpc and/or the PDS? "detector" is used for the full four-module FD.}
uses monitoring to guarantee that the top \dwords{gp} are always
...
e.g., the same model camera was used successfully to observe discharges
in \lar from \SI{120}{cm} away \cite{Auger:2015xlo}.
\fixme{which same model?}
...
\fixme{and readers would know that this is high?}
...
Figure~\ref{fig:cisc_PAB_photo} shows the PAB facility at FNAL with the ICEBERG \SI {3000} {liter} cryostat being readied for outfitting. This cryostat serves as a fast turn-around facility for the \dword{dune} Cold Electronics Small TPC test facility. 
\fixme{fix prev sentence. a facility for a facility? And can we indicate what is what in the photo?}
...
\fixme{clarify 'outside use'. Or can we drop it?} In the recent past, Blanche has been used for \dword{hv} studies, TallBo for \dword{pd} studies, and Luke for the material test stand work. These studies have contributed to the design and testing of  \dword{pdsp} components.
...
\fixme{\surf{}'s?} 
general computing infrastructure managed by project \dword{tc}. The network infrastructure requirements are described in
Section~\ref{sec:fdgen-slow-cryo-slow-infra}.
...
\fixme{it'sa available but you expect that less will be needed?}
...
\fixme{lots of "should" above - sounds wishywashy! (anne)}

The \dword{pdsp} detector control system software \cite{pdspdcs_proc}
provides a prototype for this required \dword{fd} slow
controls software, as described in subsection \ref{sec:cisc-slow-control-pdsp} below.
...
\fixme{can we reduce the text in the description column so that we don't have to move to longtable format? anne}
\begin{dunetable}
...
no electrical interference with the \dword{apa} and no possibility of coming in contact with the \dword{apa} wire planes.
Location of cameras/lights (provided by \dword{cisc}) for inspection of health of \dword{apa} wires.                 & \citedocdb{6679} 
\\ \colhline

\dword{pds}	     & 

PrMs and light emitting system for cameras both emit light that might damage PDs.
Under consideration hardware interlocks that avoid turning on lights accidentally when PDs are on.      & \citedocdb{6730}  
\\ \colhline
...
Safety issues. \efield simulations to guarantee proper shielding (\dword{cisc} responsibility with \dword{hv} guidance).
Avoid generation of bubbles any time the inspection camera is inside the cryostat, as it can lead to discharges when the \dword{hv} is turned on. 
...
\dwords{gp} could be used as support for cold cameras and individual temperature sensors.        & \citedocdb{6787}   
\\ \colhline
...
Calibration ports could be used to extract cables from \dword{cisc} devices. 	       & \citedocdb{7072} 
...
Assuming that the scope of software \& computing (SWC) group includes scientific computing support to project activities, there are substantial hardware and software
interfaces with that group      & \citedocdb{7126}  
\\ \colhline

Cryogenics             &  As mentioned in Section~\ref{sec:fdgen-slow-cryo-purity-mon} purity monitors and gas analyzers will be essential
to mitigate the \lar contamination risk. The appropriate interlock mechanism to prevent the cryogenics system from irreversible contamination
must be designed and implemented.        &  -   
...
are installed
after the detector \fixme{installation?} is complete.
...
\fixme{selection and installation of these things will be coordinated..?}
...
\fixme{m is not a unit of weight}
