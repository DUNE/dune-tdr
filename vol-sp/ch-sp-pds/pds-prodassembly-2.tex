...
The largest relative contraction of mechanical components is between the FR-4 frame and the polystyrene \dword{wls} plates. The most critical relative shrinkage is between the face of the photosensors and the \dword{wls} plate, where the significantly greater contraction of the \SI{92}{mm} width of the plate relative to the \dword{pd} module structure will result in an increase in separation between the sensor face and the plate of approximately \SI{1.3}{mm}.  
 Simulation and testing are ongoing to understand the impact this will have on the performance of the \dword{xarapu}.  Relative contraction along the long axis of the \dword{wls} plate, while greater in magnitude (the \SI{487.0}{mm} long plate results in a \SI{5.8}{mm} relative contraction), is less critical to the performance of the detector and is addressed by the \dword{wls} bar mounting structure.
...
Prototype testing confirmed these calculations.  Similar modeling of final-design \dword{dune} \dword{pd} modules will be completed prior to submission of the \dword{tdr}.
...
\forlbnc{LBNC: ``pg 1-62 - is procurement of 288000 \dwords{sipm} a problem, before you even take into account the QA issues?''. A. Vendors indicate that 100,000/year we plan on is okay. Comment added in this section.}
...
\dword{pdsp} has provided essential operational experience with a passive ganging board and signal transport provided by Teflon Ethernet CAT6 cables, as described in Section~\ref{sec:fdsp-pd-assy-frames}.
...
%\fixme{Bob/Vishnu:  referred back to the earlier discussion, with some added text there.  Dave/Vishnu: Is this settled or is there a connector  R\&D plan we can outline?}
...
The \dword{pds} consortium gained extensive experience in manufacturing processes for electronic systems %was gained 
...
\forlbnc{LBNC: ``1-56   Photon Detector Module Assembly.  I got to the end of this and realized that there is no linear description of the process, which says what gets done where, and in what order, how many assembly areas there are, how many shifts are operating etc etc..'' This section has been added to address this.}
...
Following assembly, these components and their associated \dword{qc} documentation will be shipped to UNICAMP in Brazil for assembly into \dword{pd} modules.
...
\item \dword{apa} support rails and electrical connectors:  Stainless steel rails and associated hardware for supporting the PD modules inside the \dword{apa} will be fabricated by local vendors under the supervision of UNICAMP.  Components for cable connection between the upper and lower APAs, and cable management pieces, will be procured from vendors, assembled and tested at UNICAMP.  
...
\item Cables:  Materials for all cables and connectors (both inside and outside the \dword{apa} frames) will be purchased by UNICAMP, and assembled and tested at their facilities.
