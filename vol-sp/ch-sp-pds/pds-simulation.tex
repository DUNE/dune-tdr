...
{ccc}
{tab:pds-ndk}
{Efficiency for tagging nucleon decay events at the \dword{cpa} (\SI{3.6}{m} from the \dwords{pd}) with the \dword{pds} for a range of light yields (LY) at that position. Also shown is the total collection efficiency required for that light yield with the simulated scattering length, \SI{60}{cm}.)}
