...
%\forlbnc{LBNC: ``1-56   Photon Detector Module Assembly.  I got to the end of this and realized that there is no linear description of the process, which says what gets done where, and in what order, how many assembly areas there are, how many shifts are operating etc etc..''  A subsection is added to make this clear. }
...
%The module mechanical components are the responsibility of UNICAMP.  This includes the FR-4 G-10 components (which will be fabricated in-house at UNICAMP), signal routing circuit boards, module electrical connectors, and other miscellaneous components required to fabricate the modules.  

%\fixme{Dave: Need the institution names.}
...
\forlbnc{LBNC: ``Section 1.8 - not much mention of the scale - are any of the scales of the production here difficult? you mention that things will be procured from a vendor - is there an issue producing so many?''  Addressed here.}
...
\forlbnc{LBNC: ``1-55 4 in addition to vendor certification can these devices be tested by the consortium??'' Addressed here.}
...
%using isopropyl alcohol according to the manufacturer's procedures. % given by the manufacturer. Since the most likely vector for scratching/damaging the coating is dragging contaminated wipes across the surface, new clean lint free wipes are used for each cleaning pass on the surface. Clean filters are then baked for water desorption at 100$^\circ$C for \SI{12}{hours}. 
...
\forlbnc{LBNC: Some More comment on scale of project provided here --  filters, coatings}
...
\forlbnc{LBNC: More comment on scale of project provided here --  WLS plates}
...
To mitigate these difficulties, most of the \dwords{pd} frame components can be water-jet cut.
...
DUNE \dword{spmod} \dword{pd} module assembly will occur at a \dword{pd} assembly facilities at Unicamp.  Assembly procedures are described below.  Final assembly planning for \dword{pd} modules is guided by the experience gained during the assembly of \num{60} \dword{pdsp} \dword{pd} modules. 

%at the Colorado State University assembly facility. 
%Our plan for distribution of the various fabrication and assembly processes to consortium institutions well developed, and outlines in, as is our understanding of available resources and their optimization advances.  
...
\forlbnc{LBNC: ``1-56 8 Who has responsibility? eg for Materials Control.'' Outlined here ...}
...
%\input{generated/req-SP-PDS-env-light-exposure}
...
%\fixme{Dave: spec table ref <--not sure what you want here Anne, we reference the relevant spec with in the table is there a better way to do it? }
...
\forlbnc{LBNC: ``1-57 11 Assembly procedures. Travelers are mentioned, do drafts exist??.'' A. Yes, discussed here.}
...
with a fifth person acting as shift leader.  This labor force will allow for production of 20 \dword{pd} modules per week, meeting our ptoduction requirements.  

The shift leader %is not directly involved in assembly, but rather 
