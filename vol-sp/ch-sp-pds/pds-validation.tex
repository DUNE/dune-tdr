...
\forlbnc{LBNC: Considerable changes to this section. The parts on early proof-of-principle are reduced to a few paragraphs with focus on results that are used later. Other sections are also shortened.}
...
We describe here only the most recent design which demonstrated that 48 Hamamatsu \dwords{mppc} in the baseline design can be ganged together on a single differential output with excellent signal performance, low noise and low power dissipation.
...
\begin{dunefigure}[Time walk from 48 ganged \dshorts{mppc}]
 {fig:fig-pds-gang-4}
...
The devices have undergone numerous temperature cycles during the testing with no deterioration in characteristics. Another sample has undergone more than 50 thermal cycles at CSU with no evidence of mechanical failures.  %The setup at CSU for thermal tests is shown in Figure~\ref{fig:csu-cryocycle}.

\begin{dunefigure}[Dark count rate for FBK NUV-HD-SF \dshorts{sipm}]
 {fig:fbk-dcr}
 {Dark count rate for FBK NUV-HD-SF \dwords{sipm}.}
\includegraphics[height=5.0cm]{pds-fbk-dcr.png}
\end{dunefigure}

%\begin{dunefigure}[Photograph of cryogenic thermal cycling setup.]
% {fig:csu-cryocycle}
% {xxxxxx}
%\includegraphics[height=5.0cm]{pds-csu-cryocycle.jpg}
%\end{dunefigure}
...
%\fixme{Ettore/Bob Need to define the performance metrics better for comparison of the different prototypes.}
...
%%early 2017 
%%%Per Gustavo 12/4/18 
%The cells each contained two of the SensL \dwords{sipm} used in the earlier prototype.  
...
Two double-shift light guide modules were also included in the test, which served as a reference for the \dword{sarapu} results.
%\dwords{sipm} with a total area of \SI{1.44}{cm$^2$}.
...
The measured collection efficiency range for the eight ARAPUCA cells was 0.72\%--0.80\%, with an effective \dword{sarapu} gain of about 4.5 times the photosensor area. These tests demonstrate that the effective area gain is maintained when scaling up of the light collecting area of the cell size by almost an order of magnitude.
%(from \SI{9}{cm$^2$} to  \SI{9}{cm$^2$}).
...
\forlbnc{LBNC: Explanation for the 1.8\% photon collection efficiency is given, except we now refer to the estimate for two configurations used in protoDUNE.}
...
\begin{dunefigure}[Full-scale \dshort{sarapu} array installed in \dshort{pdsp}]{fig:arapuca-protodune}
{Visible in the center of the photograph is the 16-cell \dword{sarapu} array installed in \dword{pdsp}.} 
...
%\textit{\dword{pdsp} Electronic Readout}
...
As described above, for the \dword{sarapu} modules the \dwords{sipm} are passively summed in groups of 12 to produce 12 signal channels per module. The 58 lightguide-style light collectors each have 12 \dwords{sipm}, which are passively summed in groups of three such that each light guide has four signal channels. 
...
%\begin{dunefigure}[\dword{sarapu} raw pulse height for \SI{5}{GeV/c} beam momentum.]{fig:arapuca_chanph}
%{Raw pulse height detected in \dword{sarapu} for each readout channel for \SI{5}{GeV/c} beam momentum. The black histogram shows the event-by-event sum of the peak pulse height from each of the \num{12} \dword{sarapu} cells in a module. The color histograms are the distributions of the same events for each of the \num{12} channels scaled up by a factor of \num{12}. No selection on particle types has been performed.}
%\includegraphics[angle=0,width=0.6\columnwidth]{pds-Arapuca-PHchan.pdf}
%\end{dunefigure}


%%The detector has collected data under many different conditions, including exposure to a range of energies from the EHN1 beam line.  

%The black histogram in Figure \ref{fig:arapuca_chanph} shows the summed response of all \dword{sarapu} cells in the module to beam triggered events in \dword{pdsp}.  The pulse height distribution, scaled up \num{12} times for display, from each of the \num{12} channels are shown by the colored histograms.  Differences between the individual channels and the summed histogram are due to geometric effects, light collection, gain, and other variations.  
%%A detailed analysis of each event will help to understand and predict the response of photon detectors in a large liquid argon detector.
 
%The correction for this response variation has been applied to the graphs in Figure~\ref{fig:arapuca_beamph}.

\begin{itemize}
   \item Figure \ref{fig:protodune-allmodules-7gev} shows the response\footnote{In units of photoelectrons, not corrected for afterpulsing and cross talk.} to tagged electrons (left) and muons (right) with a momentum of \SI{7}{GeV/c} of each of the \dword{pd} modules on the beam side of the cathode; the \dword{sarapu} module is in position APA~3, PD module~4. The observed energy is mostly contained within the first APA width of liquid argon for electrons but is distributed through the whole width of the TPC for the minimum ionizing muons.
...
%They were from runs \num{4584}, \num{4627}, \num{4624} and \num{4598} respectively, collected 
\end{itemize}

%Plots from Dante 19apr19. Ettore likes them!
\begin{dunefigure}[PDS response to \SI{7}{GeV/c} momentum electrons and muons in \dshort{pdsp}]{fig:protodune-allmodules-7gev}
{PDS response (in photoelectrons) to \SI{7}{GeV/c} momentum electrons (left) and muons (right) in \dword{pdsp}.}
...
\begin{dunefigure}[Detected photons in \dshort{sarapu} for several beam  momenta]{fig:arapuca_beamph}
{Integrated charge converted to detected photons summed from all 12 channels in a  \dword{sarapu} module exposed to the beam at beam momenta of \num{1}, \num{3}, \num{5}, and \SI{7}{GeV/c}. No selection on particle types has been performed (top).  Peak position of each distribution vs. beam momentum with a linear fit (bottom).}
...
The \dword{xarapu} is an evolution of the \dword{sarapu} concept that moves the second wavelength-shifter from the  inner surface of the dichroic filter to a wavelength-shifter doped plate that acts as a waveguide. This design change was motivated by  simulations that indicate a significant increase in collection efficiency for this configuration. 
...
The dichroic windows for the prototype are the same size as one of the six windows in a \dword{fd}-design \dword{xarapu} supercell: \SI{10.0}{cm}$\times$\SI{7.8}{cm}. The filter plate was coated with \dword{ptp} by vacuum evaporation (film thickness $\sim$ \SI{400}{${\mu}$g/cm$^2$})  using an in-house deposition system at Unicamp. The adhesion was tested by submerging the coated filter in liquid nitrogen several times. No detachment of the evaporated film was observed.
...
% rjw The source is described in the earlier section.
%that is an alloy of natural uranium and aluminum in the form of a metallic disc of \SI{1}{cm} diameter and \SI{150}{$\mu$m} thickness emitting three alpha lines with energies of  \SI{4.187}{MeV}, \SI{4.464}{MeV} and  \SI{4.759}{MeV}. 
...
%	\includegraphics[height=7cm]{pds-sphe-fit-nolog} \quad
	\includegraphics[height=6cm]{pds-spectrum-fitted-double}
\end{dunefigure}  
...
\forlbnc{LBNC: We anticipate updating this section prior to submission of the final document for LBNC review.  The final document will contain the measurement of the  absolute collection efficiencies of the double sided readout \dword{xarapu} configuration and of the single-sided \dword{sarapu}, allowing for a complete suite of comparisons of detector performance.}
...
%\footnote{In order to use existing \dword{apa} components from \dword{pdsp} as a cost-saving measure, however, slight modifications were made to the \dword{apa} frame resulting in \dword{pd} modules \SI{50}{mm} shorter than final modules will be, requiring slight modifications to the \dword{pd} module design.}

%The photos are nice but not directly germane to the PD tests.
%Figure~\ref{fig:fig-iceberg} shows the ICEBERG TPC. 
%\fixme{Bob:  I added text and modified dates.  Please edit...  Dave: Need to update the ICEBERG schedule.}
...
\begin{itemize}
    \item An initial test run occurred in February-March 2019, incorporating one full-length \dword{sarapu} supercell and one full-length \dword{xarapu} supercell.  Both of these supercells used single-sided readout, allowing comparisons of \dword{sarapu} and \dword{xarapu} performance to summer and fall 2018 \dword{pdsp} measurements.  This run demonstrated that both module prototypes and the cryogenic active ganging circuitry were operational and saw signals from both modules using a \dword{mu2e} front end electronics system modified for use by the \dword{dune} \dword{pd} (in a separate stream from the TPC \dword{daq}). Significantly, no cross-talk between the \dword{pd} and \dword{ce} readout electronics was observed.  Unfortunately, difficulties with the \dword{tpc} \dword{ce} and \dword{hv} systems prevented readout of ionization tracks required to allow direct comparisons between the PD modules, and required the test to end before significant progress was made.
...
%Since the \dword{apa} for the ICEBERG test stand is nearly half the width of a full far-detector \dword{apa} frame, including full-size PD insertion slots (\SI{136}{mm}$\times$\SI{25}{mm}) and electrical connections.  
    ...
    \item A second campaign is planned for early summer 2019, using the same photon detector modules as the first campaign but allowing for comparison of data from the \dword{sarapu} and \dword{xarapu} modules with cosmic ray tracks measured in the \dword{tpc}.  These tracks will facilitate comparisons of \dword{ssp} and \dword{mu2e} readout systems for similar events as indicated by \dword{tpc} tracking, and additional \dword{pd} and \dword{ce} cross-talk checks.

    \item A third campaign is planned for late summer 2019 that will incorporate two \dword{sarapu} and two \dword{xarapu} supercells, partially occluded in the frame due to the limitations in \dword{apa} size mentioned above.  One supercell of each kind will be single-sided and one double-sided, allowing for additional comparisons of \dword{pd} technologies.  This campaign will also demonstrate readout of \dword{mu2e} electronics by the \dword{iceberg} \dword{daq}.
...
\forlbnc{LBNC: We anticipate updating this section with data from the second \dword{iceberg} campaign prior to submission of the final document for LBNC review.}

\subsubsection{SBND}
\label{sec:valid-sbnd}

Eight modified \dword{xarapu} modules will be included as part of the light collection system for the \dword{sbnd} experiment at \dword{fnal}, providing an opportunity for a long-term system test of a significant number of new/modified components.  Each \dword{sbnd} \dword{xarapu} will consist of two dichroic filter plates with dimensions
%\num(100) X \num{78} X num{1.5}mm thick,
\SI{100}{mm}$\times$\SI{78}{mm}$\times$\SI{1.5}{mm},
 that are coated with \dword{ptp} at the UNICAMP vacuum deposition facility.  These filters will be produced by Opto Electronics S.A. in Brazil, a leading potential vendor for the \dword{dune} \dword{pd} modules.  Also, each \dword{sbnd} module will contain an Eljen \dword{wls} plate, 
 with dimensions 
\SI{200}{mm}$\times$\SI{78}{mm}$\times$\SI{4}{mm}.
 %\num(200) X \num{78} X num{4}mm thick.  


The \dword{sbnd} \dword{xarapu} modules will be fabricated in Brazil at UNICAMP, representing the first large-scale fabrication at that site and providing an initial demonstration of the \dword{dune} \dword{pd} consortium assembly plan.

Module assembly for \dword{sbnd} will be complete by October 2019, and they will be installed in the detector beginning in November 2019.  Initial filling and operation will occur in summer 2020, and we expect initial results in fall 2020.  \dword{sbnd} will represent the first large-scale operational testing of these components.

%\fixme{Ana--  we need run dates for SBND}

\begin{dunefigure}[SBND Modules]
 {fig:fig-SBND-modules}
 {Software solid model of two-cell \dword{xarapu} modules for SBND test bed exploded (left) and assembled (right).}
...
\forlbnc{LBNC: In the end, I think we will not have seen the performance of X-Arapucas at scale in a Proto-DUNE module before fabrication commences. What risk does that situation engender?}

Following completion of the initial run of \dword{pdsp} a second test run called \dword{pdsp2} is planned in the same cryostat.  This test will serve as a final validation of all pre-production \dword{spmod} detector designs, verifying their performance and ensuring they perform in concert with no interference.  It is foreseen to replace three complete \dword{apa} assemblies with pre-production modules, allowing for testing of 30 final design \dword{pd} modules.  
...
\begin{itemize}
    ...
    \item Readout of full-scale \dword{xarapu} modules using modified mu2e electronics, including integrating \dword{tpc} and \dword{pd} event matching into the \dword{daq} system
\end{itemize}
...
If the decision has been taken to proceed with our performance enhancement alternates (xenon doping or CPA-mounted reflector foils), they will be tested in \dword{pdsp2} as well.

\subsubsection{Long Term Cryogenic Aging}
\label{sec:valid-longtermaging}

\forlbnc{LBNC: ``As pointed out there are several issues for which there will be no verification of LONGEVITY before installing in DUNE. Are their plans for long term operation of test stands which might aid in diagnostic work should issues arise ten years out??'' This new section added.}
...
%\fixme{Ana/Dave: Need dates.}
    \item SBND will provide a long-term test of many X-ARAPUCA components, such as coated filter plates, \dword{wls} plates, and photosensors.  \dword{sbnd} will run for at least three years, starting in late 2020.  The system will be continuously monitored for gain and response of the detectors, and will provide information regarding aging of FR-4 G-10 structures, photosensors, and coated filter plates, and \dword{wls} plates to be used in the \dword{xarapu} modules.
...
Shown in Table~\ref{tbl:fdsfpdshrink}, the thermal contraction of FR-4 is closely matched with that of stainless steel, simplifying design of the module interface with the \dword{apa} frame.
...
\begin{dunefigure}[\dshort{pds} coating test stand]
 ...
 {\dword{pds} coating test stand. Left: VUV monochromator (foreground) and 2-axis scanning chamber (background) at Syracuse University, USA, currently under-going recommissioning. Right: Draft of 74 L liquid argon cryostat for quality control studies and future detector development.}
...
The \dword{xarapu} wavelength shifting plates are fabricated by the same vendor as the light guide bars utilized in the double-shift \dword{pdsp} modules.  The plates are made with the same transparent matrix material, but have a different wavelength shifting dopant added.  
...
a LAr test stand 
with an alpha source behind a small sample of the \dword{wls} plate to scan the attenuation length of a short sample. Finally, we built a long darkbox with a $\sim\,$~\SI{420}{nm} LED scanning down the length of a full bar to verify its attenuation length and compare to the LAr data. (This formed the basis of the threshold requirements on in-air attenuation length measurement for the \dword{pdsp} batch.) 
...
Although at a longer wavelength than \lar scintillation light, the UV light from the system exercises the full chain of measurement steps initiated by a physics event in the \dword{detmodule}, starting from the wavelength conversion, photon capture in the \dword{xarapu}, photon detection and the \dword{fe} electronics readout.
