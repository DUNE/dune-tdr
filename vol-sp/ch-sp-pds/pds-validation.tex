...
An \dword{sarapu} array during assembly is shown in Figure~\ref{fig:sarapuca_array_prod}; the array installed in \dword{pdsp} is shown in Figure~\ref{fig:arapuca-protodune}. If the \dword{sarapu} cells achieved the same detection efficiency as earlier prototypes (1.8\%), they would give an average light yield of \SI{13}{PE/MeV}, meaning a full \dword{sarapu} module would have an effective area equivalent to a detector with \SI{23}{cm$^2$} active area and 100\% detection efficiency.
