...
Risk 1 in the table addresses the performance specification that the \dword{pd} system detect 0.5 pe/MeV of deposited energy.  The system may not reach this requirement.  Current design validation (Section~\ref{sec:pd-validation}) 
\fixme{check ref} provides firm indication that this specification will be met by the \dword{xarapu}, so a risk level of "L" is assigned to this risk.  Mitigation of this risk is being achieved by:  Allocating enough development resources to the PD to continue developing improved light collection modules, increasing the APA slot size to allow for larger modules, and possibly increasing the number of photosensors per \dword{xarapu} supercell.
