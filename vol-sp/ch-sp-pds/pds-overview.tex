...
To detect scintillation light over a large area in a compact space requires a multi-step process.  First, the \dword{vuv} scintillation photons are converted to longer wavelength by chemical wavelength shifters. These photons are then channelled as efficiently as possible towards much smaller photosensors that produce an electrical signal. Due to the severe space constraints these must be a silicon photosensors with dimensions of just a few millimeters,  rather than a traditional photomultiplier
...
Based on the studies described in Section~\ref{sec:fdsp-pd-simphys}, Table~\ref{tab:specs:SP-PDS} summarizes the \dword{pds} specifications necessary to achieve the DUNE science objectives. 
In this remainder of this chapter we present a design that meets or exceeds the specifications. Section~\ref{sec:fdsp-pd-validation} summarizes an extensive set of prototypes that provide validation of the assumptions used in the design.

\input{generated/req-longtable-SP-PDS}

%We distinguish two levels of specifications for the \dword{pds} necessary to achieve the DUNE science objectives. The top level relates to measurement of physical parameters associated with events of scientific interest such as the event time and the energy of the observed particles. Once these are determined, we can establish the detector hardware performance required to make those measurements, such as the sensitivity to the produced light signal per unit of deposited energy (light yield). 

%Table~\ref{tab:pds-sys-req} summarizes the first level specifications: 
...
%{tab:pds-sys-req}
%{\dword{pds} measurement specifications and goals. 
%%\fixme{replace (Anne)}\fixme{\color{blue}Bob: Waiting on response from Anne on whether this table is entirely supplanted by the autogenerated one and if we refer to the labels for the relevant entries e.g. SP-FD-3 for the light yield.}}
...
\end{dunetable}
%\fixme{\color{blue}Zelimir: add in missing numbers. anne    Email sent to Zelimir 1/12/19 => done by Zelimir}
...
Additional details on the design too specialized for the scope of this \dword{tdr} can be found in the DUNE Single-Phase Photon Detector Design Report~\citedocdb{11644}  
 currently under development by the consortium. (Note to reviewers.  This document is currently a placeholder.  The document is being developed and will be uploaded in late March 2019, prior to the TDR official release).
...
DUNE investigated numerous \dword{pd} light collector module options prior to the formation of the \single \dword{pd} consortium, of which we selected four for further development. 
...
Following the successful design, fabrication, operation and performance of \dword{ssp} readout in \dword{pdsp}, and with initial high-quality beam and cosmic ray day data collected by the \dword{pds}, we are further cost-optimizing the readout electronics. To this end we have adopted a lower-cost solution based on lower sampling rate commercial \dword{asic} chips rather than digitizers based on flash \dwords{adc}. Inspiration for this \dword{fe} comes from the system developed for the Mu2e experiment cosmic ray tagger readout system as described in Section~\ref{sec:electronics}. 

%One of the remaining R\&D efforts will be to 
We plan to further optimize and test the \dword{fe} electronics. %In any case, t
The \dword{pds} signals from inside the cryostat will be transmitted directly  to a digital \dword{fe} outside the cryostat for processing and digitization. Ongoing simulation and testing efforts will inform the optimal sampling rate. %It is envisioned for 
The \dword{fe} is planned to continually digitize the input signals for each channel, and to store waveforms and  event metadata that meet the trigger conditions. In an externally triggered waveform mode, the configured waveform window at the time of the external trigger is also stored. These internally or externally triggered waveforms are transmitted to the \dword{daq} board reader processes for storage. The \dword{daq} system and data storage limitations impose constraints on the data bandwidth, readout rates, and zero suppression. 

%There are also constraints on readout system imposed by requirements are on \dword{s/n} and dark rate. 
Requirements on \dword{s/n} and dark rate also impose constraints on the read out system. 
Single \phel signals from \Ar39 and radiological backgrounds will dictate threshold level adjustments. %s to be adjusted. 
We will configure the photon readout %will be configured 
to trigger on signals from the central trigger and timing systems for a variety of configurable events, e.g., beam events, cosmic
...
%There are a number of optimization and development tasks still to be  being pursued, 
A number of optimization and development tasks are still to be pursued, 
including incorporation and programming of high-performance  \dwords{fpga}, readout system optimization based on ongoing and planned \dword{pdsp} and ICEBERG tests, and improved understanding of sampling rates and dynamic ranges.
...
As described in Section~\ref{tab:pds-ndk}, the performance required for the \dword{pds} to achieve 99\% for tagging nucleon decay events at the furthest point (near the \dword{cpa}) is a light yield of \SI{0.5}{PE/MeV}, corresponding to an effective detection area of \SI{23}{cm$^2$}. The more challenging goal of enabling a calorimetric energy measurement for low-energy events like \dwords{snb} with the \dword{pds} establishes the light yield specification for the system (FD-SP-3 in Table~\ref{tab:specs:SP-PDS}) of \SI{13}{PE/MeV} averaged over the active volume, which corresponds to an effective area of \SI{30}{cm$^2$}/module (Section~\ref{fig:pds-snb-calo}). 

Although full validation of the \dword{spmod} \dword{pds} light collection system is still in progress, initial results on the \dword{xarapu} prototype are very encouraging - the single cell prototype (Section~\ref{sec:xarapuca-unicamp}) has achieved a detection efficiency of $\SI{4.7\pm0.8}{\%}$ corresponding to an equivalent area of about \SI{90}{cm^2} per PD module.  This is substantially higher than the specification.
