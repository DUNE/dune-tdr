...
Neutrino \dword{cpv} and other elements of the DUNE long-baseline neutrino program do not rely, to first order, on the \dword{pds}.  The neutrino beam timing allows full functionality of the \dword{apa}s, and the deep underground location reduces the possibility of in-time background events from cosmic rays and other sources to a negligible level.  By contrast, DUNE’s nucleon decay physics could not be executed without the \dword{pds}.  The inability to establish a clock start time would make it impossible to determine whether a candidate proton decay event were fully contained in the detector volume or associated with objects entering the detector from the outside.  DUNE can detect \dwords{snb} originating within the galaxy without a \dword{pds}.  In this case, the presence of tens to thousands of even \SI{5}{MeV}{ scale events occurring over tens of seconds would provide an unambiguous signal that the TPC could characterize on its own.
...
\fixme{Review Michel electron discussion}
...
\end{dunetable}
\fixme{add in missing numbers. anne}
...
This section outlines the physical design considerations for a \dword{pds} %photon detection system 
...
%from sec n.2
The principal task of the \dword{spmod} \dword{pds} is to measure the \dword{vuv} scintillation light produced by ionizing tracks in the TPC within the geometrical constraints of the \dword{apa} structure. The modular arrangement of the \dword{spmod} TPC calls for a configuration across the width of the cryostat starting with an \dword{apa} plane against one cryostat wall, and following with \dwords{apa} and \dwords{cpa} arranges as follows:  APA-CPA-APA-CPA-APA. This means that the central \dword{apa} will collect charge and see scintillation light from \lar volumes on both sides, whereas those by the wall collect from only one side. 

\begin{dunefigure}[Schematic view of a DUNE \SI{10}{kt} \dword{spmod}]{fig:FarDet-interior-for-pds}
{End-on schematic view of the active argon volume showing the four drift regions and anode-cathode plane ordering of the TPC inside the \dword{spmod}. The three rows of \dword{apa}s are two high and 25 deep.}
\includegraphics[width=0.445\textwidth, trim=0mm 3.7mm 0mm 0mm, clip]{sp-apa-dune-sp-end-view.jpg}\hspace{0.01\textwidth}
\end{dunefigure}

The \dword{pd} system must fit within the innermost wire planes of the \dwords{apa} and be  installable through slots in a (wound) \dword{apa} frame (see Chapter~\ref{ch:fdsp-apa}).  Individual \dword{pd} modules are restricted to a profile of dimensions \SI{2.3}{cm} $\times$ \SI{11.8}{cm} $\times$ \SI{209.7}{cm}.  Ten \dword{pd} modules per \dword{apa} comes to a total of \num{1500}.  Of these, \num{500} are mounted in central \dword{apa} frames and must collect light from both directions, and \num{1000} are mounted in frames  near the vessel walls and collect light from only one direction. Figure~\ref{fig:apa-frame-pds} illustrates the configuration for one \dword{apa}. 

\fixme{(Anne added 12/27)}
\begin{dunefigure}[Position in \dword{apa} frame of \dword{pds} modules]{fig:apa-frame-pds}
{An \dword{apa} frame (on its side) showing the \num{13} separate stainless steel tube sections that bolt together to form a complete frame and the ten \dword{pd} modules (vertical in figure). Notice the five slots on the frame's side (top of figure) that they fit through. The other five slots are on the other side, not shown.}
...
A final but significant consideration for the \dword{pds} design is the presence in the \lartpc of the long-lived cosmogenic radioisotope \Ar39, which has a natural abundance with an activity of approximately \SI{1}{Bq/kg}~\cite{}. The isotope undergoes beta decay %with 
...
\fixme{new spec table; content that follows needs revising. Anne}
...
convert incident \SI{127}{nm} scintillation photons into photons in the visible range (>\SI{400}{nm}) that the compact \dwords{sipm}, in turn,  convert to an electrical signal. The light collector design must optimize the costs of various components of the system while meeting the performance requirements.  %It will use \dwords{sipm}, which provide a commercially available compact solution for photon measurement.  However, the response of these devices, which typically peaks in the visible range (>\SI{400}{nm}), is not well-matched to incident \SI{127}{nm} scintillation photons, so a wavelength shifter or some sort is required.  In addition, e
...
The light collector design aims to maximize the active \dword{vuv}-sensitive area of the \dword{pds} while minimizing the necessary photocathode (\dword{sipm}) coverage. This is detailed in Section~\ref{sec:fdsp-pd-design}. 
%%%%%%%%%%%%%%%%%%%%%
...
\fixme{I moved the "DUNE investigated" text to the validation section and moved the top of the light collector design text  1.2.1 here (anne)}
...
illustrated; the final \dword{detmodule} will contain just \dword{xarapu}.}
...
In addition, the design has the flexibility to accommodate greater demands, such as might be desired for SBN physics, without major changes to the design and that  could be adopted quite late in the final design stages. 
Two wavelength-shifter-coated light guide options were also  investigated in detail over several years. %the concept of 
...
a compact silicon photosensor rather than a traditional photomultiplier performs the final stage of converting a visible wavelength photon into an electrical signal. 
...
\fixme{this seems like way too much information in an overview section (Anne)}
...
The light collector options currently under consideration involve shifting the \SI{127}{nm} \lar scintillation light to 
...
\fixme{ Packaging vs packing is confusing here. Anne}
...
Implementing 
This option% would 
requires good visible light sensitivity from the photon collectors; this is not the case for current implementation of the \dword{xarapu}. Either changing the external \dword{xarapu} wavelength-shifter or leaving some detectors uncoated could enable this capability. %The capability could be incorporated e. 
