...
\fixme{Ettore/Alex - Review Michel electron discussion in the next paragraph}
...
\fixme{(Anne added fig from apa 12/27 and PD module figures) -- replicates ones on later sections.  Dave w. The text in the PD module figure should be larger.  }
\fixme{Anne - how do we move the cell figure so it is centered vertically with respect to the left figure?}
...
\fixme{new spec table; content that follows needs revising. Anne. 1/12/19 rjw  Dave and Alex are working on updating the docDB 6422 spreadsheet}
...
\fixme{The document does not exists yet (it is a placeholder with just a title page); Dave what is the timescale? Should we leave this statement out until it is ready - or at least give a warning so LBNC doesn't get a surprise if they do look it up?}
...
\fixme{Zelimir - following the general preamble in the first paragraph we need a short overview of the baseline electronics here.}
