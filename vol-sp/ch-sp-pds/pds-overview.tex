...
\subsection{Scope}
The scope of the \single \dword{pds}, provided by the DUNE \dword{pds} consortium, includes selecting and procuring materials for, and the fabrication, testing, delivery and installation of light collectors (\dword{xarapu}), silicon photosensors (\dwords{sipm}), electronics, and a calibration and monitoring system. This \dword{tdr} chapter will describe the design, validation, assembly, and \dword{qa}/\dword{qc} testing of the photon detector system for a single \nominalmodsize \dword{dune} \dword{detmodule}. 
...
limits its effectiveness in \dword{dune}, where most events in which such particle ID would be beneficial are multi-particle, but it could be a powerful supplement to the charge measurement in some cases.
...
The structure of the APA, along with the imperative to maximize the active volume of \dword{lar}, precludes the use of traditional large area \dwords{pmt}.  
...
which absorbs \dword{vuv} photons and re-emits them with a spectrum centered around \SI{420}{nm}, close to the wavelength of maximum quantum efficiency for photo-conversion in most commercial photosensors.}.
...
\dword{dune} investigated many \dword{pd} light collector module options before forming the \single \dword{pd} consortium; we selected four for further development. 
...
Several iterations of \dword{sarapu} design were tested in small cryostats (Section~\ref{sec:valid-initial}) and \dword{pdsp} (Section~\ref{sec:valid-pdsp}), establishing the viability of the concept for \dword{dune}.
