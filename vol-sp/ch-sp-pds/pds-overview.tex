...
%\fixme{Ettore: New figure or reference for ArDM needed.}
...
\begin{dunefigure}[Arrangement of APAs in an \dshort{spmod} and position of PD modules in APA frame]{fig:apa-frame-pds}
{Top: End-on schematic view of the active argon volume showing the four drift regions and anode-cathode plane ordering of the TPC inside the \dword{spmod}. The three rows of \dword{apa}s across the width of the \dword{spmod} are two frames high and 25 frames deep. Bottom: Schematic of an \dword{apa} frame (on its side) showing the ten pairs of \dword{pd} module support rails (vertical in figure). Notice the five slots on the frame's side (top of figure) that they fit through. The other five slots are on the frame's other side, at the bottom of the figure.}
...
Two of the designs, \dword{sarapu}\footnote{\textit{Arapuca} is the name of a simple trap for catching birds originally used by the Guarani people of Brazil.} %(\textbf{S}tandard-) 
...
\end{itemize}
The effective area of a \dword{pd} module is another useful figure-of-merit that is defined to be the photon collection efficiency multiplied by the photon collecting area of a \dword{pd} module. 
...
\textit{\dword{sarapu}:} In an \dword{sarapu} cell, enhanced photon trapping is achieved through the
...
{\threed model of \dwords{pd} in the \dword{apa}. The model on the left shows the full width of a one \dword{apa} deep slice of the TPC illustrating the APA-CPA-APA-CPA-APA the configuration. The figure on the right shows a detail of the top far side of the TPC where three photon collector modules deployed in \dword{pdsp} are illustrated.}
...
{Left: Image of full-scale \dword{pd} module.  Right: \dword{pd} module being inserted into an \dword{apa} frame (wires not shown), which is done after the APA assembly is completed.}
ap\includegraphics[width=0.45\textwidth]{pds-pd-module-image}
...
\forlbnc{LBNC: This section reworded.}
...
As described in Section~\ref{sssec:photosensors}, each the electronic signals from a \dword{pd} module is formed from an ensemble of 48 Hamamatsu \dwords{mppc} summed into a single channel by a combination of passive and active ganging.  A cold amplifier is used to adjust the \dword{mppc} output signal level prior to transmitting the signal over $\sim\,$\SI{20}{m} long\footnote{Cable lengths are not uniform across all \dwords{pd}, ranging from \SI{15}{m} at the shortest to SI{27.25}{m} at the longest, averaging approximately \SI{20}{m}.} twisted-pair cables to the input of a \dword{fe} \dwords{adc} outside the cryostat.  The twisted pair cable is impedance-matched to  the receiver amplifiers for the \dwords{adc} to optimize common-mode noise rejection at the input of the front-end digitizer. 

%\fixme{Bob:  The correct average cable length and range are in the new footnote (21m average, 15m to 27.25m).  We need to make sur that is the same number used everywhere}
...
The measured collection efficiency in excess of the specification ensures a safety margin against degradation in performance of the optical components over time and failures of a fraction of inaccessible active components during long term operation of the detector. It also opens broader opportunities for the photon detection system to extend the physics reach of the experiment, perhaps in unanticipated ways.
