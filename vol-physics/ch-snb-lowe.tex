...
\begin{dunefigure}[\dword{es} and CC events]{fig:escc}{Example
 ...
 \includegraphics[width=0.4\textwidth]{PDF_5_12-14MeV.pdf}
 \includegraphics[width=0.4\textwidth]{PDF_11_24-26MeV.pdf}
\end{dunefigure}
...
\begin{dunefigure}[Pointing for full supernova]{fig:fullSN}{Left:
  ...
  \includegraphics[width=0.44\textwidth]{costheta_fullSN_ESCCPDFs_TDR.pdf}
\end{dunefigure}
...
\begin{dunefigure}[Fit to three pinching
  ...
  time-integrated flux.}
	\includegraphics[scale = 0.37]{ComparingFluxModels_Updated.png}
  \end{dunefigure}
...
\begin{dunefigure}[Example of distance
  effect]{fig:exampleAsimovDistance}{Sensitivity regions generated
	...
	\includegraphics[scale = 0.37]{ComparingDifferentSNDistances_Filled_Updated.png}
  \end{dunefigure}
...
%are elastic scattering on electrons as well as \dword{cc} absorption
%of $\nu_e$ on $^{40}$Ar (equation~\ref{eq:nueabs}), which has a
%4.5-MeV energy threshold and a large cross section compared to elastic
%scattering on electrons.
%Furthermore, the \dword{cc} absorption differential cross section (the interaction products track neutrino energy closely) potentially enables precise solar-neutrino spectral measurements.
...
\includegraphics[width=0.6\textwidth]{Fig_Efficiency_Enu_TDR.png}
\end{dunefigure}
...
%However,
%a huge exposure ($>$\SI{500}{\ktyr})%
%would likely be required for observation in liquid argon.  
...
%\item Far Detector Triggering and \dword{daq}:

%The far detector must
...
\spchdaq %Chapters~\ref{ch:sp-daq} 
%and ~\ref{ch:dp-daq}  NOT PROVIDED YET!
describes the DUNE triggering and
DAQ configurations designed to meet these challenges for the \dword{spmod}. The chapter for \dword{dpmod} will be available soon.
\fixme{Anne added this 8/8 to fix broken references. Will need to fix when DP is ready}

% \item Far Detector Event Reconstruction:   
% The far detector must be capable of collecting low energy ($<$\SI{100}{\MeV})  charged-current electron neutrino interactions on $^{40}$Ar nuclei that arrive in a short period of time. The final-state electron (or positron) shall be detected and its energy measured.   An energy threshold of 5 MeV or better is highly desirable; most supernova burst events are expected to have energy depositions in the range 5--50~MeV.
