...
\begin{dunefigure}[Pointing for full supernova]{fig:fullSN}{Left: Log
  ...
  \includegraphics[width=0.44\textwidth]{costheta_fullSN_oldPDFs.pdf}
\end{dunefigure}
...
$\nu_e$CC interaction channel has a weak anisotropy, with intrinsic physics-related
...
transitions are
described by a $\propto (1-\frac{1}{3}\cos\theta)$ angular distribution (modeled in MARLEY).  We have shown
that addition of $\nu_e$CC events does not degrade the
pointing resolution, even without ES vs $\nu_e$CC channel tagging.
...
\begin{dunefigure}[Fit to three pinching
  parameters]{fig:example3params}{Sensitivity regions for the three
    pinched-thermal parameters generated using
    the Asimov study.
  \dword{snowglobes} used a
    smearing matrix assuming a cross section
  ...
  Nakazato, Huedepohl black hole formation models, and Huedepohl
  cooling models.  For the Nakazato parameters (for which there is no
  pinching, corresponding to $\alpha=2.3$), the parameters are
  given directly; for the Huedepohl models, they are fit to a
  time-integrated flux.\fixme{Need citations}}
	\includegraphics[scale = 0.37]{ComparingFluxModels_TDR.png}
  \end{dunefigure}

\begin{dunefigure}[Example of distance
  effect]{fig:exampleAsimovDistance}{Sensitivity regions generated using
