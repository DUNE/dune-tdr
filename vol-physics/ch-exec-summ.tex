...
error, as described in detail in Chapter~\ref{ch:osc}.  Strong evidence ($>3\sigma$) for \dword{cpv} is obtained for favorable values (half of the phase space) of \deltacp after seven years of running, leading to a $>5\sigma$ determination after 12 years.
...
By virtue of its deep underground location, large fiducial 
mass, and excellent event imaging, particle identification and 
...
\dword{pdsp}]{fig:pdsp_dedx_execsum}
...
which indicates a pointing resolution of better than $10^\circ$ that 
can be obtained by analysis of subdominant $\nu$-$e$ elastic scattering 
events within a supernova burst, based on full reconstruction and analysis, but 
assuming ideal event identification capability. The DUNE results can be 
...
\begin{dunefigure}[Supernova direction determination from $\nu-e$ elastic scattering events]{fig:fullSN_execsum}{Left: Log
  ...
  \includegraphics[width=0.52\textwidth]{costheta_SN_llhminbin_finebins.pdf}
\end{dunefigure}
...
Sec.~\ref{sec:exec-sensitiv-results}, Figure~\ref{fig:cpv_staging_execsum}
