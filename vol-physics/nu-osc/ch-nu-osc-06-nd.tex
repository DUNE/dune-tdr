...
The \dword{dune} \dword{nd} system consists of a \dword{lartpc} functionally coupled to a magnetized \dword{mpd}, and a 
\dword{sand}. 
%\dword{3dst}. 
The \dword{nd} hall is located at \dword{fnal} 574 m from the neutrino beam source and 60 m underground. The long dimension of the hall is oriented at 90 degrees with respect to the beam axis to facilitate measurements at both on-axis and off-axis locations with a movable detector system. 
...
Muons with kinetic energy greater than $\sim$1 GeV typically exit the \dword{lar}. An energetic forward-going muon will pass through the \dword{ecal} and into the gaseous \dword{tpc}, where its momentum and charge are reconstructed by curvature. For these events, it is possible to differentiate between $\mu^{+}$ and $\mu^{-}$ event by event. Muons that stop in the \dword{lar} or \dword{ecal} are reconstructed by range. Exiting muons that do not match to the \dword{hpg} \dword{tpc} are not reconstructed, and events with these tracks are rejected from analysis.
