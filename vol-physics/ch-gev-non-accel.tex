...
%The numbering in the decay modes follows the PDG convention.  The primary decay is simulated using a phase-space-decay generator. For bound nucleons, the nuclear environment is simulated as in neutrino scattering. The nucleon is assigned a Fermi momentum and removal energy and it is off the mass shell.  The relativistic Fermi gas (RFG) nuclear model is used with a Bodek-Ritchie  tail to incorporate short range nucleon-nucleon correlations \cite{BRtail}.  For $^{40}$Ar the Fermi momentum has a default value of 242.0 MeV/c with binding energy = 27.0 MeV. Fig \ref{fig:pdcy_m} shows the momentum distribution for a bound proton in $^{40}$Ar. If a bound nucleon decays the remaining nucleus can be in an excited state and will typically de-excite by emitting nuclear fission fragments, nucleons, and photons. At present de-excitation photon emission is simulated only for oxygen (Cherenkov detectors) \cite{geniemanual}. 
...
\todo{Add paragraph on CNN.} Including the CNN score in the BDT analysis gives a 40\% background reduction.
...
\todo{Aaron will provide n->Ke, what else?}
...
\todo{Will add more week 11/12 (waiting on tech note)}
...
\fixme{the expression with ccoeff above is failing to build. Anne}
...
\begin{dunefigure}[Sensitivity to Lorenz and CPT violation with atmospheric neutrinos]{fig:atm}{Sensitivity to Lorenz and CPT violation with atmospheric neutrinos in the nonminimal isotropic SME.}
%\includegraphics[width=0.8\textwidth]{DUNE-atm.eps}
\end{dunefigure}
...
%\includegraphics[width=0.8\textwidth]{DUNE-atm2.eps}
\end{dunefigure}
