...
the dominant decay mode to be \ptoepizero.  Although the LHC did not find any evidence for \dword{susy} at the electroweak scale, as was expected if \dword{susy} were to solve the gauge hierarchy problem in the \dword{sm}, the appeal of a \dword{gut} still remains. In particular, gauge-coupling unification can still be achieved in non-supersymmetric \dword{gut} models by the introduction of one or more intermediate scales (see, for example, \cite{Altarelli:2013aqa}).
