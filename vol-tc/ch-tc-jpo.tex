...
\dword{lbnf-dune} \dword{esh} Manager who reports directly to
the \dword{tcoord} and \dword{ipd} who have responciblity for the
implemenation of the \dword{lbnf-dune} \dword{esh} program.
The on-site \dword{dune} \dword{esh} coordinator oversees the day to
...
from the consortia.  The on-site \dword{dune} \dword{esh} coordinator
and safety officer management reporting chain goes directly to the
\dword{lbnf-dune} \dword{esh} Manager to minimize any conflict
...
\dword{lbnf-dune} \dword{esh} manager functions with an
...
direction of the on-site \dword{esh} coordinator.  The safety officer
...
\begin{itemize}
  ...
  \item {\bf On-call Electricians, Ventilation, Plumbing and Network
    team}--- As the different phases of the \dword{lbnf-dune}
...
\end{dunefigure}
The \dword{uit} %Underground Detector Integration/Installation Team 
management consists
of three levels:
\begin{itemize}
...
It is expected that \dword{sdsd} will arrange for inspection and maintenance of
...
\dword{sdsd} is responsible for badging and access to the leased areas at \dword{surf}. This
