...
\fixme{Orig:  Construction of the \dword{dune} detectors is carried out by
``consortia of collaboration institutions'' who assume responsibility
for detector subsystems.  The consortia plan and
...
\fixme{orig: The \dword{dune} collaboration is pursuing far detector designs based on both
...
subsystems required for the two detector types currently under
...
specific subsystems.  An additional five consortia (HV System, DAQ,
...
\fixme{orig: Management of the consortia is through an overall Consortium Leader
...
\fixme{orig:Consortium responsibilities for subsystem deliverables are in many
...
\fixme{orig: Each consortium is represented on the \dword{dune} \dword{exb} by
...
\fixme{light edits to above pgraph}

\fixme{orig: The \dword{tc} organization supports the work of the consortia and
...
systems) 
\fixme{and beamline?} that house the \dword{dune} \dword{fd} modules. \dword{lbnf} is a \dword{us} DOE
...
\dword{rcoord}) \fixme{and someone from lbnf?}.  The \dword{efig} is responsible for steering the global
project and operates via the consensus.  %In the event that an issue arises for which 
...
The \dword{efig} is supported by a \dword{jpo}, headed by the
...
\label{sec:far_site}
\fixme{Add an initial sentence like:  All the collaboration organizations described in this chapter have responsibilities for for far site integration and installation. }
...
the far site.  SDSD members of the \dword{jpo} core team responsible for
