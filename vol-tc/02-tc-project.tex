...
The \dword{dune} project builds from significant development in
...
construction and operation of the \dword{protodune} detectors. These detectors
use full size \dword{dune} components and processes. The construction of
\dword{protodune} has established the teams, production lines, QA and QC
...
beyond \dword{protodune} to \dword{dune}. Most subsystems have completed \dword{pdr}
or 60\% reviews on the design modifications beyond \dword{protodune} in
advance of the \dword{tdr}. The overall level of design is now at
$\sim$90\%. The breakdown of design maturity level for \dword{dsp} by
...
version deployed in \dword{pdsp} is very close to that for \dword{dsp}
...
successfully in MicroBooNE and \dword{pdsp} and is very mature
(90\%). The front end chip has gone through eight iterations and has
been successfully demonstrated in MicroBooNE and \dword{pdsp}
(90\%). The frontend motherboard has gone through a similar number of
iterations and has been successfully demonstrated in \dword{pdsp}
...
The \dword{dune} WBS at level 1 includes six categories as shown in
...
\dword{dune} \dword{tdr} \dword{dsp} Volume~\volnumbersp\ and
\dword{ddp} Volume~\volnumberdp\ chapters for each consortium. A
...
\dword{mou} will be executed in which all deliverables will be
...
country. The \dword{comfund} is overseen by the Resource Coordinator.
A budget plan that outlines the need of the \dword{tc} will be reviewed and
approved by the \dword{rrb} annually.
...
\dword{dune} scientific goals as described in the \dword{dune}
...
\end{dunetable}
There are another eleven high level specifications with impact on
physics that are owned by the \dword{exb} and another twelve high
...
held by the \dword{exb} are listed in Table~\ref{tab:dunephysicsspecs}.
\begin{dunetable}
  [\dword{dune} physics-related specifications owned by \dword{exb}]
  {p{0.025\textwidth}p{0.06\textwidth}p{0.2\textwidth}p{0.35\textwidth}p{0.15\textwidth}p{0.1\textwidth}}
  ...
  8   & APA & wire angles  & 0$^\circ$ for collection, $\pm$35.7$^\circ$ for induction &  &  \\ \colhline
  ...
  12  & HVS & Cathode HV power supply ripple contribution to system noise & very small compared to intrinsic electronics noise & $<\,\SI{100}{enc}$ &   \\ \colhline
  13  & CE & Front-end peaking time  & optimize vertex resolution & \SI{1}{\micro\second} &  \\ \colhline
  14  & CE & Signal saturation level  & largest signals occur with multiple protons produced in the primary vertex & 500k $e^-$ &  \\ \colhline
  15  & cryo & LAr nitrogen contamination  & optical attenuation length in liquid argon with 50~ppm of N$_2$ contamination is roughly 3~m & $<\,\SI{25}{ppm}$ &  \\ \colhline
  16  & all & Detector dead time  & risk of missing a supernova burst if all operating cryostats are offline & $<\,\SI{0.5}{\%}$ &  \\ \colhline
\end{dunetable}

Lower level detector specifications are held by the consortia and are
...
development of \dword{lar} detectors going back to the 1970's and
especially on the large \dwords{lartpc} ICARUS and
\dword{microboone}. Significant prototyping by both LBNE and LBNO has
furthered the value engineering process, leading to the construction
of the \dword{protodune} detectors. These detectors validate the
\dword{dune} designs and establish that the necessary performance is
...
of \dword{pdsp} has been compiled in DocDB-8255. These lessons have
driven the planning for \dword{dune} and have led to design changes for
\dword{dune}. Lessons learned will continue to be updated through the final
