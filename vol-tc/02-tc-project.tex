...
beyond ProtoDUNE to \dword{dune}. Most subsystems have completed \dword{pdr}
...
$\sim$90\%. The breakdown of design maturity level for \dword{dune}-SP by
subsystem is provided in Table~\ref{tab:designmaturity}. The table
shows the final \dword{dune} design maturity at the time of ProtoDUNE and now
...
version deployed in ProtoDUNE-SP is very close to that for \dword{dune}-SP
...
\begin{dunetable}
  ...
  {tab:designmaturity}
  {\dword{dune}-SP design maturity}
  System & weight & ProtoDUNE & \dword{dune}   \\ \toprowrule
...
A preliminary total \dword{dune} project cost estimate scaled from ProtoDUNE
costs was provided to the Neutrino Cost Group in August 2018. Based on
feedback from that review and updated cost estimates based on \dword{dune}
designs, an updated cost estimate has been developed. A corresponding
project schedule has been developed and discussed in
Section~\ref{sec:fdsp-coord-controls}. Cost estimates are provided for
both single-phase and dual-phase detectors. The sequencing plan is
described in Volume~\ref{vol:exec_summary}. A cost summary is provided
in Table~\ref{tab:cost_L1}.
\begin{dunetable}
  [\dword{dune} Project Cost at WBS level 1]
  {|p{0.05\linewidth}|p{0.3\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|}
  {tab:cost_L1}
