...
is supported directly in this role by the global project \dword{esh}
manager.  A dedicated \dword{dune} \dword{esh} manager sits within
the \dword{tc} organization and coordinates the \dword{dune} safety
program under the direction of the global project \dword{esh} manager.
...
\dword{esh} manager interacts directly with each of the participating
...
the dword{dune} \dword{esh} manager particpates directly in production
...
Follow-up on-site visits by the dword{dune} \dword{esh} manager over
...
the \dword{tc} detector integration team.  Appendix~\label{appD}
...
The \dword{tc} engineering team also works directly with the lead
LBNF/DUNE systems engineer, who heads the \dword{jpo} engineering
...
The \dword{dune} project follows the global change control process
described in Chapter~\label{vl:tc-xxx}.  The decision path for
changes that do not impact the \dword{lbnf} project or LBNF/DUNE
...
For document management, the \dword{dune} project relies on the global
project document management system administrated by the \dword{jpo}
team configuration manager.  \dword{tc} works directly with the global
...
QA manager sits within the \dword{tc} organization and coordinates the
\dword{dune} quality assurance program under the direction of the
global project QA manager.  The \dword{dune} quality assurance program
...
risks associated with overall project management.  The \dword{tcoord}
...
\dword{tcoord} does not have direct control over contingency funds
held by the internal projects of the participating funding agencies.
In cases of identified need, the \dword{tcoord} works directly with
...
discussed at Project Board Meetings and brought to the \dword{dune}
\dword{exb} if they need to be addressed at a higher level.
