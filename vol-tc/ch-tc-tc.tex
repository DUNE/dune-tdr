...
\begin{dunefigure}[\dword{dune} org chart]{fig:DUNE_org}
  ...
  \includegraphics[width=0.9\textwidth]{DUNE_Mgmt_20191001_v2a}
\end{dunefigure}
...
\begin{dunefigure}[\dword{dune} technical coordination org chart]{fig:DUNE_tc}
  {\dword{dune} \dword{tc} organizational chart}
  \includegraphics[width=0.99\textwidth]{TC_OrgChart_v2a}
\end{dunefigure}
...
\dword{surf} sitting within the \dword{lbnf}/\dword{dune} integration 
office (IO) also head the \dword{tc} installation interfaces team.  
...
\begin{dunefigure}[\dword{dune} construction \dword{esh}]{fig:dune_esh_construction}
...
coordinated through the IO under the direction of the \dword{ipd} as 
described in Chapter~\ref{ch:tc-jpo}.  The IO also coordinates efforts 
...
The IO interacts with \dword{dune} \dword{tc} during detector design, 
