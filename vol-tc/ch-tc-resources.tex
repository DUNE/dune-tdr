...
both the \dword{spmod} and the \dword{dpmod}. 
%single-phase (\dword{dsp}) and dual-phase (\dword{ddp})detectors. 
The sequencing plan is described in
Volume~\volnumberexec:~\voltitleexec. A cost summary is provided in
Table~\ref{tab:cost_L1}.
\begin{dunetable}
  [DUNE Project Cost at WBS level 1]
  {|p{0.07\linewidth}|p{0.15\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|}
  {tab:cost_L1}
  {\dword{dune} Project Cost for each \dword{detmodule}  at WBS level 1}.
%  \dword{wbs} & Detector type & Detector \#1 & Detector \#2 & Detector \#3   \\ \toprowrule
 \dword{wbs} & Module type & Module \#1 & Module \#2 & Moduel \#3   \\ \toprowrule
  2.0 & \dword{sp} & &  & \\ \colhline
  3.0 & \dword{dp} & & & \\ \colhline
\end{dunetable}
...
\dword{dune} \dword{tdr} Volume~\volnumbersp\: \voltitlesp and
Volume~\volnumberdp\: \voltitledp for each consortium. A companion
