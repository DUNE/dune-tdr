...
Within each \dword{detmodule}, components from various consortia are assembled
...
interfaces, controlling drawing, dimensions, and configurations. \fixme{control drawings? Not sure how to parse this sentence}
...
alignment section. \fixme{sec ref}
...
interfaces will be finalized. %fixed. 
\fixme{`fixed' is ambiguous}
...
service floor with other components.  Before installing the \dword{detmodule}, a set of cryogenic distribution pipes  
...
%of the cryostat will have corrugations that impede movement. Therefore, 
a temporary service floor. % will be installed.  The floor 
It will be installed, and later removed, in sections. 
\fixme{somehow the prev pgraph didn't flow -felt like it came out of the blue. hence edits. anne}
...
frame. (Note: we have not yet defined this.) \fixme{want a `forlbnc' here?}


For the \dword{spmod}, we define a flat and horizontal %level 
reference plane %is defined that is 
coplanar with the upper \dword{apa} yoke plane; i.e., %Thus,
...
These guidelines are specified in EDMS-2095958\cite{bib:cernedms2095958}. \fixme{add as ref} They are the same as
were followed at \dword{protodune} and developed during extensive testing of
the \dword{apa} wire readout at \dword{bnl} and \dword{protodune}.  Guidelines are included
...
primary interface document between the consortia, but that document must point to it. %it must be pointed to within that document.
...
\begin{enumerate}
 ...
 \begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \dword{qa}/\dword{qc} files, and
