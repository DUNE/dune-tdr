...
inside the \dword{lbnf} \dword{fscf}.  This layered structure allows
...
\dwords{detmodule}.  The integration of the modules is carried out by
the \dword{jpo}/\dword{tc} engineering team.
...
\dword{cpa}s and eight \dword{fc} and \dword{gp}. A total of 25 rows
...
Figure~\ref{fig:dune-dp_overall} shows the overall model of the
\dword{dpmod} with one wall of the cryostat removed to show the
interior components. The detector has 20 rows, all of which are
constructed the same way. Each row comprises four \dwords{crp},
...
\forlbnc{More detailed models of the \dword{dpmod} integration
interfaces will be shown here. Need analogous figures and explanations for Dual Phase}
...
supported on the
same beam are relatively constant since \dword{apa} frames and 
...
{\dword{spmod} interface of cryogenic distribution pipes, service floor, and detector.}
\includegraphics[width=0.8\textwidth]{Interface_lower_mid_apa.pdf}
\end{dunefigure}
\forlbnc{Analogous figures and explanation for Dual Phase will go here.}
...
adherence to the \dword{dune} electrical design rules.
...
must be verified by the \dword{tc} team.  Much of the
...
Consortium technical leads and the \dword{tc} team must sign off
...
\begin{dunefigure}[\dword{edms} file structure]{fig:config_structure}
  ...
  \includegraphics[width=0.95\textwidth]{config_storage_v2}
\end{dunefigure}
...
among consortia and between consortia and \dword{jpo}/\dword{tc}.  The \dword{jpo}/\dword{tc}
team handles such changes through the \dword{tb}. Any change that
