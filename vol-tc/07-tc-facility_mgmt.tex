...
engineers). The teams organized by \dword{tc} are:
\begin{enumerate}
...
initiated, and the \dword{itf}, where the \dword{apa}s are integrated.
...
surface and underground. \surf and the \dword{cmgc} \fixme{The
  abbreviation should be capitalized but not the phrase. ANybody know
  how to fix this???} are responsible for moving the components from
...
Yates headframe, the \dword{lc} is used for both short and
long-term storage and re-packaging before anything is shipped underground.
The function of the \dword{lc} is shown in
...
the \dword{itf} and \surf and for dunnage coming back.  \dword{tpc}
\fixme{The first letter should be capitalized in the previous
  phrase. Anybody know how to fix this???} components that must go to
the \dword{itf} arrive at the \dword{lc}, are logged into the
inventory system, and then are transported to the \dword{itf} as they
are needed. Once the integration is complete \dword{apa} pairs are
...
riggers and equipment operators must still move the \dword{apa}s on
...
to progress underground.  Estimated work force at the \dword{itf} is a
...
integration of the \dword{apa}s to minimize wasted time.
...
handled by the \dword{uit}. The responsibilities of the \dword{uit} is
...
\dword{dune} mechanical tests to confirm designs, particularly if the
design has been modified from \dword{protodune}.  The \dword{nova} Far
...
completed a series of different mounting schemes for the \dword{gp}
on the \dword{fc}.

Phase 1: We will construct an \dword{apa} tower using a steel frame large
...
\begin{dunefigure}[View of the Ash River Phase 1 assembly.]{fig:ashriver1}
  {Phase 1 Trial Assembly \dword{apa} Tower at the NOvA Far Detector Lab in Ash River.}
...
rail system to allow full movement of the \dword{apa} and \dword{cpa} pairs. We would
