...
(e.g. students, postdocs, etc.) and from \dword{tc} (e.g. techs,
engineers, etc.).
\begin{enumerate}
...
\dword{itf} where the integration of the
...
components on the surface and underground. \surf and the \dword{cmgc} are
...
Civil Construction the shaft is controlled by the \dword{cmgc} and all loads
...
After compiling the lessons learned from the consortia and installation team
\dword{protodune} the most common comment was better
...
management must work directly with \dword{lbnf} Logistics team at \surf and
...
\surf/\dword{cmgc} are responsible for moving the TPC materials from the
headframe to the three caverns after that it is the \dword{uit}. The
\dword{uit} responsibility is more complex due to the material
movement involved:
\begin{itemize}
 \item North, Central and South Cavern --- TPC components and
   infrastructure materials can be stored in the available cavern
   space during the construction of detector 1 and 2.  \dword{uit} team will
 ...
 \item Inside the SAS \& Cleanroom --- There are several main tasks, APA
   ...
   pallet jacks.
 \item Inside the Cryostat --- The \dword{uit} installs the DSS and detector
...
and \dword{lbnf} ES\&H to minimize any conflict of interest.  But it is
important to state that all \dword{uit} and consortia have the right to stop
work for any safety issues.

\subsection{Underground Facilities Management}

The management of the \dword{uit} can be seen in Figure~\ref{fig:orgchart_uit}
\begin{dunefigure}[Org Chart for the \dword{uit}.]{fig:orgchart_uit}
  {Org Chart for \dword{dune} \dword{uit}.}
...
\begin{itemize}
  \item Underground Installation Manager --- The \dword{uit} manager oversees
    ...
    with the Logistic Manager and \surf/\dword{cmgc} for organization of cage
...
been modified from \dword{protodune}.  The NOvA Far Detector Lab is managed by
...
installation of the \dword{dune} TPC is possible in a safe and efficient
