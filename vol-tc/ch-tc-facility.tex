...
fully described in~\cite{bib:cernedms2095975}. %EDMS-2095975 
We have planned a separate detector ground, isolated from the rest of the facility, for each of the four detectors.  % The plan is to have a separate detector ground for each of the four detectors which is isolated from the rest of the facility.  
...
are given in~\cite{bib:cernedms2095958}. % EDMS-2095958
...
The \dword{ce} readout dissipates an estimated maximum of 360~W
...
The \dword{pds} electronics is based on the \dword{mu2e} (\dword{daphne}) electronics, from which we  estimate a
total power budget of approximately 6~kW. \dword{dune} plans a slightly higher power budget of
8~kW to account for cable drops and power supply inefficiencies.  The \dword{pds} 
electronics present a significantly lower power load than the
alternative solution, used in \dword{pdsp}.  \dword{pdsp} used \dword{ssp}
modules which would represent a power load of approximately 72~kW per
detector module. \fixme{check edits in prev pgraph. anne}
...
\dword{fscf} will supply the \dword{daq} room with cooling
water, an 18~inch raised floor, lighting, HVAC, dry fire protection
and a dedicated 750~kVA transformer.  \dword{jpo} will be responsible for
