...
\begin{itemize}
\item Cryogenic infrastructure, which includes \dword{lar} and LN$_2$ receiving
  ...
  components in the detector cavern and the \dword{cuc}, and process controls,
  integration and supporting equipment. \fixme{This list has internal punctuation and so needs to use semicolons to separate items in the list. For instance, the way it is now punctuated, I cannot tell if  LN$_2$ is one item in the list or if it is part of a grouping (LN$_2$, GN$_2$ and GAr piping). The last three items (process controls,
  integration and supporting equipment) might all be one complex item with three sub items or they might be individual items.}
\item Proximity cryogenics, which include reliquefaction \fixme{Should this be reliquification?} and
...
\item Internal cryogenics, which include \dword{lar} and GAr distribution
...
Infrastructure (BSI) \fixme{This abbreviation is not in the glossary. Is it necessary? I suggest that you go through the entire volume to locate the abbreviations that are not defined. DDSS comes to mind. Some of these abbreviations are used so little that the abbreviation is not necessary. DDSS is a particular problem because the glossary already contains an abbreviation for DSS, which makes DDSS particularly confusing.} and utilities, such as HVAC systems for
...
\end{enumerate}
 \item Ufer \fixme{Is this an abbreviation? I see that it is sometimes Ufer and sometimes UFER. That should be consistent throughout} ground consisting of metal rebar embedded in concrete floors. The \dword{lbnf}/\dword{dune} Ufer ground system
...
two pairs are reserved for \dword{gps}.
...
The \dword{cuc} contains the \dword{daq} and control
...
The \dword{daq} room will contain a minimum of 52 racks that will be used for
fiber optic cable distribution, networking, \dword{dune} \dword{daq}, and the
requirements of conventional facilities.  A minimum of 48 racks are
reserved exclusively for \dword{daq}.  Conventional facilities supply the \dword{daq}
room with a tap to cooling water, an 18~inch raised floor, lighting,
HVAC, dry fire protection, and 500kVA available power.  \Dword{tcoord} will be responsible for the final outfitting and layout
...
The \dword{dune} experiment will need three surface rooms: a surface
control room, a \dword{daq} surface computer room, and a facilities room.
...
The \dword{daq} consortia require a surface computer room with a
...
The \dword{dune} detector safety system (DDSS) \fixme{As I said before, this particular abbreviation is confusing because of other abbreviations used elsewhere in the document. I strongly recommend simply spelling this out instead of abbreviating it.} functions to protect
...
Reporting Utility System (FIRUS) \fixme{This abbreviation is not in the glossary and possibly should be eliminated as an abbreviation.} and the \dword{dune} slow controls
...
\begin{dunefigure}[Detector Safety System]{fig:dune-DDSS}
  {Detector safety system block diagram.}
  \includegraphics[width=0.85\textwidth]{DSS_Block_Diagram.pdf}
\end{dunefigure}


The detector safety system system will be implemented through hardware interlocks and a \dword{plc}.  Listed below are some of \dword{dune} experimental conditions that would
...
\begin{enumerate}
 ...
 \item A water leak detected near energized equipment in the \dword{daq}
   ...
   report to the detector safety system \dword{plc}, and a decision will be made to either
