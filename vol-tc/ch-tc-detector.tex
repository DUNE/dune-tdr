...
%The following sections contain technical summaries of the
The purpose of this chapter is to provide an overview description of the  \dword{dune} \dword{fd} modules. The \dword{fd} consists of \SI{40}{\kilo\tonne} active
...
\dword{sp} and \dword{dp}. %The purpose of this
%chapter is to provide an overview description of the modules. 
Full
descriptions of the \dword{spmod} and \dword{dpmod}  can be found
in \dword{tdr} \dword{sp} Volume~\volnumbersp\ and \dword{dp} Volume~\volnumberdp{}, respectively.
...
{fig:DUNE_SP_model} 
{A \nominalmodsize DUNE \dword{fd}
    \dword{spmod}, showing alternating \dwords{apa},
    \dwords{cpa}, \dword{fc} and \dwords{gp}, \dword{dss}, cryostat
...
internal dimensions of \cryostatlen %\SI{62}{\meter} 
long, \cryostatwdth %\SI{15.1}{\meter} wide
and \cryostatht %\SI{14}{\meter} 
high.  Four drift volumes are created between
alternating \dword{apa} and \dword{cpa} walls on sides, \dwords{fc} and
\dwords{gp} on top and bottom, and two \endwall{}s.  Each drift volume
is \sptpclen %\SI{58.17}{\meter} 
long, \spmaxdrift %\SI{3.52}{\meter} 
wide and
\tpcheight %\SI{12.01}{\meter} 
...
\num{150} \dword{apa}s in one module. Each \dword{apa} contains
\num{10} \dwords{pd}. Each of the two \dword{cpa} walls consists of an
array of \num{25} wide and \num{1} high individual \dword{cpa}s, each
...
across the drift volume. The drift in the \dword{spmod} is in the horizontal 
direction. The \dword{lar} level is above the top set of \dwords{gp} and just
below the \dword{dss}.
...
As with the \dword{spmod}, each \dword{dpmod} is a \nominalmodsize \dword{lartpc},
contributing to the full \SI{40}{\kilo\tonne} \dword{fd} fiducial
...
Also as with the \dword{spmod}, each \dword{dpmod} is contained in a
cryostat with internal dimensions of \cryostatlen %\SI{62}{\meter} 
long,
\cryostatwdth %\SI{15.1}{\meter} 
wide and \cryostatht %\SI{14}{\meter} 
high. But unlike the
\dword{spmod}, \dword{dpmod} consists of a single drift volume with
a vertical drift direction.

The drift volume is encompassed on the top by an array of \num{80}
\dwords{crp}, on the bottom by cathode planes and on the perimeter by a 
\dwords{fc}. It is \dpmaxdrift %\SI{12}{\meter} 
in height with a
\dpnominaldriftfield %\SI{500}{\volt/\centi\meter} 
gradient. The cathode is held at a
potential of \dptargetdriftvoltneg{}. %$-$\SI{600}{\kilo\volt}. 
The \dword{dp} \dword{pds}
consists of \dpnumpmtch %\num{000} 
\dwords{pmt} at the bottom of the drift volume
and is integrated with the cathode planes.  
\fixme{check number in defs.tex} 
The \dword{lar} level in
\dword{dpmod} is within the \dword{crp}, just above the
...
consisting of a %@D ????
set of two \dpstrippitch\si{mm}-pitch %3.125 mm-pitch 
...
The cryogenic \dword{fe} electronics is installed in the \dwords{sftchimney} on the roof of the cryostat to process the
\dword{lartpc} signals. Each \dword{sftchimney} is coupled to a {utca} crate to
digitize the signals. These crates are connected via optical fiber links to the \dword{daq}
back end. Arrangement of equipment on top of the cryostat is similar to the \dword{spmod}.
...
\begin{dunefigure}[DUNE consortia]{fig:DUNE_consortia}
  {\dword{dune} consortia organization. CL refers to consortium leader and TL refers to technical lead.}
  \includegraphics[width=0.99\textwidth]{DUNE_Consortia_OrgChart}
\end{dunefigure}
...
in Table~\ref{fig:WBS_level2}.  At Level-1 the \dword{wbs} is broken down into 
...
\dword{core} and non-\dword{core} activities and also of the costs associated with one-time 
...
large \dword{lartpc} detectors (\dword{icarus} and \dword{microboone}) and on
substantial development from \dword{lbne} and \dword{lbno}. One of the most important
elements that has significantly advanced the project development is
the successful construction and operation of the \dword{protodune}
...
teams, production lines, \dword{qa} and \dword{qc} processes, installation, operation
and performance of the final \dword{dune} detectors. 
...
level for the \dword{spmod} by subsystem is provided in
...
The \dword{apa} conceptual design was developed in
2010 and prototyped at 40\% scale and again in the \dword{35t}. The version
deployed in \dword{pdsp} is close to that for the \dword{spmod} (85\%). The
\dword{ce} low-noise system design, including feedthroughs,
cables and grounding, was successfully prototyped at large scale in
\dword{microboone} and \dword{pdsp}, and is 90\% mature. The \dword{fe} chip has
gone through eight iterations and was successfully demonstrated in
\dword{microboone} and \dword{pdsp} (90\%). The \dword{femb} has gone
through a similar number of iterations and was successfully
demonstrated in \dword{pdsp} (80\%). The \dword{adc} chip has evolved from a
previous version used in \dword{cmos} 180~nm technology that was tested to $-50\circ$C. (50\%). Key
elements of the \dword{coldata} chip have been prototyped (25\%). The \dword{hv}
design has evolved from \dword{icarus}, \dword{microboone} and the \dword{35t}.  It
has been prototyped in subsequent runs of the \dword{35t} and
demonstrated in \dword{pdsp} (80\%). The \dword{pds}  %ARAPUCA 
 \dword{xarapu} design
has been prototyped at small scale and in \dword{pdsp} (20\%). The
mechanical design has been extensively developed using the \dword{35t}
detector and \dword{pdsp} (85\%). The \dword{daq} \dword{artdaq} back end has been
developed in several experiments, including the \dword{35t} and
\dword{pdsp}. The \dword{daq} \dword{felix} \dword{fe} has been developed by \dword{atlas} and
...
The design maturity of the \dword{dp}  detector technology is also quite
...
benchmark is the operation of the \dword{wa105} %$3\times1\times1$ demonstrator 
at
\dword{cern}. A critical test will be the operation of \dword{pddp} at \dword{cern}. A
similar table of design maturity for the \dword{dpmod} will be included
here.
