...
operating \dword{dune}. \fixme{redundant} Accidents and injuries are preventable, and
...
The \dword{tcoord} and \dword{ipd} have \fixme{share?} responsibility for
...
project.  An environmental assessment \fixme{clarify diff between `evaluation' and `assessment' - it sounds like the same thing} was prepared %on 
that identifies the
...
\fixme{prev pgraph feels repetitive}
...
in \dword{feshm} Chapter 2110, \fixme{check bib} to establish code equivalency between
...
\end{itemize}\fixme{check 4th item }
...
found i  the \dword{esh}Q Section Document Database DocDB-3303\footnote{\url{https://esh-docdbcert.fnal.gov/cgi-bin/cert/ShowDocument?docid=3303}}. \fixme{not in our bib}
...
Personnel shall wear the following \dword{ppe} when on site the \dword{surf} site.
\begin{itemize}
\item At a minimum, all workers personnel shall wear
  ...
  \item Hard hats must meet the ANSI Z89.1 standard as defined by 29
    CFR 1926.100 and bear the Z89-.1 designation. \fixme{ANSI not in glossary}
...
\fixme{prev pgraph could be cleaned up. The goal of planning is to`ensure' safety, etc., but planning itself promotes/encourages, it but doesn't actually ensure} 

A work planning meeting will be held %each day/shift 
before each shift, prior to the
start of the shift's daily work activities. \fixme{check my edit - and question for rest of pgraph - is it once per day or once per shift? Not clear.} The meeting(s) %should be 
are led by the
...
Department before arriving at site.  SDS documentation should \fixme{must?} be
...
emergency until the ERT arrives. \fixme{check edits in prev pgraph}
...
\dword{lbnf-dune} \dword{esh} coordinator. \fixme{the doc is collected?} The
\dword{lbnf-dune} \dword{esh} coordinator will evaluate
appropriate storage cabinets for the \dword{ipd}. \fixme{for the PI-DIR? Awkward}
...
\dword{lbnf-dune} for evacuation.  \fixme{based on the results?} All caverns will be equipped with
...
the drifts are classified as  \dword{odh} 0. \fixme{which corresponds to what?} The caverns will be equipped with
...
the worker %workers 
should \fixme{will be trained to? expected to? must? } %recognize that the work activity in which they are engaged should be stopped 
stop the activity and notify their supervisor and \dword{dune} \dword{esh} coordinator of
...
site. During the implementation of these programs, \fixme{new?} lessons learned will
