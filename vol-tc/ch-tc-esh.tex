...
Access Control Program. \fixme{Should we define \dword{surf} Site
  Access Control Program at the end of the previous paragraph? - does
  it define the \dword{surf} identification badge?} All personnel are
...
presentation. \fixme{Why FRA and not FNAL ESH section, \dword{sdsd} or LBNF/DUNE ESH Manager?}
...
\item At a minimum, all Subcontractor \fixme{Why only Subcontractors?
  ...
  performing \fixme{non-office?} work at \dword{surf}. 
  \item All personnel entering the work site \fixme{please define
    ...
    \dword{lbnf}/\dword{dune} \dword{esh} Manager and noted in the
    activity-specific HA.\fixme{Is the activity-specific HA part of
      the TAP? or how do they relate?}
  \item When working underground, a site-approved \fixme{What does ``site-approved'' mean? Will these be provided by ESH? or how will purchase of appropriate one be made?} Self Rescuer (e.g.,
   ...
   \item Hard hats shall meet the ANSI Z89.1 standard as defined by 29
     ...
     Z89.2 standards and bear the {\em Z89.2} designation. \fixme{This is presumably only for 'energized'' HV work, correct? Is HV defined as $>$50V?}
    ...
    \item Hearing protection appropriate to the work environment, as
      defined in the hazard analysis.\fixme{Which HA?}
    \item Any specialized PPE required for specific work tasks as
      defined in the hazard analysis. \fixme{Which HA? Where are the HA's stored?}
\end{itemize}
...
Medical Clinic for treatment.\fixme{Who will send them? by what means?
  Should there be a discussion about how long to get an elevator ride
  up the shaft?} The supervisor shall complete an initial Incident
...
at frequent and regular intervals. \fixme{by whom?} Containers shall be provided for
...
be stored and disposed of separately.\fixme{Who makes this assessment? who monitors?}

Chemicals and hazardous materials must be in proper storage cabinets
with SDS documentation readily available.\fixme{SDS documentation is from Safety Coordinator? who provides ``proper storage cabinets''? who defines ``proper''}
...
Competent Person \fixme{who is ``competent''?} shall identify the
