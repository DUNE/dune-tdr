...
acceptable. Individual activity based \dwords{ha} will be
developed for each work \dword{lbnf}/\dword{dune} activity at
\dword{surf}.
\begin{dunetable}
...
to perform work.  The \dword{lbnf}/\dword{dune} \dword{esh} management
...
\item At a minimum, all workers personnel shall wear
  ...
  \item When working underground, all personnel will carry an Ocenco M20.2 self rescuer
    and a hard hat cap lamp. In addition, Ocenco 7.5 emergency breathing apparatus device
...
separately as assessed by the \dword{lbnf}/\dword{dune} \dword{esh}
coordinator.
...
\dword{lbnf}/\dword{dune} \dword{esh} coordinator. The
\dword{lbnf}/\dword{dune} \dword{esh} coordinator will evaluate
