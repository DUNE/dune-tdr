...
Coordinator reports to the \dword{lbnf-dune} \dword{esh} manager and
has primary responsibility for \dword{esh} support and oversight of
the \dword{dune} \dword{esh} program for all activities at
\dword{dune} activities.  The far and near site \dword{esh}
Coordinators are responsible for providing support and oversight
for all installation activities at the \dword{surf} and\dword{fnal} sites.
...
electrically or by diesel fuel, no other fuels are allowed (Gasoline
or propane powered equipment is not allowed in the underground
facility). Diesel is allowed due the large ventillation rate in the
underground area.
