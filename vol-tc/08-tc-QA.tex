...
\dword{dune} \dword{tc} monitors the technical contributions from
...
The primary objective of the \dword{lbnf}/\dword{dune} Quality Assurance (QA) program
...
public, and the environment. The \dword{qap} aligns the \dword{lbnf}/\dword{dune} QA
...
for the \dword{lbnf}/\dword{dune} \dword{qap}.

The \dword{lbnf}/\dword{dune} \dword{qap} outlines the QA requirements for all \dword{lbnf}/\dword{dune}
...
approach. This \dword{qap} is implemented by the development of quality
plans, procedures and guides by the Consortia to accommodate those
specific quality requirements.

\subsection{Scope}

The \dword{lbnf}/\dword{dune} \dword{qap} provides Quality Assurance requirements
...
\dword{lbnf}/\dword{dune} \dword{qap}. Oversight of the work of the Consortia will be
the responsibility of the \dword{dune} Technical Coordinator and \dword{lbnf}/\dword{dune} \dword{qam}.

\subsection{Graded Approach}

A key element of the \dword{lbnf}/\dword{dune} \dword{qap} is the concept of Graded
...
that provide the tier structure for the flow down of \dwords{qap}, with
the \dword{lbnf}/\dword{dune} \dword{qap} as the top tier.

Specific \dwords{qap} shall be developed by the Consortia with the
assistance of the \dword{lbnf}/\dword{dune} \dword{qam} for
...
\dword{lbnf}/\dword{dune} \dword{qap} for their scope of work. In case of conflict between
sets of QA requirements, \dword{dune} Project Management will provide
resolution.

With many institutions carrying responsibility for various aspects of
the Project, institutional \dwords{qap} will be reviewed by the \dword{dune}
Project to ensure compliance with the \dword{lbnf}/\dword{dune} \dword{qap}. Using a
...
the responsibility of the \dword{dune} Technical Coordinator.

\subsection{Responsibility for Project Management}

The \dword{dune} Consortia Leaders manage their Projects and are responsible
for achieving performance goals. The \dword{lbnf}/\dword{dune} \dword{qam} is responsible for
ensuring that a quality system is established, implemented, and
maintained in accordance with requirements. The \dword{lbnf}/\dword{dune} \dword{qam} reports
to the \dword{dune} Technical Coordinator and provides oversight and support
to the Consortia Leaders to ensure a consistent quality program.

The \dword{dune} Consortia Leaders are responsible for quality within their
Project and report their Quality Assurance issues to the \dword{dune}
Technical Coordinator and \dword{lbnf}/\dword{dune} \dword{qam}. \dword{dune} Consortia Leaders
may designate Quality Assurance Representatives (QAR) within their
organization to perform some of their work defined in the \dword{lbnf}/\dword{dune}
\dword{qap}. The \dword{dune} Consortia Leader shall retain overall
responsibility for these activities even though they have designated a
QAR.

The \dword{dune}-US Project Manager is appointed by the \dword{dune}-US Project
Director and runs the \dword{dune}-US Project Office. The \dword{dune}-US Project
Manager, in coordination with the \dword{dune}-US Project Director, is
responsible and accountable for the day-to-day management and
execution of the project. For the responsibilities of the \dword{dune}
Technical Director that are defined in the \dword{lbnf}/\dword{dune} \dword{qap}, the
\dword{dune}-US Project Manager will retain the same responsibilities for
\dword{dune}-US activities. The \dword{dune}-US Project Manager is responsible for
coordinating the activities of the \dword{dune}-US L2 Project Managers. The
\dword{dune}-US L2 Project Managers will have the same responsibilities as the
\dword{dune} Consortia Leaders within the \dword{lbnf}/\dword{dune} \dword{qap}.

\subsection{Levels of Authority and Interface}

The \dword{dune} Management Plan, the \dword{lbnf}/\dword{dune} PMP, and the \dword{lbnf}/\dword{dune} \dword{qap}
define the responsibility, authority, and
interrelation of personnel who manage, perform, and verify work that
affects quality. The \dword{qap} defines the QA roles and responsibilities of
these management and working levels of the \dword{dune} Project.
...
affect quality of \dword{dune} products to their respective \dword{dune} Consortia
Leader and the \dword{lbnf}/\dword{dune} \dword{qam}. The Consortia Leader responsible
...
The \dword{lbnf}/\dword{dune} \dword{qam} is responsible for periodically reporting on
the performance of the quality system to the \dword{dune} Technical
Coordinator for their review and as a basis for improvement of the
quality system. The \dword{dune} Technical Coordinator may call for Quality
Assurance Program readiness assessments as the Project nears major
milestones. The \dword{dune} Technical Coordinator, Consortia Leaders, and
\dword{lbnf}/\dword{dune} \dword{qam} are all responsible for providing the resources
...
The \dword{lbnf}/\dword{dune} QA Manager may request personnel from the \dword{dune} Project
Teams to act on behalf of the \dword{lbnf}/\dword{dune} \dword{qam} to perform quality
...
\dword{lbnf}/\dword{dune} QA Manager and the \dword{lbnf}/\dword{dune} \dword{qam} will periodically
report these non-conformances to \dword{dune} Project Management.
...
To promote continuous improvement, the \dword{dune} Project will develop a
...
In addition, the \dword{lbnf}/\dword{dune} \dword{qam} will periodically publish a best
practices and lessons learned report. Lessons learned from the \dword{dune}
Project will be screened for applicability to other organizations. The
\dword{dune} project will periodically check external lessons learned sources
for applicability to the \dword{dune} Project. Sources of lessons learned
include the \dword{doe} Lessons Learned List Server, the \fnal ESH\&Q
Lessons Learned Database, and \dword{dune} team members who participate in
peer reviews of other Projects Reviews of the \dword{dune} Project serve as
...
designee. The \dword{dune} Project Offices manage all documents under
a document control system as identified in the \dword{dune}
...
\dword{lbnf}/\dword{dune} \dword{qam} review and signature. Project
documents that contain quality requirements shall be reviewed by the
\dword{lbnf}/\dword{dune} \dword{qam}.
...
and methods of cost and schedule change control. \dword{lbnf}/\dword{dune} will follow
the guidelines for storing and maintaining records for the Project in
accordance with \fnal Records
Management(http://ccd.fnal.gov/records). \dword{dune} Technical Coordinator,
\dword{lbnf}/\dword{dune} QA Manager and Consortia Leaders are responsible for
...
Fabrication work on the \dword{dune} Projects shall be performed to
...
members and the \dword{dune} Technical Coordinator shall exercise a degree of
care commensurate with the work and the associated hazards. See the
\dword{lbnf}/\dword{dune} \dword{ieshp} for more details on \dword{lbnf}/\dword{dune}
integrated safety management systems.

\subsection{Change-Controlled Work Processes}

Change-controlled work processes are those for which the \dword{dune} Change
...
effective. The \dword{lbnf}/\dword{dune} Configuration Management Plan defines the
change control process in detail.

\section{Design}

The \dword{dune} design process provides appropriate control of design inputs
and design products. The primary design inputs are the \dword{dune}
...
The \dword{lbnf}/\dword{dune} Systems Engineering website documentation defines the
...
design of \dword{dune} by breaking their work down into sets of engineering
...
Consortia Leaders and the \dword{lbnf}/\dword{dune} QA Manager to determine QA
inspection criteria of fabricated products and installations. Close
coordination must be made with \dword{dune} scientists to assure the
engineering satisfies the scientific requirements of the
experiment. Configuration Management as documented in the \dword{lbnf}/\dword{dune}
Configuration Management Plan will be systematically implemented for
\dword{dune}. Final Design work sets the final Quality Assurance parameters
...
The \dword{dune} Review Plan describes the design reviews recommended for its
...
systems or components shall be identified to the \dword{lbnf}/\dword{dune} Systems
Engineering team.
...
\dword{lbnf}/\dword{dune} \dword{qap}.  The Consortia members requesting
...
\begin{itemize}
 ...
 \item \dword{dune} intention to perform acceptance sampling in lieu of full inspection and test item acceptance
\end{itemize}
...
interval. The \dword{lbnf}/\dword{dune} QA Manager shall oversee and support the \dword{dune}
...
The \dword{lbnf}/\dword{dune} \dword{qam} will plan reviews as independent assessments
to assist the \dword{dune} Technical Coordinator in identifying opportunities
for quality/performance-based improvement and to ensure compliance
with specified requirements. Independent assessments of the \dword{dune}
Projects can be requested by \dword{dune} management. Independent assessments
...
management. The \dword{dune} Project uses a formal process for assigning
...
Neutrino Committee (LBNC) to advise it and \dword{dune} Management. The role
of this standing committee is described in the \dword{lbnf}/\dword{dune} PMP. The \dword{doe}
...
the control of \dword{dune}, they are not necessarily considered a part of the
independent assessment criterion. However, \dword{dune} management considers
