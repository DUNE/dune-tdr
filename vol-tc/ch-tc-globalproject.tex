...
systems) that house the \dword{dune} \dword{fd} modules. \dword{lbnf} is a
...
also serves as the \dword{fnal} Deputy Director for \dword{lbnf}.  The \dword{dune}
project is responsible for providing all components required for
fabricating the \dwords{detmodule}. \dword{dune} is an international project
with some \dword{doe} contributions that is directed by \dword{dune}
collaboration management. The organization of the \dword{lbnf-dune} 
...
integration and installation of the \dwords{detmodule} and supporting
...
takes direct responsibility for design and construction of the \dword{fd} 
...
conventional  facilities for the detector (up to the point of beneficial  % sm chg by anne 4/18
...
coordination between the \dword{lbnf} and \dword{dune} projects.  
 \dword{lbnf} Project Director and
the \dword{ipd} co-chair the dword{efig}.  \dword{efig} leadership also incorporates the four
members of the \dword{dune} collaboration management team
(co-spokespersons, \dword{tcoord}, and \dword{rcoord}).  The
\dword{efig} is responsible for steering \dword{lbnf-dune} and
...
the issue is passed to the \dword{fnal} Director.
...
\end{dunefigure}
\fixme{figure is blurry. Also, should B. Miller be W. Miller? anne}
...
\dword{jpo} functions include \dword{lbnf-dune} configuration and integration,
...
\dword{cf} subproject delivers beneficial occupancy of
...
\dword{jpo} functions associated with its \dword{lbnf-dune} coordination role
...
directs separate safety teams responsible for implementing \dword{lbnf-dune} \dword{esh} program within both the \dword{lbnf} and
...
\dword{esh} manager works directly with the \dword{fnal} and \dword{surf} safety
...
infrastructure and the \dword{cf} that house them.  The
...
that hosts the \dword{daq} electronics.  It also oversees the design of the
...
The \dword{lbnf-dune} partners have agreed to adopt the formal change
...
\dword{lbnf-dune} change control process is described in~\citedocdb{82}. %\dword{dune} DocDB-82.
...
project changes are documented in detail and approved %signed-off on 
by the
...
and managing the \dword{lbnf-dune} document management system, which will
be hosted in \dword{edms}.  All technical documents and drawings will be
...
infrastructure) through construction, assembly, testing, %sm chg by anne
transport and installation.  The \dword{lbnf-dune} \dword{qa} manager who
...
\dword{lbnf-dune} schedule incorporating all \dword{lbnf} and
\dword{dune} project activities in addition to the
\dword{lbnf}/\dword{dune} integration and installation activities at
\dword{surf} with their correct interdependencies in order to track
the overall progress of the \dword{lbnf-dune}.  The project partners have
agreed that the \dword{lbnf-dune} schedule will be maintained within the same
\dword{p6} framework used to plan and status the resource-loaded
schedule of activities associated with the USA contributions to
\dword{lbnf} and \dword{dune}.  Activities that fall under the
responsibility of other international partners are included and
directly linked within the \dword{p6} schedule, but do not incorporate
the associated resource information required for USA activities.  The
non-USA activities are not tracked using the formal \dword{evms}
...
To achieve coherency in the review process across the \dword{lbnf-dune}
...
\dwords{orr}.  Central coordination through the
...
\dword{dune}, there will be a single \dword{mou} 
detailing the contributions of all participating partners.  The \dword{mou} 
...
obtain the appropriate partner approvals %sign-offs 
on each.  The financial
reporting required for partners to track \dword{comfund} contributions
will also be organized through the \dword{jpo}.

\section{ProtoDUNE Experience}
\label{sec:dune_protodune}

The structure of \dword{lbnf-dune} is based heavily on the
organization that successfully executed the construction,
installation, commissioning and operation of the \dword{protodune}
detectors at \dword{cern}.  The on-site team at \dword{cern} responsible for the
...
applied in creating the \dword{lbnf-dune} organization for the
\dword{dune} \dword{fd}.  The integration of installation safety
...
installation of some \dword{pdsp} components stemming from the absence
of a %sm chg anne coordinated approach in the review process for these
items.  The creation of a \dword{jpo} team with responsibility for
organizing a coherent review process addresses this issue directly.
In general, the successful implementation of the \dword{protodune}
