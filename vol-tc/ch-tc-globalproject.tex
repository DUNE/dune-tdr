...
organization of the \dword{lbnf-dune}, which encompasses 
...
\begin{dunefigure}[Global project organization]{fig:DUNE_global}
  ...
  \includegraphics[width=0.8\textwidth]{fs-integration-org-chart} %{HighLevel_OrgChart_v3}
\end{dunefigure}
...
caverns is managed as a separate element of \dword{lbnf-dune} under
the responsibility of the \dword{ipd}, who is appointed by and reports
...
\dword{dune} \dword{exb}, respectively. \fixme{Can we add here?
  \dword{fnal} has created the \dword{sdsd} to provide laboratory
  support services. \dword{sdsd} works closely with \dword{surf} to
  insure the appropriate infrastructure for safe operations.}
...
at the \dword{surf}, the \dword{dune} consortia maintain responsibility 
...
\dword{lbnf}/\dword{dune}  Installation (IO) 
project teams.
\begin{dunefigure}[JPO organization chart]{fig:DUNE_jpo}
  {\dword{jpo} organization chart}
  \includegraphics[width=0.85\textwidth]{JPO_OrgChart_v3}
\end{dunefigure}
...
and \dword{dune} projects as well as the \dword{lbnf}/\dword{dune}
 installation activities at \dword{surf}.  The 
\dword{lbnf-dune} \dword{esh} manager works directly with the \dword{fnal} 
...
\dword{jpo} team to ensure proper  installation 
...
change control process is described in DocDB-82\citedocdb{82}.  The 
...
\dword{lbnf} and \dword{dune} activities together with the 
 installation activities at \dword{surf} 
...
schedule will be managed within the same Primavera \dword{p6} framework 
...
\dword{jpo}.  \dword{dune} collaboration management via the
\dword{tcoord} has direct responsibility for design and production
reviews focusing on the different detector elements.  Similarly,
\dword{lbnf} project management has responsibility for design and
production reviews covering the supporting infrastructure pieces
within its scope.  Installation and \dwords{orr}, on the other hand,
are the responsibility of the \dword{ipd}.  Central coordination of
the review process through the \dword{jpo} ensures that issues related
...
operations reviews with detailed hazard analyses.  The \dword{jpo} team
