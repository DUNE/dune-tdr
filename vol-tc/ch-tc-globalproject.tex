...
of international partners including the U.S. \dword{doe}.  The 
global organization of the \dword{lbnf-dune}, which encompasses 
...
The \dword{efig} is responsible for steering \dword{lbnf-dune} 
...
properly coordinated across the entire enterprise.  Members 
of the \dword{jpo} organization are drawn directly from the 
project teams supporting \dword{lbnf} and \dword{dune} as well 
as the team supporting the overall integration effort.  
Figure~\ref{fig:DUNE_jpo} shows thecurrent organizational 
structure of the \dword{jpo}, indicating the members being 
pulled in from the \dword{lbnf} (LBNF), \dword{dune} (TC) and 
\dword{lbnf}/\dword{dune} Integration (NP) project teams.
...
the time when the \dword{lbnf} \dword{cf} subproject delivers 
beneficial occupancy of the underground detector caverns at 
\dword{surf}, the \dword{jpo} will evolve to incorporate the 
stand-alone organization that takes responsibility for the 
...
Some members of the current \dword{lbnf} and \dword{dune} project 
...
and installation activities at \dword{surf} is described in 
...
there is a single Project \dword{esh} manager who reports directly 
...
organizations.  For example, the Project \dword{esh} manger 
...
infrastructure and the \dword{cf} that house them.  The team 
builds and maintains a full \threed CAD model of everything in 
...
to the individual \dword{lbnf} and \dword{dune} design teams 
to validate that their design modifications have been properly 
incorporated within the global model.  After receiving the
appropriate sign-offs from all parties, the \dword{jpo} team tags 
...
The configuration manager is also responsible for administrating
...
project partners have agreed that the \dword{lbnf-dune} 
schedule will be managed within the same \dword{p6} framework 
used to plan and status the resource-loaded schedule of 
activities required for U.S. \dword{doe} contributions to 
\dword{lbnf} and \dword{dune}.  Activities falling under the 
responsibility of other international partners are included 
and directly linked within the \dword{p6} schedule, but do 
not incorporate associated resource information required for 
U.S. \dword{doe} activities.  The non-\dword{doe} activities 
...
through the \dword{jpo}.  \dword{dune} Collaboration 
management via the \dword{tcoord} has direct responsibility
for design and production reviews focusing on the different 
detector elements.  Similarly,  \dword{lbnf} Project 
management has responsibility for design and production 
reviews covering the supporting infrastructure peices  
within its scope.  Installation and \dwords{orr}, on the 
other hand, are the responsibility of the \dword{ipd}. 
Central coordination of the review process through the
\dword{jpo} ensures that issues related to installation 
...
structural analysis.  The \dword{jpo} team also takes 
...
the case of \dword{dune}, there will be a single \dword{mou} 
...
partner approvals on each.  The financial reporting required 
by partners to track \dword{comfund} contributions will also be 
organized through the \dword{jpo}.
...
detectors at \dword{cern}.  The on-site team at \dword{cern} 
