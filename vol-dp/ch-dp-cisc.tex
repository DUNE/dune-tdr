...
The \dword{cisc} system must head off fault conditions that could develop in the \dword{detmodule} over long periods of operation.
...
\begin{dunefigure}[\dshort{cfd} example]{fig:cfd-example}
  {Distribution of temperature on a plane intersecting ... \fixme{describe new figure location and orientation here}.}
...
\begin{dunefigure}[Arrays in gas phase]{fig:cisc-tsensor-gas}
 ...
 \includegraphics[width=0.8\textwidth,angle=0]{cisc_dp_tsensor_gas.png}
\end{dunefigure}
...
In addition to these level meters, \dword{crp} will use high precision (less than 1~mm) capacitive level meters attached to the \dword{crp} frame to monitor constant \dword{crp} alignment to the liquid phase. These fall under the scope of the \dword{crp} consortium and are described in chapter 4 of this Volume.
