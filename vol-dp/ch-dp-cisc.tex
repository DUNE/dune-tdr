...
%
% I replaced the followings sentence with a new one that gives the reader a better idea where to find the differences between DP and SP chapters. -Glenn
% The \dword{dpmod} uses the same control system as the \dword{spmod}, as well as all \dword{sp} cryogenic instrumentation. It also includes dedicated instrumentation for monitoring temperature and pressure in the gas phase. The gaseous argon instrumentation includes pressure meters and a vertical array of thermometers.
% I removed the following line to avoid circular reference. -Glenn
% A detailed description of instrumentation used in the \dword{spmod} can be found in the corresponding chapter of Volume~3 of this \dword{tdr}.
%
...
\dword{dpmod} which is unnecessary in the \dword{spmod}.  The
corresponding chapter of Volume~3 of this \dword{tdr} is
...
% {Tim's text starts here}
Each element of \dword{cisc} contributes to the DUNE physics program primarily by maintaining high detector live time.  As described in Volume~2 of this \dword{tdr}, neutrino \dword{cpv} and resolution of the neutrino mass hierarchy over the full range of possible neutrino oscillation parameters will require at least a decade of running the \dword{fd}.  Similar requirements apply to searches for nucleon decay and \dword{snb} events from within our galaxy.  Throughout this long run time, the interior of any DUNE cryostat remains completely inaccessible.  Repairs cannot be made to any damaged components within the \dword{tpc} structure; hence, environmental conditions that present risks must be detected and reported quickly and reliably. 
...
% \fixme{An illustration showing where all these instruments go would be helpful. Anne. --- Agreed, but locations are not defined yet for all devices, can't be completed by deadline.}
...
% All rephrasing comments in this fixme have been implemented. Changing column justificatin to \raggedright is a table style suggestion that would have to be implemented in the table generation templates.
...
% \fixme{DP "3rd draft" or supplement: Figure 1.2 and paragraph below to be updated for DP --- right now we only have SP}
...
%% Below is the SP module figure and related paragraph, left in because it is referenced in two places later as justifying a low expected gradient...
\begin{dunefigure}[\dword{cfd} example]{fig:cfd-example}
...
validating a \dword{cfd} model for \dword{pddp} using \dword{pddp} \lar instrumentation data, followed by modeling the \dword{fd} \dword{dpmod} to derive specifications for instrumentation. 
...
% Preliminary results comparing the vertical temperature profiles already exist. Fiure~\ref{fig:cisc-cfd-valid} shows the fluid temperature measured by the dynamic T-gradient thermometer and compares it to the current \dword{pdsp} \dword{cfd} model, with parameters shown in Table~\ref{tab:fdgen-cisc-CFDparam}.
...
% \begin{dunefigure}[Comparison of ProtoDUNE-SP temperature data and CFD siulations]{fig:cisc-cfd-valid}
...
%The \dword{cfd} reassuringly predicts a reasonable response for more than one set of initial conditions. It is still important to %, but measure the instrumentation response to help establish he validity of the \dword{cfd} model. We did not run tests with differing initial conditions during the beam run because even controlled changes of the cryostat environment could have undesirable effects.    However, dedicated tests to validate the \dword{cfd} under various deliberate changes of the cryostat environment are planned.  Possible additional tests include pump and recirculation manipulations such as pump on-off, change of pumping speed, and bypassing of filtration, and changing the cryostat pressure set point to a higher (or lower) value\footnote{The \dword{hv} could be ramped down for this exercise because dropping the pressure too fast might provoke boiling of the \dword{lar} near the surface.} to induce changes in the pressure for a specified time while monitoring the instrumentation. Any change in pressure could change the temperatures everywhere in the cryostat. Studying the rate of this change, as detected at various heights in the cryostat, might provide interesting constraints on the \dword{cfd} model.
...
%\fixme{Please check the rephrasing of the previous sentence. I think this is correct. ACV: IT IS CORRECT}
...
%As shown in Figure~\ref{fig:sensor_cable}, the PT102 sensor has a length of \SI{21}{mm}, which can easily be accommodated in the \dword{dune} T-gradient monitors. 
...
%As Figure~\ref{fig:cisc-cfd-valid} shows, a configuration with 24 sensors was appropriate in \dword{pdsp}, as it should be in the \dword{dpmod} where the expected total gradient is no larger than in \dword{pdsp} (see Figure~\ref{fig:cfd-example}). To mitigate the risk of sensor failures, installing two sensors at each position is under consideration.
...
pieces %,  %(see Figure~\ref{fig:cable-support})
%which 
...
in August 2018. The analysis of this data and the comparison with \dword{cfd} simulations is actively underway, but interesting preliminary results are available and will be described below. Fig.~\ref{fig:pd_inst_map} shows the distribution of temperature sensors in the \dword{pdsp} cryostat.  
...
Fig.~\ref{fig:pd_tgradient_results} shows the vertical temperature profiles as measured by the dynamic and static T-gradient monitors at a given moment in time (10 minutes in May 2019). The stability of those profiles has been carefully studied: the relative variation between any two sensors on the same profiler is below 3 mK along the entire data taking, probing that the shape of the profiles is nearly constant in time. In Fig.~\ref{fig:pd_tgradient_results} one immediately observes that the shapes of the two profiles are similar, with a bump at 6.2 m, but the magnitude of %the 
...
%\fixme{LHC is not defined in either the common glossary or in the glossary for this document. Does it need to be defined? If not, should it be spelled out?. CP: Added as a dword}
...
%\fixme{I don't see LAPD defined in either the common glossary or in the glossary at the end of this manuscript. Does it need to be defined in the glossary?. CP: Added to the glossary at the end of this text. Glenn: now using dword in text.  This fixme can be closed.} 
...
%The assembly of the individual purity monitors into the string would follow the steps laid out in the first 5 panels of Figure~\ref{fig:PrMon-Assembly}.
...
% \fixme{GAHS: Update paragraph below for DP "3rd draft" or supplement once more is known.}
...
SUB-D 
%\fixme{I'm going to assume SUB-D is commonly understood in the field. If not, it should be defined in the glossary.CP: It is commonly understood, not added to the glossary.} 
...
The \dword{pab} 
%\fixme{PAB is in neither glossary. Does it need to be included in the common glossary? CP: Added to the glossary at the end of this text. Glenn: now using dword.  This fixme can be closed.} 
facility at \fnal houses the \dword{iceberg} \SI {3000} {liter} cryostat that allows fast turnaround for testing of the \dword{dune} \dword{ce}. 
...
%\fixme{Is GPIB a commonly accepted abbreviation or should it be included in the glossary? CP: Added} 
...
available in the \dword{cuc}'s \dword{daq} server room for \dword{cisc} use. This is ample space (see Section
...
% \fixme{When I checked the glossary for the definition of IFBeam, the definition included database, indicating that IFBeam is by definition a database. Glenn: glossary appears to be incorrect; correct acronym is "IFBeam DB" -- see their conference paper at http://cd-docdb.fnal.gov/0047/004781/001/IFBPaper2.pdf.}
...
% a commercial toolkit used extensively at \dword{cern}, with device interfaces supported using several standardized interface protocols.
...
\begin{description}
 ...
 \item[Argon Instrumentation:] purity monitors, thermometers (\dword{lar} and \dword{gar}), capacitive level meters, pressure meters (\dword{gar}), cameras and light emitting system, and \dword{citf}; feedthroughs; \efield simulations; instrumentation precision studies; \dword{protodune} data analysis coordination and validation. 
...
% commented out cost and labor sections
  % Note all nested comment environments are commented out below with %
  % because comment package does not allow nesting of comments!
\begin{comment}
...
\fixme{Comment command commented out because they don't nest. Glenn}
%begin{comment}
\begin{dunetable}
[CISC Cost]
...
\dfirst{citf}        & 1 & \$62,000  & Material cost includes liquid argon costs for years 1 to 5 and minor fixture costs  \\ \colhline
...
{p{0.3\textwidth}p{0.08\textwidth}p{0.1\textwidth}p{0.45\textwidth}}
{tab:cisc-cost}
{Cost estimates of the different \dword{cisc} subsystems. All cost estimates include packing and shipping costs.}
...
\dfirst{citf}      & 1 &-  & Material cost includes liquid argon costs for years 1 to 5 and minor fixture costs  \\ \colhline
...
\dfirst{citf} & 1768& 2400& 2400& 180& 525& \textbf{7273}\\ \colhline
...
2.1     & \dfirst{citf} ready                                          & 12/19 & 05/20   \\ \colhline
2.2     & Production of first full system prototypes of all CISC hardware devices                      & 11/19 & 06/20   \\ \colhline
2.3     & Test instrumentation device prototypes at \dword{citf}                             & 06/20 & 10/20   \\ \colhline
...
2.4.4   & Begin integrating/testing instrumentation devices in the cold box at the SURF \fixme{was ITF-replace!}            & 04/22 & 11/22   \\ \colhline
2.4.5   & Begin integrating/testing Slow controls hardware/software at \fixme{was ITF-replace!} (as part of the DAQ)        & 02/22 & 07/22   \\ \colhline
...
2.4.14  & Install Dynamic T-gradient monitors, Cameras, Purity monitors after \dword{tpc} installation         & 06/24 & 07/24   \\ \colhline
...
\dfirst{citf} ready                                          & 12/19 & 05/20   \\ \colhline
Production of first full system prototypes of all CISC hardware devices                      & 11/19 & 06/20   \\ \colhline
Test instrumentation device prototypes at \dword{citf}                             & 06/20 & 10/20   \\ \colhline
...
\textbf{Begin integrating and testing Detector\#1 components at \fixme{was ITF-replace!}} & 02/22 & 02/22 \\ \colhline
Begin integrating/testing instrumentation devices in the cold box at the SURF \fixme{was ITF-replace!}            & 04/22 & 11/22   \\ \colhline
Begin integrating/testing slow controls hardware/software at \fixme{was ITF-replace!} (as part of DAQ)        & 02/22 & 07/22   \\ \colhline
...
\textbf{Cryostat\#1 ready for \dword{tpc} installation} & 05/23 & 05/23 \\ \colhline
...
Install dynamic T-gradient monitors, cameras, purity monitors after installing \dword{tpc}          & 06/24 & 07/24   \\ \colhline
...
%\input{generated/risks-longtable-DP-FD-CISC.tex}
...
%19 & The baseline design of the \dword{citf} is not suitable for some of the alternative designs mentioned in risk 1 in Table~\ref{tab:fdgen-slow-cryo-risk1}. & L & The constraints of the CITF should be taken into account when designing new prototypes, so that such that those new designs can be easily accommodated in the CITF.   \\ \colhline
...
M
&
Cameras will have to be tested during the R\&D phase under conditions of illumination similar to the ones of the actual detector. The \dword{citf} %cryogenics test facility 
should have sufficient depth for testing the ability of seen deep objects.
\\ \toprowrule
14
&
...
&
The current design of these devices could fail if operated in impure liquid or gas argon because of the degradation of photocathode for a long time. A significant photocathode degradation was observed during \dword{lapd} running when the cryo-pump failed for a few days. In principle, since Purity Monitors are most useful during commissioning, this may not be a major issue but since cosmic muons will be sparse in the \dword{fd}, with no purity monitors, we may be at a risk for monitoring purity especially after recovering from cryogenic issues, power outages, etc., when one cannot use tracks to make the purity measurement.
&
M
&
...
\dword{tpc} electronics	               & 6784  & \\ \colhline
...
%  the APAs as they go in.  If the there is a slow controls test stand
...
%  DAQ/Electronics groups for a slow controls test stand at SURF,
...
\paragraph{Static T-gradient monitors}
 %should 
will be installed after \dword{fc}s along the long cryostat sides. The profiles %will be 
...
\paragraph{Individual temperature sensors} on pipes/\dword{pd}s\footnote{Actually on \dshort{pd}'s support structure}  and cryostat floor %will be 
should be installed after \dword{pd}s and the grounding grid. First, vertical strings for cable routing %will be 
are installed following a procedure similar to the one described above for the static T-gradient monitors. Next, anchor all cable supports to pipes/\dshort{pd}s. Then each cable %will be 
...
%\fixme{ACV: Text added. To be checked by experts. I'll send email to Dominique. DONE, Dominique is happy with it}

\paragraph{Vertical arrays in gas phase:} The baseline design assumes twenty arrays, each of which is attached to a \dword{crp} frame. Thus, they should be installed on the corresponding \dword{crp} before being moved into the cryostat. Cables will be routed to the assigned cryostat port using the appropriate elements on the \dword{crp} frame. 
...
%\fixme{LN is not in the common glossary. Does the abbreviation need to be defined? If so, it should be added. CP: Done.}
...
The pressure sensors will be tested with a modest sample of gaseous argon in the laboratory before they are installed. After installation, they are tested \textit{in situ} at atmospheric pressure. The whole pressure readout chain, (including slow controls \dword{plc} and software protocol) 
%WINCC conversion) 
...
%\fixme{I am assuming LN is liquid nitrogen. Does LN need to go in the common glossary?. CP: Added} 
...
\item Hazardous chemicals (e.g., epoxy compounds used to attach sensors to cryostat inner membrane) and cleaning compounds:
  ...
  %(MSDS)
  %\fixme{MSDS is not in the common glossary nor the document glossary. Should it be included? If it is used only once, then instead of abbreviating, simply spell it out. CP: Done}
