...
\begin{dunefigure}[DP module's light calibration system.]{dppd-6-LCS}
...
\begin{dunefigure}[Light guide structure (dev phase) and light path from LED to ref sensor]{fig_source}
{(Left) Picture of the light guide structure during the development phase. The six arms are visible, and on one of them a prototype \dword{led} driver is mounted. (Right) Schematics of the light path from the \dword{led} to the reference sensor.}
\includegraphics[width=0.25\textwidth]{dppd_LightGuide.jpg}
\includegraphics[width=0.6\textwidth]{dppd_Stray.png}
\end{dunefigure}
\fixme{anne changed `light way' to `light path' in figure caption - short and long. 4/29}
