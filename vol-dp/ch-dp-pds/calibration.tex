...
\fixme{This sub-section heading can be removed and the paragraph following can be used as an introduction after the major section heading.}
...
After operating a previous \num{3}$\times$\num{1}$\times$\num{1}\,m$^3$ \dword{lar} \dword{tpc} detector~\cite{Aimard:2018yxp} and the \dword{pmt} characterization at cryogenic temperature~\cite{Belver:2018erf}, we concluded a light calibration system is needed during experiment data taking.
