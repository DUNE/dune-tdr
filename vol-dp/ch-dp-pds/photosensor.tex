...
\fixme{This subsection heading can be omitted, and the section can serve as an introduction to the chapter under the section heading, Photosensor System.}
...
The baseline number of \dwords{pmt} is \dpnumpmtch with \num{80} spares.  Several operations and tests must be performed with the \dwords{pmt} before they are installed. The \dwords{pmt} must be ordered with sufficient lead time to complete the following planned operations: assembling the voltage divider circuit, mounting on the support structure, testing at room and cryogenic temperatures, packing and shipping to the \dword{itf}. The \dword{tpb} coating of the \dword{pmt} windows will be done at the \dword{itf}. The \dwords{pmt} will be re-tested for validation of basic functionality at the \dword{itf} and at \surf before installation (see Section~\ref{sec:dp-pds-installation}). Considering the large number of \dwords{pmt} required by \dual \dword{pds}, the purchase order must be completed at least two years before installation. A staged or staggered order with a steady supply of \dwords{pmt} would be most convenient and will be negotiated with the manufacturer.
