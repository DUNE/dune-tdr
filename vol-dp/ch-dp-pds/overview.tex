...
The \dwords{pmt} are individually attached to the cryostat floor \SI{1.02}{m} apart in both directions, via a \dword{pmt} support structure that counteracts the \dword{pmt} buoyancy. In collaboration with the \dword{hv} consortium, to increase the spatial uniformity of the \dword{pds} response, installing \dword{wls} reflector foils on the inner surfaces of the four \dword{fc} vertical walls is under consideration. The mechanical aspects of the \dword{pds} design are described in Section~\ref{sec:dp-pds-mechanics}.
