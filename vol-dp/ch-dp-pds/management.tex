...
The \dual \dword{pd} consortium was formed in 2017;  it comprises eleven institutions from France, Peru, Spain, UK, and USA. The charge of the \dual \dword{pd} consortium is to plan and execute the construction, installation, and commissioning of the \dword{dp} \dword{pds}.
% I think we should check with Peru and UK if they are still interested in being included in the consortium

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Consortium Organization}

The current \dual \dword{pd} consortium leader (CL) \fixme{This abbreviation won't work because CL is already in the common glossary; it stands for confidence level.} is %In\'{e}s Gil-Botella
 from CIEMAT (Spain) and the technical lead (TL) \fixme{This abbreviation is not yet in the common glossary. It is also missing from the glossary for this volume.} is %Dominique Duchesneau 
 from LAPP (France). They are members of the \dword{dune} technical board and represent the consortium in the overall \dword{dune} collaboration. The CL is responsible for the subsystem deliverables and for effectively managing the consortium. The TL acts as the overall project manager and is the interface to the international project office (IPO) \fixme{This abbreviation is also not in the common glossary nor the volume glossary.}; he is responsible for monitoring and reporting on progress on the agreed schedule and for interface documentation.
...
The \dual \dword{pd} consortium is divided into four working groups: photosensors and electronics; calibration system; mechanics and integration; and simulation and physics. The corresponding current WG \fixme{This abbreviation is not in the common glossary, the glossary for this volume, and has not been defined in line.}convener institutions are
\begin{itemize}
% AH REMOVING NAMES
...
An initial cost estimate of the \dual \dword{pd} photon detection system for one 10kt \dword{dune} detector was developed in 2018 (see Table~\ref{tab:dppd_t1.7}). This is based on protoDUNE-DP costs.
