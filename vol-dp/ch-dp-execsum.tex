...
Photon detection system time resolution & $\leq$  1$\mu$s  \\  \colhline 
\end{dunetable}
...
The filling of \dword{pddp} was completed in August 9th 2019. After a period of a couple of weeks devoted to the \dword{crp} calibration, alignment and commissioning first tracks were seen on August 29th as soon as the LEM were switched on at the minimal voltage of 2.9 kV. The operation of the \dword{crp} has been then progressively performed at higher voltages. The first phase of commissioning of \dword{pddp} allowed validating the good performance of the \dword{crp}, of the LEMs, of the pthoton detection system, of the charge readout electronics and DAQ system.

\begin{dunefigure}[Event recorded on a \dword{crp} of \dword{pddp} ]{fig:dp-execsum-event}
  {Cosmic ray event recorded on a \dword{crp} of \dword{pddp}. The event, recorded with the LEM operating at 3.1 kV, includes two muon decays and some electromagnetic showers }
...
\begin{dunefigure}[A view of a  \dword{pddp} cathode module]{fig:dp-execsum-dune-dp-cathode}
...
A set of 80 $3~m \times 3~m$ ground grid modules, standing on the cryostat floor, are installed to protect the array of \dword{pmt} from any electric discharge from the cathode.
 ...
 \dword{pddp} in its actual phase has been built with a HV system designed for 300kV, corresponding to a minimal requirement of E ${\simeq}$ \SI{0.25}{kV/cm} for the \dword{dpmod}. 600 kV could be tested in the second phase of  \dword{pddp} by performing some changes to the cathode and the high voltage feedthrough which should be directly interfaced to the HV power supply since there are no commercial cables availables for 600 kV. The design of the integration of the HV feedthrough with the 600 kV power supply is foreseen to be performed in collaboration with Heinzinger. A 600 kV HV system can be also tested in a separated setup that Fermilab is considering to build to test at the same time the 600 kV operation and 12 m drift. The equivalent of 12 m drift can be tested as well in  \dword{pddp} by reducing the drift field.
