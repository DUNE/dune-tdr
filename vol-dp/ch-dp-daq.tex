...
Strategies will be developed and validated during commissioning and early running that will reduce the data volume through the selection of data based on the activity in the detector (in time and geographical areas), as well as through data compression.
...
Such triggers will prompt \snbtime of data from the entire module to be read out. There is, however, no requirement to
...
{tab:daq:rates}{Summary
    ...
    given for a single \SI{10}{\kilo\tonne} \dword{spmod}.  The equivalent figures for a  \SI{10}{\kilo\tonne} \dword{dpmod}   for cosmics and beam data are significantly smaller.
...
%is estimated to consume less than \SI{50}{\kilo\watt} and use less than 200U of rack space \cite{bib:docdb15544}. %sync overleaf w/ recent github update (10/23/19)
\hlfix{should consume}{These are SP estimates, provide DP versions} less than \SI{63}{\kilo\watt} and all fill less than 300U of rack space \cite{bib:docdb15544}.
...
\subsection{Design Validation and Development at ProtoDUNEs and Other
  LArTPC Experiments}
