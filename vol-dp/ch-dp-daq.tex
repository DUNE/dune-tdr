...
\begin{dunefigure}[DAQ conceptual design overview for one
  ...
  \includegraphics[width=0.8\textwidth,trim=4cm 3cm 4cm 3cm]{graphics/daq-layout-2.pdf}
\end{dunefigure}
...
Full size of TPC Localized Event Record per Module (uncompressed) & \SI{4.2}{GB} \\  \colhline
Full size of TPC Extended Event Record per Module (uncompressed) & \SI{56}{TB}\\ 
\end{dunetable}
...
dynamically from channel to channel. 

An example of possible data format is described in the following,
...
\begin{dunefigure}[DAQ system overview]{fig:daq-conceptual-overview}{Conceptual
  ...
  \includegraphics[width=0.9\textwidth]{graphics/daq-overview.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DUNE upstream DAQ subsystem and connections]{fig:daq:readout}{\dshort{dune} upstream \dshort{daq} subsystem and its connections.}
  \includegraphics[width=0.8\textwidth]{graphics/daq-readout}
\end{dunefigure}
...
\begin{dunefigure}[Flow diagram of the DUNE upstream DAQ subsystem.]{fig:daq:readout-blocks}{\dword{dune} upstream
 ...
 \includegraphics[width=0.8\textwidth]{graphics/daq-ud-func-blocks}
\end{dunefigure}
...
\begin{dunefigure}[Data selection strategy and hierarchy]{fig:daq:data-selection-hierarchy}{Data selection
 ...
 \includegraphics[width=0.9\textwidth, trim=0cm 0cm 0cm 0cm]{graphics/DS_hierarchy.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DUNE DAQ data selection
  ...
  \includegraphics[width=0.95\textwidth, trim=0 2cm 0 0]{graphics/DS_summary_2.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DUNE DAQ back-end operation]{fig:daq:backend}{Illustration of \dword{dune} \dword{daqbes} operation.}
  \includegraphics[width=\textwidth]{graphics/daq-backend.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DAQ CCM subsystem interaction]{fig:daq-ccm-subsys}{Main interaction among the three \dword{daqccm} subsystems.}
 \includegraphics[width=0.9\textwidth]{graphics/daq-ccm-subsys.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DAQ control subsystem roles and services]{fig:daq-ccm-control}{Roles and services that compose the \dword{daq} control subsystem.}
  \includegraphics[width=0.8\textwidth]{graphics/daq-ccm-control.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DAQ CCM subsystem interaction]{fig:daq-ccm-config}{Main components of the \dword{daqccm} configuration subsystem.}
  \includegraphics[width=0.8\textwidth]{graphics/daq-ccm-config.pdf}
\end{dunefigure}
...
\begin{dunefigure}[DAQ monitoring subsystem roles and services]{fig:daq-ccm-monitoring}{Roles and services that compose the \dword{daq} monitoring subsystem.}
  \includegraphics[width=0.8\textwidth]{graphics/daq-ccm-monitoring.pdf}
\end{dunefigure}
...
\begin{dunefigure}[ProtoDUNE DAQ hardware as used during beam data taking]{fig:daq:protodunedaqpic}{ProtoDUNE DAQ hardware as used during beam data taking.}   %\includegraphics[height=0.5\textheight]{graphics/DAQ_hardware_NP04.pdf}
\end{dunefigure}
...
The overall activity of the online storage and processing farm is monitored with grafana dashboards. A run control interface ensures the control and monitoring of all the components of the system in order to start and stop runs, and check the data transfer to the local EOS. The  \dword{pddp} back-end system is shown in picture \ref{fig:BE_machines}
 ...
 \includegraphics[width=0.7\textwidth]{graphics/daqroom.pdf}
\end{dunefigure}

The FE AMC digitization cards have also a data acquisition mode in continuous streaming with data compression, which is foreseen for the triggerless operation in the \nominalmodsize\dword{dpmod}. The \dword{pddp} back-end can also support this operation mode in which the trigger is defined as back-end by using the L1 event builders for memory buffering and definition of the trigger primitives, interconnected to a trigger definition node deciding which data have to be dumped on disk. The L2 machines will be then still used in order to assemble the files to be written on the distributed storage system. In this operation mode data are supposed to be decompressed on the fly by the L1 network cards in order to perform the search for the trigger primitives. Dedicated R\&D is being performed in order to optimize the decompression and trigger primitives search firmware by using Bittware FPGA cards. This firmware can be then implemented on other network cards with FPGA processing capabilities such as the \dword{felix} cards. It is foreseen in a second phase to replace the Mellanox network cards connecting the L1 event builders to the uTCA crates with \dword{felix} cards in order to have the operation mode of \dword{pddp} compatible with both the trigger based mode which has been designed for beam application and the possibility of performing full scale trigger-less tests of the \dword{dune} operation mode.

\subsubsection{\dword{pddp} Outcomes}
\begin{comment}%Anne rewriting
The installation of the \dword{daq} system of \dword{pddp} was completed in March 2019 with the installation of the digital front-end and timing systems. The \dword{daq} system was immediately operational without having to implement any configuration changes and since then it  %has been  constantly collecting  
collected noise data constantly through August 2019 when the TPC started operating. The data taking for cosmic events started on August 29th 2019, after the Charge Readout Planes commissioning period. Since then more than 1.2 million cosmic events have been collected, corresponding to 130 TB. The data taking has been mostly performed with random triggers at a a rate of tens of hertz, %few 10 Hz rate 
...
The installation of the \dword{pddp} \dword{daq} system was completed in March 2019 with the installation of the digital front-end and timing systems. It was immediately operational without having to implement any configuration changes,  and 
collected noise data constantly through August 2019, when the TPC started operating. The data taking for cosmic events started on August 29, after the \dword{crp} commissioning period. Since then, it has collected more than 1.2 million cosmic events, corresponding to 130 TB. The data taking has been mostly performed with random triggers at a rate of tens of hertz, %few 10 Hz rate 
given that the scintillator panels were not available  to tag cosmic rays. 
\fixme{connect rate to fact that panels were not available. anne}
This running mode -- based on random triggers -- also allowed to stress and test the system at a higher rate than what the cosmic rays would have provided. The online storage facility has  systematically  reconstructed events in real time.


%Anne rewriting The \dword{daq} system has shown to be very stable with no issues encountered at the level of the event builders and the associated software. The only problems occurred were related to some sporadic cooling failures of the \dword{daq} room (in common for both \dword{pdsp} and \dword{pddp}) which hosts the network infrastructure and the event builders and the storage facility. These cooling failures obliged to shut down the machines for a few hours until when the chilled water re-circulation was properly restarted by the \dword{cern} infrastructure. The back-end system implemented for \dword{pddp} is equivalent to the system which will be implemented for a \nominalmodsize \dword{dpmod}, it is fully interfaced with the same front-end electronics foreseen for the \nominalmodsize module and it has shown to be working with no issues, guaranteeing a smooth detector data taking. The online storage system of \dword{pddp} has already the capabilities to satisfy the storage needs of a \dword{dune} far detector module.
The \dword{daq} system has demonstrated good stability with no issues at the level of the event builders and the associated software. The only problems have been related to some sporadic cooling failures in the \dword{daq} room (for both \dword{pdsp} and \dword{pddp}), which houses the network infrastructure, the event builders, and the storage facility. These cooling failures forced us to shut down the machines for a few hours until the chilled water re-circulation was properly restarted. The back-end system implemented for \dword{pddp} is equivalent to that which will be implemented for the \nominalmodsize \dword{dpmod}, fully interfaced with the same front-end electronics.  It has been operating smoothly with no problems. The \dword{pddp} online storage system is already adequate to satisfy the \dword{dpmod} storage needs.
...
The \dword{daq} will be installed in an enclosure in the west end of the \dword{cuc} (``Data Room'' in Figure~\ref{fig:cavern-layout}.  Roughly
...
\begin{dunefigure}[Layout of the DUNE underground areas]{fig:cavern-layout}
  ...
  The \dword{cuc} in the middle houses the \dword{dune} data room where the DAQ will be installed and the underground utilities. }
\includegraphics[width=.9\textwidth]{graphics/cavern-layout}
\end{dunefigure}
...
\includegraphics[width=.85\textwidth]{graphics/cuc-layout}
\vspace{-2pt}
\includegraphics[width=.9\textwidth]{graphics/cuc-cf-racks}
\end{dunefigure}
...
\begin{dunefigure}[DAQ counting room in the CUC]{fig:daq-room}{Initial engineering design for the \dword{daq} counting room in the CUC}
 \includegraphics[width=0.9\textwidth]{graphics/daq-room.jpg}
\end{dunefigure}
...
\begin{dunefigure}[DAQ consortium org chart]{fig:daq-org}{Organizational chart for the \dword{daq} Consortium
  ...
  %\includegraphics[width=0.9\textwidth]{graphics/daq-organogram.pdf}
\end{dunefigure}
...
The high-level \dword{daq} milestones are listed in Table~\ref{tab:daq-sched}, interleaved with the top-level DUNE project milestones, and illustrated in %Figure~\ref{fig:daq:schedule}.
...
\fixme{check that Figure~\ref{fig:daq-schedule} is the right figure}
...
\begin{dunefigure}[DAQ schedule for first \dshort{dpmod}]{fig:daq-schedule}{\dword{daq} schedule for first \dshort{dpmod}.}
  \includegraphics[width=0.95\textwidth,clip,trim=1cm 2cm 1cm 2cm]{graphics/daq-schedule.pdf}
\end{dunefigure}
