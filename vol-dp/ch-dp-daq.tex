...
An \dword{fd} composed of \hlfix{four single-phase modules}{Convert to DP} using a
...
\hlfix{single-phase module}{Convert to DP}. There is, however, no requirement to
...
\hlfix{should consume}{These are SP estimates, provide DP versions} less than \SI{63}{\kilo\watt} and all fill less
...
custom \dword{fpga} based data reciever board and commodity computing hardware, 
all of which is hosted in \hlfix{80-85 servers}{This is an SP estimate, provide DP version} in the \dword{cuc}. A
...
\dword{tpc}, \hlfix{75 functionally identical}{SP numbers, provide DP estimates} \dword{daqrou} are each responsible for the readout of raw data from two
...
\fixme{This section has several DP/SP differences and needs careful checking}
...
\hlfix{single-phase detector module}{Fix for DP}.
...
Figure~\ref{fig:daq:readout-blocks} \hlfix{shows}{this diagram is SP-specific. Needs to be remade for DP.  Probably fine to simply remove "from WIBs"}
...
\begin{dunefigure}{fig:daq-rates}{ProtoDUNE DAQ hardware as used during beam data taking. \label{fig:daq:protodunedaqpic}}
...
\fixme{Something is needed ro replace or justify this SP-specific section to DP.}
...
\fixme{Something is needed to replace or adapt this SP-specific section to DP.}
...
\end{dunefigure}
\fixme{check UCL and PNNL, they are not listed in the SP chapter}
\begin{dunetable}
...
\fixme{This section was copied from latest SP chapter, fix if needed and delete this}
