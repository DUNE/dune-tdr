...
\section{Risks}
%% From Anne 10/31, based on what is in sp install
The process of constructing \dword{pddp} has been extremely important in planning the \dword{dpmod} installation. Lessons learned were used to develop a list of risks for the \dword{dpmod} installation and to formulate mitigation strategies to reduce the risks. The highest-impact risks -- those requiring a mitigation strategy -- are listed in Table~\ref{tab:risks:DP-FD-INST}. These mitigation strategies and all the lessons learned from
\dword{pddp} will be factored into the detailed installation plan.
