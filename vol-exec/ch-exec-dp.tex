...
Each \dword{sftchimney} is coupled to a \dword{utca} crate that houses in \num{10}  digitization cards  transmits the data %via the \dword{mch} switch 
...
to the %\dword{amc} 
digitization cards and also distributes the \dword{lv} and slow control signals to the analog \dword{fe} electronics.  
...
the design of the charge readout and hosts a specific circuitry based on the \dword{catiroc} \dword{asic} to trigger the light-readout. %By assuming a \dword{pmt} channel density similar to \dword{pddp}, five \dword{utca} crates are sufficient to read \dpnumpmtch \dwords{pmt}.
...
The drift cage design shares common structural elements (aluminum profiles and \dword{frp} supporting beams) with the \dword{sp} \dword{fc} design, but with a different arrangement (vertically hung structure) to cope with the \dword{dpmod} geometry.
...
As shown in  Figure~\ref{fig:dp-execsum-dune-dp-cathode}, each cathode module is constructed of two \SI{12}{m} long trusses made from thin-walled stainless steel tubes with approximately \SI{50}{mm}  outer diameter. %They can either be prefabricated and transported underground like the large cryostat beams or assembled in the underground cleanroom from sectional parts. 
The trusses are 2m apart and are connected at both ends to stainless steel tubes that run under the \dword{fc} and form the outer edges of the cathode plane. %A total of 122 resistive rods 10mm in diameter are installed at a 10 cm pitch 
...
The Ethernet-based \dword{daq} back-end system is designed by a joint \dword{sp}-\dword{dp} consortium, and the system is identical for the two. The \dword{daq} connects to 
