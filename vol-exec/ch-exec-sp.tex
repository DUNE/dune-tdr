...
\fixme{seems like this needs a reference}
...
The \dword{daq} is divided between an upstream section, located underground in the \dword{cuc}, and a downstream \dword{daqbes} to be located above ground at the \dword{surf}. All trigger decisions are made %underground
upstream, and the data is buffered underground until the \dword{daqbes} indicates it is ready to receive data; this minimizes the rate of data flowing to the surface. An end-goal of the \dword{daq} is to achieve a data rate to tape of no more than \SI{30}{\peta\byte/\year}.
\fixme{do we really want to ``minimize the data rate to the surface''? or just control it?}
