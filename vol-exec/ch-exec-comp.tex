...
The full \dword{dune} software and computing effort will be much larger than that of the \dword{protodune} experience. It is imperative that serious construction and operations work by the \dword{csc} be maintained well before detector commissioning begins at \dword{surf}. %ddm
...
Offline computing for \dword{dune} creates new and considerable challenges because of the experiment's large scale and more diverse physics goals.  In particular, in the advent of \dwords{lartpc} with an exquisite resolution and sensitivity, combined with the enormous physical volume of the \dword{dune} far detectors, there are challenges in acquiring, storing, reducing, and analyzing a %magnanimous 
prodigious amount of data, orders of magnitude more data than in previous neutrino experiments. 

Neutrino experiments, including \dword{dune}, operate at low event rates; \SI{1}{Hz} even for near detectors. 
...
Each of the 150 \dword{spmod} \dwords{apa} (described in Chapter~\ref{ch:exec-sp}) has 2,560 readout channels, each of which % each.Each channel 
...
Table \ref{tab:daq-data-rates-sp}, adapted from Volume~\volnumbersp{} Chapter~7, %the \dword{daq} section,
...
Updating the frameworks is a major task for the \dword{dune} computing R\&D for the future. 
...
{Example of pedestal-subtracted data for one \dword{pdsp}  wire plane.  The top pane shows the \dword{adc} values in a V (induction) plane with the x-axis as channel number and the y-axis as time slice. The bottom pane shows the bipolar pulses induced on one channel.}
...
\section{Conclusion}
\fixme{No other chapter has a ``conclusion'' section}
