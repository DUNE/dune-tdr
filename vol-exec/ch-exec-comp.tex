...
HEP has considerable infrastructure in place for international computing collaboration thanks to the \dword{lhc} program.  Additional large non-\dword{lhc} experiments, including \dword{lsst}, \dword{ska}, \dword{dune}, and \dword{hyperk}, \fixme{These are also not in the common glossary.} will enter operation over the next decade and must use and expand upon this model for international cooperation.  Organizing the broader HEP community is formalized through the \dword{hsf} \cite{Alves:2017she}.  The \dword{hsf} is an organization of interested parties working to use the extensive knowledge gained over the past two decades and anticipate the needs experiments will have over the next two decades to develop a sustainable computing landscape for the HEP community.  The \dword{hsf} white papers and roadmaps emphasize common tools and infrastructure as the underpinnings of this landscape.
...
\begin{dunetable}[Useful quantities for computing estimates]{lrr}{tab:exec-comp-bigpicture}{Useful quantities for computing estimates for single phase readout}%\rowtitlestyle
...
\begin{dunefigure}[Figure from volume \todo{pointer to SP DAQ sectin} Expected physics-related activity
...
\todo{Update to reflect 90 sec supernova window and other new parameters}
...
\fixme{anne commented out includegraphics line because graphic not found}
\begin{figure}[t]
...
Volume~\volnumberphysics, Chapter~\ref{Need a reference here}. \fixme{for anne} 
