...
HEP has considerable infrastructure in place for international computing collaboration thanks to the LHC program.  Other large experiments - LSST, SKA, DUNE and HyperK will be coming on board over the next decade.   This cooperation is being formalized through the HEP Software Foundation\dword{hsf}\lcite{Alves:2017she}, an organization of interested parties working to gather and disseminate common solutions based on the extensive knowledge we have gained over the past two decades. 
DUNE's strategy is to work with and contribute to this global community to maximize the use of common tools for data movement and storage, job control and monitoring, accounting and authentication.   All large-scale experiments will encounter similar issues and worldwide cooperation on common tools is the most cost-effective way to proceed. For example, in collaboration with the Fermilab, CERN and the UK groups, we are investigating the use of  \dword{rucio}  as our primary data manager.
...
There is a global \dword{rucio} team which now includes Fermilab SCD staff, DUNE collaborators, and CMS collaborators in addition to the core ATLAS developers who wrote it in the first place.  Some tasks are currently under way or will soon begin.  These include (a) making object stores (such as Amazon S3 and compatible utilities) work with Rucio.  There is a large object store in the UK on which DUNE has a sizable allocation.  (b) Monitoring of the \dword{rucio} system, leveraging the Landscape system at Fermilab.  (c) Getting a metadata engine that can be used as a replacement for the SAM system we currently use.
...
There is currently a Data Organization, Management, and Access (\dword{doma}) taskforce working in the larger HEP community\cite{Berzano:2018xaa}
 in which several DUNE collaborators are involved in some way or other. This has several sub taskforces. These include taskforces on authorization, caching, third party copy, hierarchical storage, and quality of service. 
The authorization issues are of much importance to DUNE and are covered in the subsection below. 

The caching technologies under discussion sound quite promising and there are DUNE sites who have staff who have been involved in the development.  These are useful for transporting auxiliary files such as flux files that are commonly used in neutrino analysis.  DUNE will have to devote some effort to testing them for our use cases.

Perhaps the key thing from DOMA is the third party copy taskforce.  The biggest difficulty we have faced to date with \dword{rucio} to date is finding appropriate sets of protocols to communicate with third party copy between all the various sites.  The task is complicated because each underlying storage element is based on different legacy technologies, each of which has its own implementation of SRM and xrootd protocols.  The trend is toward web based services.  We will also have to find some way to trigger staging of files off tape for the third party copies; there is not a uniform way to do this at the moment.

There are also various low level technologies for transferring data across the WAN which may be better than the conventional TCP/IP stack.  Some work should be done to evaluate these, particularly for transferring data between the so-called "Data lakes".

We will probably have to investigate high throughput storage for some data buffering at the interface of DAQ and offline.  We will also have to deal with the issue of how to utilize proprietary storage systems on commercial clouds, and non-grid-enabled storage systems at high performance computing centers which don't have the same type of network connectivity to which HEP is accustomed.

\subsection{Data Management and Retention Policy Development}

Rucio has already been shown to be a powerful and useful tool for getting data from point A to point B.  We need to develop a policy to figure out how many of its features we will expose to users.  We also need to specify and then develop some kind of an auditing mechanism so we understand the chain of custody of our data, tracking moves, writes, and deletes.  We also have to re-explore the process of defining data sets in the future SAM replacement.
...
\subsection{Authentication and Authorization Security and Interoperability}
...
\section{Evaluations of other important infrastructure}
...
For workflow management, we are evaluating DIRAC \cite{Falabella:2016waj} and plan to investigate PANDA \cite{Megino:2017ywl}. Both of these systems are used by multiple LHC and non-LHC experiments and are being integrated with \dword{rucio}. 
