...
{The \dword{3dst} inside the KLOE magnet. The drawing shows the \dword{3dst} in the center (white), \dword{tpc}s (magenta), \dword{ecal} (green), magnet coil (yellow), and the return yoke (gray).}
...
Neutrons can be produced from the struck nucleus, as well as from follow-on interactions of the neutrino's reaction-products with other nuclei. The energy carried away by neutrons is difficult to detect and can bias the reconstructed neutrino energy. The \dword{3dsts} and \dword{mpd} have capabilities that allow neutron energy to be directly measured. The \dword{duneprism} program constrains the true-to-reconstructed energy relation and is thus also sensitive to energy carried by neutrons.
...
{\dword{dune}  \dword{nd} hall shown from above (top) and from the side transverse to the beam (bottom). The \dword{arcube}, \dword{mpd}, and \dword{3dsts} are shown (in that order, bottom to top, in the upper figure) in position on the beam axis (black arrow) in both drawings. }
...
Because the \dword{3dsts} does not  move, it is placed in a dedicated alcove downstream of the other  detectors.
