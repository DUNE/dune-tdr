...
{The variation in the neutrino energy spectrum is shown as a function of detector off-axis position, assuming the nominal \dword{nd} location 574~m downstream from the production target.}
...
\item {{\bf Measure particles that leave the LAr \dword{nd} component and enter the \dword{mpd}}  
...
\begin{dunetable}[\dword{mpd} performance parameters]{l|c|c}{tab:TPCperformance}{Expected \dword{mpd} performance, extrapolated from \dword{alice}}
...
%      \caption{The results of a full 4-flavor near+far oscillation fit are shown for a fit to the nominal MC (dashed) and a fit to the fake dataset (solid). The true values of the oscillation parameters in each of the datasets are indicated by the dashed yellow lines. Clear biases can be see in all oscillation parameters that are well outside the 1$\sigma$ (black), 2$\sigma$ (red), and 3$\sigma$ (blue) contours.}
 ...
 %  \end{center}
%\end{figure}
\begin{dunefigure}[Oscillation fits to nominal and fake datasets for \dword{duneprism} fake data study.]{fig:duneprismfit}
...
\begin{itemize}
    ...
    \item{\bf O0.3: Constrain the cross-section model} The \dword{nd} must measure neutrino cross-sections in order to constrain the cross-section model used in the oscillation analysis. In particular, cross-section mismodelling that causes incorrect \dword{fd} predictions as a function of neutrino flavor and true or reconstructed energy must be constrained well enough to achieve the required \dword{cp} coverage. 
