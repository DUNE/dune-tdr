...
\dword{lbnf}/\dword{dune} integration and installation (NP) \fixme{What does NP stand for? It doesn't appear in the common glossary.}
project teams.
...
The \fixme{another?} \dword{jpo} engineering team documents and
...
\fixme{I agree on the sentence. It sounds like something a lawyer would write.}
...
Response Team (ERT) \fixme{I checked the common glossary, and ERT is the abbreviation for Emergency Rescue Team. Are these the same? If so, the terms must be consistent. If they are the same, use the dword instead of abbreviating in text.},  trained guides on all underground levels during all shifts, and protocols for treating injuries, accidents, or spills.
...
\dword{surf} will monitor all fire alarms and system supervisory signals, and the \dword{surf} Emergency Response Team \fixme{See previous fixme.}
...
\fnal hot work permits, coordinated by the \dword{dune}
\dword{esh} coordinator, are completed and approved for all open
