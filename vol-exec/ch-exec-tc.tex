...
\fixme{I think the figure is whole IO or  IO ``installation planning'' team, not the ``on-site'' team; clarify and fix}
\begin{dunefigure}[\dshort{lbnf-dune} \dshort{integoff} organization]{fig:io-org-chart}
  {\dfirst{integoff} organization chart}
...
installation teams 
\fixme{not defined; only the heads are listed in figure. is there more than one team? Who besides consortia people are in it/them?}
...
\fixme{prev pgraph doesn't mesh well with figure}
...
The \dword{sdwf} is a leased \SI{5000}{\square\meter} facility, hosted by 
...
six elements, which correspond to \dword{tc}, %the five \dword{dune} detector modules (
four 
...
Integration engineering for \dword{dune} encompasses three principal focus areas. It %focuses on configuring the
...
installed into their final configurations. Thirdly, it covers %The third major focus is
integration of the necessary services provided by \dword{cf} 
...
the consortia, which provide engineering data for their detector subsystems to the \dword{jpo} team for incorporation within the global configuration files.
