...
\textit{
This chapter provides a brief introduction to DUNE physics.  The text below closely follows the presentation in the introductory chapters of Volume~\volnumberphysics{}, \voltitlephysics{}, where many more details may be found.}
...
in the leptonic charged-current interactions.  That the neutrino masses 
are not zero is among the most important discoveries in fundamental 
...
Almost all neutrino data can be understood within the three-massive-neutrinos 
paradigm, the simplest extension of the standard model capable of reconciling 
...
three-massive-neutrino paradigm, and, critically, 
...
all of the oscillation parameters in the three-flavor paradigm with massive neutrinos, but 
...
the world's neutrino data. While the best fit to the world's data 
favors large CP-invariance violation, it is unknown whether there is 
\dword{cpv} in the lepton sector. All of these questions can be addressed 
...
unitarity of PMNS matrrix from the $3 \times 3$ \dword{pmns} matrix due to extra heavy 
neutrino states can
become particularly sizable the lower the mass of the new states are.
...
\textit{Searches for violation of Lorentz or \dword{cpt} Symmetry:}  DUNE can improve the present limits on Lorentz and \dword{cpt} violation by several orders of magnitude, contributing as a very important experiment to test these fundamental assumptions underlying quantum field theory.

\textit{Searches for neutrino trident production:}  
...
\textit{Search for \dword{ldm}:}  The lack of evidence for \dwords{wimp} dark matter 
candidates at direct detection and the LHC experiments has resulted in a reconsideration of the \dword{wimp} paradigm, and has revitalised the searches for light dark matter candidates 
...
in the core of the sun, allowing to provide competitive results for both production scenarios

\textit{Search Heavy Neutral Leptons (HNLs):} HNLs in the context of the $\nu$MSM model with 
...
results with present and future proposed other facilities can be obtained.
...
\begin{itemize}
  ...
  \item {\bf Far detector site selection for long baseline}\\
    ...
    the 1285-km baseline offered by locating the DUNE far detector
...
\SI{1285}{km} baseline enables a variety of probes of \dword{bsm} 
