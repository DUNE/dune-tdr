...
The U.S DOE \dword{lbnf} project is building a facility in South Dakota that will house and provide infrastructure for the first two \dword{dune} \dword{fd} modules and the \dword{nd} in Illinois.  Figure~\ref{fig:lbnf} shows
...
A simple array of photomultiplier tubes (\dwords{pmt}) coated with a wavelength-shifting material sits below the cathode in the \dword{dp} design. The \dwords{pmt} record  the time and pulse characteristics of the incident light.
...
%{tab:NDsumm-esch1}{This table gives a high-level breakdown of the function of the three major near detector components along with the capability of moving the \dword{dune}{} \dword{nd}{}.}
...
%\dword{3dsts} 
%3DST spectrometer & On-axis & Beam flux monitor &  On-axis flux stability \\ 
...
\dword{3dst} tracker within KLOE magnet.  The concept for this tracker is a  plastic scintillator detector made of \SI{1}{cm} cubes read out along each of three orthogonal dimensions.  
...
U.S. \dword{doe} project management requires approval at critical decision milestones before allowing the \dword{lbnf}/\dword{dune} project to move on to the next step. 
