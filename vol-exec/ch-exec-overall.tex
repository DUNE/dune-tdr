...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The DUNE Science Program and Supporting Facilities}

The international \dword{dune} will be a world-class neutrino observatory and nucleon decay detector designed to answer fundamental questions about elementary particles and their role in the universe. The experiment will carry out a comprehensive program of neutrino oscillation measurements, aiming for the discovery of \dword{cpv} in neutrino oscillations, which may give insight into the origin of the matter-antimatter asymmetry. \dword{dune} will also 
detect and measure the $\nu_\text{e}$ flux from a core-collapse supernova within the Milky Way, should one occur during the lifetime of the experiment. Such a measurement would provide a wealth of unique information about the early stages of core collapse and could signal the birth of a black hole. Its third primary goal is a search for proton decay in several decay modes and for other baryon-number-violating processes. The observation of baryon number violation would represent a ground-breaking discovery in physics, providing a key requirement for grand unification of the forces.

The \dword{dune} experiment, hosted by the U.S. Department of Energy's \dword{fnal}, will consist of a \dword{fd} located approximately \SI{1.5}{km} (almost a mile) underground at the \dword{surf} in South Dakota, USA, \SI{1300}{\km} from \dword{fnal}, and a multi-functional \dword{nd} located at \dword{fnal} in Illinois. The far detector, the subject of this \dword{tdr}, will be a very large, modular \dword{lartpc} with a \fdfiducialmass (\SI{40}{\giga\gram}\footnote{This is almost the mass of a standard fast battleship, for comparison.} %fiducial 
...
The \dword{lbnf}, also hosted by \dword{fnal}, provides infrastructure for this complex system of detectors at the Illinois and South Dakota sites. \dword{lbnf} is responsible for building a beamline at \dword{fnal} to deliver the world's most intense neutrino beam to both \dword{dune} detectors, and providing the deep-underground site for the \dword{fd}. 

The upgrade to \dword{pip2}~\cite{pip2-2013}, a leading-edge, superconducting, linear proton accelerator under construction at \dword{fnal}~\cite{pip2-2013}, will deliver between \num{1.0} and \SI{1.2}{MW} of proton beam power from the \dword{fnal} Main Injector to \dword{lbnf}, which will aim and focus the beam, whereupon the protons will collide with a target, creating a secondary beam from which the intense neutrino flux will emerge, traveling in the direction of the \dword{dune} detectors.  
\dword{pip2} will deliver protons in the energy range \SIrange{60}{120}{\GeV} at the start of \dword{dune} operations. 
...
potentially extending the \dword{dune} science reach. It will also increase the reliability of the \dword{fnal} accelerator complex and provide the flexibility to produce customized  beams tailored to specific scientific needs.  

The intense neutrino beam, the massive \dword{lartpc} \dword{fd}, and the high-resolution
\dword{nd} will provide a rich ancillary science program for the \dword{dune} experiment, beyond its primary goals,   
including accelerator-based neutrino flavor-transition measurements with sensitivity to \dword{bsm} physics, measurements of tau neutrino appearance, measurements of neutrino oscillation phenomena using atmospheric neutrinos,  and  a rich neutrino interaction physics program using the \dword{dune} \dword{nd}, including a wide range of measurements of neutrino cross sections and studies of nuclear effects, and searches for dark matter. 
\fixme{is DM search only available w ND?}
Further advances in \dword{lartpc} 
technology during  \dword{fd} construction may even open up possibilities to observe very low-energy phenomena such as solar neutrinos or even the diffuse supernova neutrino flux -- measurements that require a sensitivity that is presently beyond our reach.
...
The U.S. DOE \dword{lbnf} project is building a facility in South Dakota that will house and provide infrastructure for the first two \dword{dune} \dword{fd} modules and for the \dword{nd} in Illinois.  Figure~\ref{fig:lbnf} shows the overall configuration and Figure~\ref{fig:caverns} is a diagram of the cavern layout for the \dword{fd} at \dword{surf}.  
The organization and management of \dword{lbnf} is separate from that of the \dword{dune} collaboration. \dword{lbnf} is also hosted by \dword{fnal}, and its design and construction are organized as a \dword{doe}/\dword{fnal} project incorporating international partners. 


\begin{dunefigure}[ 	
Underground caverns for DUNE in South Dakota]{fig:caverns}{Underground caverns for the \dword{dune}{} \dword{fd}{} and cryogenics systems at \dword{surf} in South Dakota. The drawing shows the cryostats (red) for the first two far detector modules  in place. Each cryostat is \cryostatlen long (\SI{216}{ft}, approximately the length of two and a half tennis courts), \cryostatwdth wide, and \cryostatht tall (four meters, or about one story of a building, taller than the Parthenon in Athens).}
...
\begin{itemize}
\item  the  technical and conventional facilities for a powerful neutrino beam using the \dword{pip2} upgrade of the \dword{fnal} accelerator 
complex; 

\item  the civil construction, referred to as the \dword{cf}, for the \dword{nd} systems at \dword{fnal}; (Figure~\ref{fig:beamline});

\item the excavation of three underground caverns at \dword{surf} to house the \dword{dune} \dword{fd}. When finished, the north and south caverns will each house two \dword{fd} modules, each of which will be immersed in a cryostat that holds a
 minimum \nominalmodsize fiducial mass of liquid argon. The \dword{cuc} will house cryogenics and data acquisition facilities for all four detector modules;

\item surface, shaft, and underground infrastructure at  \dword{surf} to support 
...
\end{itemize}
\fixme{doesn't lbnf also supply the LAr?}
...
\begin{itemize}
\item \Dword{sp}: In the \dword{sp} technology, ionization charges drift horizontally in the \dword{lar} under the influence of an electric field towards %wire planes on 
...
This design requires very low-noise electronics to achieve readout with good \dword{s/n} because no signal amplification occurs in the liquid. 
...
{The general operating principle of the \dword{sp} \dword{lartpc}. The U, V and X sense wires make up the anode. The right-hand side represents the time projections in two dimensions as the event occurs.}
\includegraphics[width=0.75\textwidth]{TheBoPicture.png} 
\end{dunefigure}

\begin{dunefigure}[The dual-phase (DP) LArTPC operating principle]{fig:DPprinciplech1}{The general operating principle of the \dword{dp} \dword{lartpc}. It also shows the light detectors (PMTs) at the bottom.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Single-Phase LArTPC Technology}
\label{sec:fdsp-exec-splar}

The operating principle of an \dword{sp} \dword{lartpc} (Figure~\ref{fig:LArTPC1ch1}) has been demonstrated by  \dword{icarus}~\cite{Icarus-T600}, \dword{microboone}~\cite{microboone}, \dword{argoneut}~\cite{Anderson:2012vc}, \dword{lariat}~\cite{Cavanna:2014iqa}, and \dword{pdsp}~\cite{Abi:2017aow}. Charged particles passing through the \dword{tpc} ionize the argon, and the ionization electrons drift in an electric field (\efield{}) to the anode planes. Each of the four drift volumes of \dword{lar} is subjected to a strong \efield{} of \spmaxfield, corresponding to a cathode \dword{hv} of \sptargetdriftvoltpos. The maximum drift length is \spmaxdrift.   Figure~\ref{fig:DUNESchematic1ch1} shows the configuration of a \dword{dune} \dword{spmod}.


\begin{dunefigure}[A \nominalmodsize DUNE far detector SP module]{fig:DUNESchematic1ch1}
{A \nominalmodsize \dword{dune} \dword{fd} \dword{spmod}, showing the alternating \sptpclen{} long, \tpcheight{} high (A) and cathode (C) planes, as well as the \dword{fc} that surrounds the drift regions between the anode and cathode planes. On the right-hand cathode plane, the foremost portion of the \dword{fc} is shown in its undeployed (folded) state.}
...
\begin{enumerate}
\item Production of components: stress-testing the production and quality assurance processes of detector components, and mitigating the associated risks for the far detector;
\item Validation of installation procedures: testing the interfaces between the detector elements,
and mitigating the associated risks for the far detector;
\item Operation of the detector with cosmic rays to validate the detector designs and performance; and 
\item Collection of test beam data to measure the physics response of the detector.
\end{enumerate}
...
\fixme{I took out a pgraph - it's too much for our targeted audience IMHO. Could be in SP chapter maybe. Need to remove fig 11 left, too. Anne}
...
\fixme{I would remove the left figure if we remove the paragraph above. Anne}
  ...
  incident electron kinetic energy.} \includegraphics[width=0.47\linewidth]{graphics/dedx_rr_data_v5.pdf}
...
The \dword{nd} will be located \SI{574}{m} downstream from the neutrino beam source and will include three primary detector components, shown in Figure~\ref{fig:neardetectors}. Two of them can move off beam axis, providing access to different neutrino energy spectra. The movement off axis, called \dword{duneprism}, provides a crucial extra degree of freedom for the \dword{nd} measurements and is an integral part of the \dword{dune} \dword{nd} concept. 
...
A \dword{lartpc} begins to lose acceptance for muons with a measured momentum higher than $\sim0.7$~GeV/c because muons will not be contained in the \dword{lartpc} volume.  Because muon momentum is critical to determining the incoming neutrino's energy, a magnetic spectrometer is needed downstream of the \dword{lartpc} to measure the momentum and charge of the muons.  %Measuring  the charge of the muons is particularly important in antineutrino running given that the beam will be an admixture of neutrinos and antineutrinos. 
In the \dword{dune} \dword{nd} concept, an \dword{mpd}, which consists of a \dword{hpgtpc} surrounded by an \dword{ecal} in a \SI{0.5}{T} magnetic field, will make these measurements. The \dword{hpgtpc} provides a lower density medium with excellent tracking resolution for muons from the \dword{lartpc}.  
...
The \dword{dune} concept is based on reconstructing the energy-dependent neutrino spectrum and comparing the far and near sites. The ability to take measurements at the near site in off-axis locations will allow us to 
...
Chapter~\ref{ch:exsum-nd} of this \dword{tdr} volume presents a more complete introduction to the \dword{nd} and further details of the system can be found in the appendices. The DUNE collaboration is now in the process of finalizing studies for the \dword{nd} CDR.
...
as a truly international collaboration with majority international participation. As such, \dword{dune} requires a new organizational and governance model that takes into account the international nature of the project and its relationship
to \dword{lbnf}.
The model used by \dword{cern} to manage constructing and operating the \dword{lhc} and its experiments served as a starting point for the joint management of \dword{lbnf} and \dword{dune}, 
...
The \dword{dune}  \dword{rrb} is part of its international project governance structure, % for the   experiment, 
...
To carry out design and construction work for the \dword{dune} \dword{fd}, \dword{dune} has  formed consortia of institutions, each of which is responsible for an individual detector subsystem. A similar structure will be formed for the \dword{nd} once the final detector concept is selected. The \dword{fd} currently includes nine consortia, including three specific to \dword{sp}, three specific to \dword{dp}, and five common to both technologies:
\begin{itemize}
...
before excavation begins, once Ross Shaft rehabilitation work is complete. \fixme{I think this is done} As each detector 
