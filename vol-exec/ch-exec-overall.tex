...
The plans for the \dword{sp} and \dword{dp} \dwords{tpc} are described briefly in the following sections, again more fully in Chapters~\ref{ch:exec-sp} and~\ref{ch:exec-dp}, respectively, and finally in great detail in Volumes~\volnumbersp{} and~\volnumberdp{} of this \dword{tdr}. 
...
{tab:NDsumm-esch1}{This table gives a high-level breakdown of the three major detector components and the capability of movement for the DUNE ND along with function and primary physics goals.}
...
Chapter~\ref{ch:exsum-nd} of this \dword{tdr} volume presents a more complete introduction to the \dword{nd} and further details of the system may be found in the appendices. 
...
The \dword{inc} is part of the international project governance structure for the  \dword{lbnf} and the  \dword{pip2} projects. The \dword{inc} is composed of representatives from the international funding agencies and  \dword{cern} that make major contributions to the infrastructure. 
The \dword{inc} acts as a highest-level international advisory body to the U.S. \dword{doe} and the  \dword{fnal} directorate on matters related to the program, which include coordination among the international partners. The \dword{inc} is chaired by the associate director for HEP in the \dword{doe} Office of Science, and includes the  \dword{fnal} director in its membership. The council meets once a year and provides pertinent advice on the \dword{lbnf} and \dword{pip2}  projects.
...
The \dword{dune}  \dword{rrb} is part of the international project governance structure for the   experiment. It was established to provide coordination among funding partners and oversight of \dword{dune}. It is composed of representatives of all funding agencies that sponsor the project, and   \dword{fnal} management. The  \dword{rrb} provides focused monitoring of the \dword{dune} collaboration, and also receives updates on the progress of \dword{lbnf},  \dword{pip2}, and the \dword{sbn} program. The  \dword{rrb} receives periodic reports from both the \dword{lbnc} and the \dword{ncg}, which are review committees for \dword{dune} that are charged by and report to the \dword{fnal} directorate. A representative from the \dword{fnal} directorate chairs the \dword{rrb} and organizes regular meetings to monitor progress on the \dword{dune} project.
...
The  \dword{lbnc} is charged by the \dword{fnal} director to review the scientific, technical, and managerial progress, plans and decisions associated with \dword{dune}. The committee chair reports to the \dword{fnal} director. 
The  \dword{lbnc}, composed of internationally prominent scientists with relevant expertise, 
provides regular external scientific peer review of \dword{dune}. It also provides regular reports and candid assessments to the \dword{fnal} director that are also available to the \dword{rrb}, \dword{lbnf}, and \dword{dune} leaderships, and to the funding agencies that support these international projects. The  \dword{lbnc} will review the \dword{tdr} for \dword{dune}, and provide a recommendation to the \dword{fnal} directorate and the \dword{rrb} on whether to endorse the  \dword{tdr}. Upon request from the \dword{fnal} director, the  \dword{lbnc} may task other \dword{dune} and \dword{lbnf} groups to provide more detailed reports and evaluations of specific systems. The chair of the  \dword{lbnc} will participate as a delegate to both the \dword{fnal}-managed \dword{rrb} and the \dword{doe}-managed \dword{inc}. At the respective sessions of the \dword{rrb} and \dword{inc}, the chair will provide reports to the international delegates of  \dword{lbnc} deliberations. The chair of the  \dword{lbnc} is an ex-officio member of  \dword{fnal}'s Physics Advisory Committee.
...
The  \dword{ncg} is charged by the \dword{fnal} director to review the cost, schedule, and associated risks for the \dword{dune} experiment and to provide regular reports to the \dword{fnal} directorate and the \dword{rrb}. This group comprises internationally prominent scientists with relevant experience. 
The  \dword{ncg} will review the  \dword{tdr} for \dword{dune} and will provide a recommendation to the \dword{fnal} directorate and the \dword{rrb} on whether to endorse the  \dword{tdr}. The chair of the  \dword{ncg} will participate as a delegate to both the \dword{fnal}-managed \dword{rrb} and \dword{doe}-managed \dword{inc}. At the respective sessions of the \dword{rrb} and \dword{inc}, the chair will provide reports of  \dword{ncg} deliberations to the international delegates.
...
Close and continuous coordination between \dword{dune} and \dword{lbnf} is required to ensure the success of the combined enterprise. The \dword{efig}  oversees the coordination between the two projects, especially during the design and construction phases and the operational phase of the program.   
...
Chapter~\ref{ch:exec-tc} of this volume provides a more complete introduction to \dword{dune} management and organization; and Volume~\volnumbertc{} of the \dword{tdr} provides even more detail.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% new 15 May
\section{Schedule and Milestones} 

A set of key milestones and dates  have been defined for planning purposes in the development of the \dword{tdr}.  The dates will be finalized once the international project baseline has been defined.  Table~\ref{tab:es-key-dates} shows the key dates and milestones (colored rows) and indicates the way that detector consortia will add subsystem-specific milestones based on these dates (no background color).
...
The schedule for the initial work is driven by the \dlong{cf} (CF) design and construction at each site.

During the initial phase of the project, the far site \dword{cf} is advanced first. 
...
The \dword{doe} project management process requires approval at critical decision milestones that allow the \dword{lbnf}/\dword{dune} project to move to the next step. 
