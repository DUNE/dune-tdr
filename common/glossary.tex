...
\newduneword{xarapu}{X-ARAPUCA}{ARAPUCA design with different \gls{wls} coating on only the external face of the dichroic filter window(s) but with a \gls{wls} doped plate inside the cell}
...
\newduneword{riskprob}{risk probabilities}{The risk probability, after taking into account the planned mitigation activities, is ranked as L (low, $<\num{10}$\%), M (medium,$\SIrange{10}{25}$\%), or H (high, $>\num{25}$\%). The cost and schedule impacts are ranked as L (cost increase $<\num{5}$\%, schedule delay $<num{2}$ months), M ($\SIrange{10}{25}$\% and $\SIrange{2}{6}$ months, respectively) and H ($>\num{20}$\% and $>\num2{2}$ months, respectively).}
